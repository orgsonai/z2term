...
object KkcConverter {
    ...
    /**
     * 辞書に無い**学習ブロック** (利用者が実際に確定した読みの塊) を合成ノードとして
     * ラティスへ入れるときの基準コスト。ここから学習ボーナスを引いた値がノードコストになる。
     *
     * ⚠ **[UNK_COST] を基準にしてはいけない** (0.8.493 で修正)。以前は
     * `UNK_COST - bonus` で組んでいたため、ボーナスが上限 (8500) でもノードコストが 8500 残り、
     * 辞書がその区間を安く分割できるケース (実測: 総コスト 6000 台) には**何度確定しても
     * 永久に勝てなかった**。「同じ塊を毎回手で区切り直さないといけない」の直接の原因がこれ。
     *
     * 学習ブロックは「利用者がその区切りでその表層を選んだ」という実績そのものなので、
     * 未知語の推定コスト (UNK) ではなく**辞書の一般語と同程度**から始めて、使うほど下がる。
     *   count=1 (bonus 3000〜4000) → 6000〜5000  … 1 度確定すれば次から効く
     *   count=2 (bonus 4500) → 4500
     *   count>=3 → 下限 [LEARNED_BLOCK_MIN_COST]
     */
    private const val LEARNED_BLOCK_COST = 9000

    /**
     * 学習ブロック合成ノードのコスト**下限**。
     *
     * ⚠ **これ以上安くしてはいけない** (実測で決めた値)。合成ノードは lc=rc=0 の
     * BOS/EOS 文脈近似なので、**どこへでも安く繋がってしまう**。コストを下げすぎると、
     * 学習した塊がその読みを部分文字列として含む**別の語の途中**にまで割り込み、
     * 辞書が正しく 1 語で変換していた読みを壊す (実測: 4 文字の塊を 1 つ学習しただけで、
     * その塊を先頭に含む別語が軒並み崩れた)。
     *
     * 実測した安全域は「実効コスト 4000〜6000」。4000 未満は割り込み、7000 以上だと
     * 学習が効かない。使い込んでも 4000 で頭打ちにして、**誤変換を増やさない範囲で最強**に
     * 張り付かせる。
     */
    private const val LEARNED_BLOCK_MIN_COST = 4000
    ...
    fun nbest(reading: String, k: Int, context: KkcContext = KkcContext.EMPTY): List<Candidate> {
        ...
        for (i in 0 until n) {
            for (j in i + 1..n) {
                ...
                // (UNK_COST * 文字数) ではなく **1 語分**を基準にする。
                // ⚠ 基準は [LEARNED_BLOCK_COST] であって [UNK_COST] ではない (0.8.493)。
                //   UNK_COST 基準だとボーナス上限でもノードコストが 8500 残り、辞書がその区間を
                //   安く分割できる読み (実測: 総コスト 6000 台) には何度確定しても勝てなかった。
                if (learned != null && (entries == null || entries.none { it.surface == learned.first })) {
                    val c = (LEARNED_BLOCK_COST - bonus).coerceAtLeast(LEARNED_BLOCK_MIN_COST)
