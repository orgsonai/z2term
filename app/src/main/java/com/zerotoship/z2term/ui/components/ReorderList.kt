...
class ReorderState internal constructor(
    ...
    /**
     * 外の一覧が変わったら取り込む。**ドラッグ中は無視する** — 掴んでいる最中に元の並びで
     * 上書きされると、指の下から行が飛ぶため。
     */
...
fun rememberReorderState(spacing: Dp, onCommit: (List<String>) -> Unit): ReorderState {
    ...
    return remember(spacingPx) { ReorderState(spacingPx) { commit.value(it) } }
}
...
fun Modifier.reorderItem(state: ReorderState, id: String): Modifier {
    ...
    return this
        ...
        .graphicsLayer { translationY = if (dragging) state.offsetY else 0f }
}
