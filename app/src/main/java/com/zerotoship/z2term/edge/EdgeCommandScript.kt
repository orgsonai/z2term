...
internal object EdgeCommandScript {
    fun create(token: String, command: String, input: String?, value: String?, arguments: List<String>? = null, inputFile: Boolean = false): String {
        ...
        val pipe = if (inputFile) "< ${path("in")} " else input?.let { "printf %s ${quote(it)} | " }.orEmpty()
