...
internal class EdgePanelPreview(context: Context, private var fields: Map<String, String>,
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val left = (width - w) / 2f
        val top = padding
        paint.color = Color.GRAY; paint.style = Paint.Style.STROKE; paint.strokeWidth = density
        ...
        paint.style = Paint.Style.FILL; paint.color = Color.argb(80, 80, 140, 180)
        canvas.drawRect(px, py, px + panelW, py + panelH, paint)
        paint.style = Paint.Style.STROKE; paint.color = Color.rgb(80, 140, 180)
        canvas.drawRect(px, py, px + panelW, py + panelH, paint)
        paint.style = Paint.Style.FILL; paint.color = Color.GRAY; paint.textSize = 12 * resources.displayMetrics.scaledDensity
