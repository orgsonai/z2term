...
android {
    ...
    lint {
        ...
        //   そちらを落とすのは testDebugUnitTest の CliTranslationCheckTest (0.8.423)。
        //   ⛔ res の検査をここへ足し込まないこと — 二重に判断すると、どちらの言い分を
        //   直せばよいのか分からなくなる。res は lint、CLI はテスト、と持ち場を分ける。
...
// ---------------------------------------------------------------------------
// 自前 ptrace エンジンと LD_PRELOAD シムを APK ビルド時に必ず現ソースから再生成する。
...
val buildZ2rootNative = tasks.register<Exec>("buildZ2rootNative") {
    group = "build"
    description = "z2root と同梱 native/shim を NDK clang で jniLibs/arm64-v8a へ再ビルド"
    val script = rootProject.file("scripts/build-z2root.sh")
    // z2root.c / 各 shim / z2attach.c / スクリプトのいずれかが変われば再ビルド。変わらなければ up-to-date。
