...
**0.8.28-alpha (versionCode 36) — z2root: `/proc` faking for `fakeroot -0`. Faking the `get*id` syscalls alone still leaked the host app uid/gid when a guest read `/proc/self/status` (or `/proc/<pid>/status`) directly — `id -a`, `dpkg`/`apt` and various scripts parse it. z2root now intercepts `read()` on those status files and rewrites the `Uid:` / `Gid:` lines to `0` and blanks the `Groups:` line, length-preserving (so the rest of the file and the read return value stay intact). Consistent with the existing `getuid`/`getgid`/`getgroups` fakery. Verified on-device: `cat /proc/self/status` shows `Uid: 0`, `Gid: 0`, empty `Groups:`, while `/etc/os-release` and `/proc/self/stat` are untouched. PRoot stays the default; z2root remains experimental behind the hidden engine selector (tap the version 7x).**
