...
**0.8.83-alpha (versionCode 91) — z2root: `name_to_handle_at` のパス変換を追加 (封じ込めギャップを塞ぐ).** seccomp 監査 (§13.4) の残ギャップを是正。`name_to_handle_at`(264) は `dirfd, pathname, ...` を取り、トレース対象外だと絶対パス pathname がホスト root 起点で解決＝封じ込めを素通りしていた。`kTraceSyscallsBase` と `syscall_paths()` に追加し、pathname (arg1) を dirfd (arg0) 基準で rootfs 配下へ変換するようにした (既定は最終 symlink 非追従、`AT_SYMLINK_FOLLOW`=0x400 指定時のみ follow＝`linkat` と同様)。対になる `open_by_handle_at`(265) は path ではなく不透明な `file_handle` を取るため変換可能なパスが無く、かつ `CAP_DAC_READ_SEARCH` 必須で untrusted_app では EPERM で弾かれるため非対象 (コードにコメント記載)。これで §13 のコード監査ギャップは解消。
