...
**0.8.24-alpha (versionCode 32) — The in-house ptrace engine "z2root" gains `--link2symlink` (hardlinks are emulated as symlinks to work around Android app-internal storage rejecting `link()` with `EACCES`; needed for `apt`/`dpkg` and some `apk`; on aarch64 `linkat` is rewritten to `symlinkat` via `NT_ARM_SYSTEM_CALL`). z2root was also launched for the first time inside the real app on-device: the wiring works (engine selection launches `libz2root.so` and passes its args), but it turns out that in the real app (`untrusted_app`) the OS forbids `execve` of files in the app's own data dir (W^X / SELinux), so the current approach of `execve`-ing the rootfs loader cannot start (PRoot avoids this via `libproot_loader`). z2root's next milestone is a `nativeLibraryDir`-resident loader that maps ELFs into anonymous executable memory. The engine remains experimental and selectable from the hidden engine selector (tap the version 7x); PRoot is the default and the usable engine. FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
