...
**0.8.46-alpha (versionCode 54) — verification build to settle the SSH-reset "stale engine" question. This session reproduced the reset over SSH (PTY `ssh -tt` only; non-PTY succeeds) and observed old-engine symptoms even after a *full app restart* (fresh engine pids): `dbsrv.log` shows `ttyname fails for openpty device` (the bug 0.8.41 already fixed) and `z2root_trace.log` is never generated despite the `.z2root_trace_on` sentinel. The earlier "relic process holding a deleted `.so`" diagnosis was undermined: from inside the proot tab `/data` and `/data/app` are simply not in the mount namespace, so `strings`/`ls` on those paths are meaningless (empty → false `z2trc=0`, always "No such file" → false "DELETED"). The only reliable signal is runtime behavior, and after a clean restart it still shows the pre-0.8.41 engine. The source wiring is confirmed correct and committed: `ProotLauncher.kt:130` injects `Z2ROOT_TRACE` from the sentinel, line 253 binds `sharedHomeDir:/root`, line 319 appends `z2rootEnv` to the engine env; the local `app-full-release.apk` (sha256 of its `libz2root.so` = `3384ecd7…edfd19c2`, `z2trc`=6, `FAKE chmod`=1) is the right trace build. Conclusion: the device is most likely running an APK older than this source. This version bump gives a freshly-visible version number so the next session can confirm, after an `adb install -r` of this build + full restart, whether the installed engine is current — if `ttyname fails`/no-trace persist on a known-fresh install, it's a real bug, not staleness. No functional code change in this version; diagnostic instrumentation intentionally retained (CLAUDE.md exception).**
