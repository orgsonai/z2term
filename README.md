...
**0.8.4-alpha (versionCode 12) — M13/0.8.4: 日本語長文の自動ブロック分割＋文まるごと一括予測 / 外部SD認識 / Android ホスト bind(実験)**

Milestone 7〜12 で SFTP、GUI (Xvnc+VNC)、複数 GUI タブ、IME 学習、英語 UI、横画面キーボード、スクロールバック検索、セッション復元、root 端末向け chroot エンジン、GUI 音声/動画再生などを実装。0.8.4 で日本語かな漢字変換を長文向けに強化。
