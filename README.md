...
**0.8.80-alpha (versionCode 88) — foss Alpine で zsh 起動失敗 (`execve(/bin/zsh) ENOENT`) を根治: シェル存在判定が絶対 symlink を誤判定.** ログインシェル既定 `/bin/zsh` が rootfs に無いとき、`ProotLauncher.resolveShell` は `/bin/sh`→`/bin/ash` 等へフォールバックする設計だが、`shellExists` の存在判定 `File.exists()` が **絶対 symlink (Alpine minirootfs の `/bin/sh -> /bin/busybox` 等) をホストの filesystem root 起点で解決**し (ゲストの rootfs ではなく host:/bin/busybox を見る) 常に不在判定 → フォールバック候補が全滅し bogus な `/bin/zsh` がそのまま execve され `No such file or directory` で起動失敗していた。`shellExists` を **NOFOLLOW でリンク自体の存在を見る** (`Files.exists(.., NOFOLLOW_LINKS)`) よう修正し、busybox symlink のシェルを正しく検出してフォールバックが効くようにした。SHELL env 解決 (`resolveLoginShell`) も同経路を通るため同時に解消。
