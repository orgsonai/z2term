...
**0.8.62-alpha (versionCode 70) — re-fixes B-3 (`git clone` hardlink spoof) after finding the 0.8.58 fix never engaged on the running 0.8.61 engine.** C probes on the live engine showed all 200 linkat copy-fallbacks producing a different-inode dest with the stat-spoof firing zero times — disproving the 0.8.58 "compiled, probably works" assumption. Root cause: the old `linkcopy_record` re-`stat()`'d the dest's host path *after the fact* to capture its inode, which could diverge from the inode the tracee reads via `newfstatat`, so the match always missed. Fix: `copy_for_link` now `fstat()`s the just-created output fd to capture the dest inode at creation (guaranteeing it equals the entity the tracee later sees), and `linkcopy_record` takes those values by argument instead of re-stat'ing. Compile-verified; e2e (git clone passing hardlink verification) needs an APK with this fix installed via the app UI. See `docs/Z2ROOT-BUILD-PARITY-HANDOFF.md` §10.
