...
**0.8.25-alpha (versionCode 33) — Work started on z2root's next milestone: a `nativeLibraryDir`-resident in-house ELF loader (`--loader` mode) that maps ELFs into anonymous executable memory and jumps to them, so it never `execve`s files in the app's own data dir (which `untrusted_app` forbids under W^X / SELinux — the blocker found in 0.8.24). WIP / not yet working: building `libz2root.so` as `-static-pie` cleared the first on-device crash (the bionic linker's `/proc/self/exe` resolution being corrupted by the tracer under ptrace), but the loader still segfaults before reaching its own entry — under investigation (suspected static-PIE bionic startup). PRoot remains the default and the usable engine; z2root stays experimental behind the hidden engine selector (tap the version 7x). FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
