...
**0.8.44-alpha (versionCode 52) — Settings now shows the engine each tab is *actually* running on. The "Execution engine" section gains a read-only "This tab is running on: PRoot / z2root / chroot / Android sh" row sourced from the real launch result (`TerminalSession.actualEngine`, set from `ProotLauncher.resolveLaunchEngine()` or the chroot path), not the selector chip — so it stays honest when the chosen engine falls back (z2root not bundled → proot, or chroot probe fails → proot). The 7-tap version row that shows/hides the engine selector now has a 3-second cooldown after it toggles, so rapid tapping can't immediately flip it back. SSH-reset investigation moves on-device (full release): the earlier on-device dropbear log showed only `Exit before auth` (0 fails) — the ad-hoc ssh client had no key in `authorized_keys` and dropbear ran with `-s` (password auth off), so that run never reached the auth-then-reset path; a proper client-key setup is needed to reproduce. Note the installed full-release APK has no active `Z2ROOT_TRACE` injection (the trace build's sentinel-gated `z2rootEnv()` code was never committed; the working tree is clean), so the trace instrumentation must be reconstructed to confirm whether the `fchmod`/`fchmodat` fake actually fires.**
