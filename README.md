...
**0.8.68-alpha (versionCode 76) — fixes the musl/Alpine guest startup crash (`Alpine Linux` → `exitCode=-1`) introduced by the 0.8.67 loader.** The 0.8.67 `skip_reloc` gate (below) fixed the loader's double-relocation of `ld.so`, but the loader still unconditionally pre-biased the program-header copy passed as `AT_PHDR` (`p_vaddr += base`). That pre-bias was meant only for bionic static-PIE direct-load (where bionic assumes `load_bias == 0`). For the dynamic-binary path that loads `ld.so`, musl's `ld.so` *recomputes its own load base from `AT_PHDR`'s `PT_PHDR.p_vaddr`*, so the pre-bias is counted twice → wrong base → `SIGSEGV`. glibc's `ld.so` bootstraps GOT-relative and ignores `AT_PHDR`, so Arch/Kali (glibc) never hit it; Alpine (musl) did. Fix: gate the `AT_PHDR` pre-bias on `!skip_reloc`, so it applies only to the static-PIE direct-load path and never to the `ld.so` path. Compile-verified; e2e needs an APK with this fix installed via the app UI. See `docs/Z2ROOT-BUILD-PARITY-HANDOFF.md` §10.
