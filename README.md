...
**0.8.23-alpha (versionCode 31) — The in-house ptrace engine "z2root" now reverse-maps `getcwd` results under bind mounts: previously `pwd` inside a bind-mounted dir (the app binds `<shared_home>:/root`) leaked the host path; it now correctly shows the guest path via `host_to_guest()`. The prior release added fakeroot (`-0`) emulation: `getuid`/`getgid` family report 0, `getgroups` hides the host app's gids (`id` shows just `groups=0(root)`), failing `set*id`/`chown` are masked as success, and stat reports `uid`/`gid` as 0. Verified in the dev environment. The engine is still experimental and selectable from the hidden engine selector (tap the version 7x); it falls back to PRoot when not bundled. FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
