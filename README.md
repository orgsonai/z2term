...
**0.8.72-alpha (versionCode 80) — Per-distro HOME isolation (fixes cross-distro native breakage).** `/root` (HOME) stays a single `shared_home` for ordinary files (documents, git repos, `~/.claude`, …), but a few **arch-dependent subdirectories are now overlaid per-distro**: `.local .cache .npm .npm-global .nvm .cargo .rustup .config`. Each is bound from `filesDir/home_overlay/<distroId>/<sub>` over `/root/<sub>` (proot adds them after `-b shared_home:/root`; the real-chroot path does the same after its `/root` bind). This root-causes the long-standing problem where installing native HOME tools (npm-global `node`/`claude`, compiled `~/.cache` addons, nvm's `node`) under musl/Alpine broke the glibc/Arch copy — and vice versa. Migration note: existing `shared_home/<sub>` contents become shadowed (hidden, not deleted) from each distro's view. See `docs/ja/DESIGN-SPEC.md` §HOME 隔離.
