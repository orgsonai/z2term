...
**0.8.85-alpha (versionCode 93) — IME: 結合読みを 1 語コスト基準にし、連続確定 run を結合ブロックとして学習 (頻用の外来語/塊が 1 ブロックに繋ぎ止まる).** 動的ブロック分割学習 (0.8.71/0.8.74) の続き。①学習ブロックの合成ノードコストを長さ比例の未知かな (`UNK_COST × 文字数`) ではなく **1 語分の `UNK_COST` 基準** にした。辞書外の外来語 (`びるど`→`ビルド`。`びるど` は lex に無い) が `UNK_COST×3` で辞書分割に頻度ボーナスを足しても勝てず 1 ブロックにまとまらなかったのを、1 語基準で「使うほど下がる」ボーナスが分割コストを越えるようにした (`BlockLearningTest` で検証)。②自動分割で割れた語は全体読みが履歴に入らなかったため、`ComposingState` が同一スプリット run の連続確定を `committedRun` に貯め、run が尽きた時と一括確定時に結合読み→結合表層を `ImeHistoryStore` へ記録 (読み長 2〜6 に限定)。これで「びる」「ど」と割れて確定した塊も次回 ① の対象になり 1 ブロックへ繋ぎ止まる。
