...
**0.8.24-alpha (versionCode 32) — The in-house ptrace engine "z2root" was verified for the first time inside the APK under SELinux on a real device, and it works (execve+ptrace from `nativeLibraryDir`, musl loader rewriting for dynamic ELFs, absolute-symlink resolution, fakeroot `-0`, and `getcwd` reverse-mapping all pass on-device; `ls -l`/`stat` showing owner as `root` was also confirmed). It also adds `--link2symlink`: Android's app-internal storage rejects `link()` with `EACCES`, so hardlinks are emulated as symlinks (needed for `apt`/`dpkg` and some `apk`; on aarch64 `linkat` is rewritten to `symlinkat` via `NT_ARM_SYSTEM_CALL`). Confirmed `ln a b` succeeds on-device. The engine is still experimental and selectable from the hidden engine selector (tap the version 7x); it falls back to PRoot when not bundled. FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
