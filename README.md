...
**0.8.39-alpha (versionCode 47) — the Linux GUI (`z2gui`: Xvnc + openbox + a terminal) now actually displays under the z2root engine, end to end. After 0.8.38 let the X server start, the VNC screen still stayed black ("Connection reset"): Alpine's `Xvnc` is built against musl, which calls `accept(2)` as syscall 202, but Android's untrusted_app seccomp policy forbids `accept` (bionic only ever uses `accept4`, so 202 isn't on the allowlist) — every incoming VNC connection hit a `SIGSYS` and z2root could only fake it away, so the server never got a usable client socket (each connection came in as "accepted: ::0" and was dropped). Re-running the trapped syscall as `accept4` from the `SIGSYS` stop proved unreliable on aarch64 (the syscall is skipped and the PC can't be cleanly rewound). Fix: a tiny, libc-agnostic `LD_PRELOAD` shim (`libz2accept.so`, raw `svc`, no libc deps) that rewrites `accept()` → `accept4(...,0)`; z2root preloads it for every guest process, so musl servers call the allowed `accept4` and never trip the seccomp trap. Verified on-device in the real app (untrusted_app): z2root + Alpine + GUI now negotiates the full RFB handshake (`accepted: 127.0.0.1::…`, protocol 3.8, pixel format) and renders the openbox desktop with an xterm. This also unblocks z2root SSH servers (dropbear) that `accept`.**
