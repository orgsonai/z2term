...
**0.8.45-alpha (versionCode 53) — the SSH-reset "fix doesn't fire on device" was a *stale APK*, not z2root logic; plus file-based `Z2ROOT_TRACE`. Root cause: `assembleFullRelease` was packaging an *old* `libz2root.so` (pre-0.8.41 — no `/proc/self` ttyname fix, no chmod fake) because Gradle's native-lib merge reused a stale `merged_jni_libs`/`stripped_native_libs` intermediate instead of the freshly `build-z2root.sh`-built `.so` in `src/main/jniLibs/`. So every prior on-device SSH test ran an outdated engine — which is exactly why the fixes seemed not to fire. The smoking gun on a correct-source build is `ttyname fails for openpty device` (the bug 0.8.41 already fixed), and no trace log despite the sentinel. Workaround: before flashing, verify `unzip -p <apk> lib/arm64-v8a/libz2root.so | strings` contains the expected symbols, and `rm -rf app/build/intermediates/{merged_jni_libs,merged_native_libs,stripped_native_libs}/fullRelease` to force a re-merge. Instrumentation: `Z2ROOT_TRACE` now appends to a file when set to an absolute path (`trc_init()`) so it no longer corrupts the SSH PTY via stderr, and logs `[z2trc] FAKE chmod` when the `fchmod`/`fchmodat` `EPERM`→0 fake fires; `ProotLauncher` injects `Z2ROOT_TRACE=<shared_home>/z2root_trace.log` only when a `.z2root_trace_on` sentinel exists in shared_home (OFF by default, safe in release). Diagnostic instrumentation intentionally retained (CLAUDE.md exception).**
