...
**0.8.90-alpha (versionCode 98) — `z2help` / `z2term`: 端末から独自 `z2*` コマンドの早見表を引けるようにした.** ディストロに `z2adb` 等の独自ヘルパーを多数注入しているが、端末側に一覧を引く手段が無かった。`/usr/local/bin/z2help` を起動毎に注入 (proot/z2root/chroot 全経路) し、引数なしで全 `z2*` コマンドの分類済み早見表 (先頭にアプリ版数も併記) を表示する。本体は静的テキストで quote 付き heredoc に入れる ([`Z2HelpScript.kt`](app/src/main/java/com/zerotoship/z2term/proot/Z2HelpScript.kt))。あわせて **`z2term` を当面 `z2help` の薄いエイリアスとして予約**し (`exec z2help`)、将来 `z2term` を別用途のコマンドへ差し替えられるよう名前だけ確保した。HANDBOOK §11 早見表にも `z2help` / `z2term` を追記。

Previously (0.8.89-alpha, versionCode 97) — `z2adb`: adb サーバを自己 exec せず先行起動するようにし、z2root 上で実接続できるようにした.** 実機 e2e で、`z2adb status`/`pair`/`connect` が内部で adb サーバを自動起動する際に `adb: execl returned -1: No such file or directory` で失敗するのを発見。真因は z2root が `/proc/self/exe` を APK 内の `libz2root.so` と返すため、adb クライアントが daemon を**自パスへ execl して再起動**しようとすると ENOENT で落ちること (adb 全般の問題で z2adb 固有ではない)。`Z2AdbScript.kt` に **自己 exec しない `adb nodaemon server` を background で先行起動**する `start_server` を追加し、`ensure_adb` から呼ぶようにした。起動前に `/proc/net/tcp{,6}` を見て `tcp:5037` (= `ADB_SERVER_SOCKET` のポート) が既に LISTEN なら立てない**冪等ガード** (`server_up`) を入れ、二重 bind による `Address already in use` の abort を避ける。以降のクライアントは fork せず既存サーバに接続でき、LADB 相当のセルフ adb が z2root/proot で実際に成立する。

Previously (0.8.88-alpha, versionCode 96) — `z2adb` ヘルパーを追加 (PC なしで端末自身の adb に繋ぐ「セルフ adb」).** 端末で「adb が無くて不便」を解消するため、`/usr/local/bin/z2adb` を起動毎に注入 (proot/z2root/chroot 全経路)。Android 11+ の開発者オプション → ワイヤレスデバッグを ON にすれば、`z2adb setup` で distro に adb クライアントを導入 (apk: `android-tools` / apt: `adb` / pacman: `android-tools` を自動判定)、`z2adb pair <ポート> [6桁コード]` でペアリング、`z2adb connect <ポート>` で接続でき、以降は `z2adb shell` / `pm` / `logcat` 等を**自端末**に対して実行できる (LADB 相当)。宛先はポートのみなら `127.0.0.1` を補い、`setup`/`pair`/`connect`/`status`/`help` 以外はそのまま adb へ passthrough。PRoot/z2root は TCP を素通しする (dropbear/SSH と同経路) ため localhost に届き、root も USB も不要。
...
Previously (0.8.84-alpha, versionCode 92) — z2root: execve 書き換えの argv 長制限を撤廃 (大きい argv の exec が ENOENT で落ちる退行を修正).** クロスディストロ e2e で、Kali の `apt-get install python3` が dpkg の byte-compile (`python3.13 -E -S py_compile.py <287ファイル>`) 段で `cannot execute: required file not found` (execve ENOENT) で失敗するのを発見。`rewrite_execve` (`z2root.c`) が ①固定長 `char blob[8192]` で argv blob が 8KB 超のとき書き換えを丸ごとスキップ→path レジスタにゲストパスが残ったまま execve→ENOENT、②`MAX_ARGS 256` で argv を 256 個に切り捨て、の二重制限を持っていた。argv 読み取りを上限なしの動的確保 (realloc) に、`blob`/`parts`/`ptrs` を argv サイズ依存の `malloc` に変更し `MAX_ARGS` を撤去。これで argv 総量に依らず exec が成立する (カーネル ARG_MAX 2MB の範囲)。Alpine/Ubuntu の cmdtest は非ゼロ 0 件で、本修正後の Kali 再検証が残る。

Previously (0.8.83-alpha, versionCode 91) — z2root: `name_to_handle_at` のパス変換を追加 (封じ込めギャップを塞ぐ).** seccomp 監査 (§13.4) の残ギャップを是正。`name_to_handle_at`(264) は `dirfd, pathname, ...` を取り、トレース対象外だと絶対パス pathname がホスト root 起点で解決＝封じ込めを素通りしていた。`kTraceSyscallsBase` と `syscall_paths()` に追加し、pathname (arg1) を dirfd (arg0) 基準で rootfs 配下へ変換するようにした (既定は最終 symlink 非追従、`AT_SYMLINK_FOLLOW`=0x400 指定時のみ follow＝`linkat` と同様)。対になる `open_by_handle_at`(265) は path ではなく不透明な `file_handle` を取るため変換可能なパスが無く、かつ `CAP_DAC_READ_SEARCH` 必須で untrusted_app では EPERM で弾かれるため非対象 (コードにコメント記載)。これで §13 のコード監査ギャップは解消。

Previously (0.8.82-alpha, versionCode 90) — z2root: `truncate` と xattr by-path のパス変換を追加 (封じ込めギャップを塞ぐ).** seccomp 監査 (§13.4) で見つけた未変換ギャップを是正。z2root は chroot を使わず ptrace パス変換で封じ込めるため、**トレース対象外の「絶対パスを取る」syscall はホスト root 起点で解決＝封じ込めを素通り**していた。`truncate`(45) と xattr by-path 群 (`setxattr`/`lsetxattr`/`getxattr`/`lgetxattr`/`listxattr`/`llistxattr`/`removexattr`/`lremovexattr` = 5/6/8/9/11/12/14/15) を `kTraceSyscallsBase` と `syscall_paths()` に追加し、絶対パスを rootfs 配下へ変換するようにした (`l*` 版は no-follow、非 `l` 版は follow)。`f*` 版 (`ftruncate`/`fsetxattr` 等) は fd ベースで openat 由来の翻訳済み fd を使うため非対象。`tar`/`rsync`/`cp -a` 等の xattr 利用が絶対パスでも正しく rootfs を指す。

Previously (0.8.81-alpha, versionCode 89) — z2root seccomp: io_uring を自前フィルタで明示 deny (防御の多層化).** seccomp 監査 (`Z2ROOT-BUILD-PARITY-HANDOFF.md §13`) で、`io_uring_*`(425-427) が z2root 自身のフィルタでは `RET_ALLOW`（素通り）で、安全性が**外側の Android untrusted_app seccomp の SIGSYS → トレーサの ENOSYS 化に依存**していたことを確認。io_uring は submission ring 経由で openat/read/write 等を非同期実行し ptrace/seccomp の per-call トラップを丸ごと素通りする（パス変換も fakeroot 偽装も効かない最危険経路）。`install_seccomp_filter` の BPF に **`SECCOMP_RET_ERRNO(ENOSYS)` 終端と deny 比較ブロック**を追加し、io_uring を **z2root 自前で ENOSYS** へ倒すようにした（Android がトラップしないコンテキストでも確実に安全な旧経路へフォールバック）。Android が io_uring を `RET_TRAP` する現行端末では action 優先順 (TRAP>ERRNO) で従来どおり SIGSYS 経路を通る＝挙動不変。`statx`/`openat2` は監査でトレース＆パス変換済みと確認（問題なし）。**未対処の別ギャップ** (`truncate`(45)・xattr by-path・`name_to_handle_at`) は §13 に記録。

Previously (0.8.80-alpha, versionCode 88) — foss Alpine で zsh 起動失敗 (`execve(/bin/zsh) ENOENT`) を根治: シェル存在判定が絶対 symlink を誤判定.** ログインシェル既定 `/bin/zsh` が rootfs に無いとき、`ProotLauncher.resolveShell` は `/bin/sh`→`/bin/ash` 等へフォールバックする設計だが、`shellExists` の存在判定 `File.exists()` が **絶対 symlink (Alpine minirootfs の `/bin/sh -> /bin/busybox` 等) をホストの filesystem root 起点で解決**し (ゲストの rootfs ではなく host:/bin/busybox を見る) 常に不在判定 → フォールバック候補が全滅し bogus な `/bin/zsh` がそのまま execve され `No such file or directory` で起動失敗していた。`shellExists` を **NOFOLLOW でリンク自体の存在を見る** (`Files.exists(.., NOFOLLOW_LINKS)`) よう修正し、busybox symlink のシェルを正しく検出してフォールバックが効くようにした。SHELL env 解決 (`resolveLoginShell`) も同経路を通るため同時に解消。

Previously (0.8.79-alpha, versionCode 87) — foss フレーバーの整備: 表示名分離・proot/talloc 同梱除外・ライセンス表示の実態整合・起動時ダウンロード確認.** foss release のランチャー表示名を **"Z2Term FOSS"** に分離し full release と区別。foss APK からは third-party prebuilt の **PRoot / talloc / Alpine rootfs を完全除外**（実行エンジンはソースビルドの z2root）し、`unzip -l` で検証済。OSS ライセンス一覧 (`OssComponents`) も実体に揃え、foss では同梱しない proot/talloc 等を非表示にする `onlyFullFlavor` を導入。配布物が同梱ディストロを持たない場合に起動時へ**ダウンロード確認ダイアログ**を表示。ビルドは **CPU 論理コア半分** (`--max-workers=nproc/2`, `scripts/gw.sh` が自動付与) を既定化。

Previously (0.8.78-alpha, versionCode 86) — z2root: musl ld.so の ET_EXEC 明示起動不可問題を根治 (`--loader-exec`).** musl の `ld.so` は **動的 ET_EXEC (非PIE) を「コマンドとして明示起動」できず** `Not a valid dynamic program` で落ちる。このため Alpine(musl) では glibc/musl の ET_EXEC バイナリ (claude 本体・`cc` 等) が z2root で起動できなかった。`z2root.c` に **`--loader-exec` 経路**を新設し、本体と `ld.so` を両方マップして**カーネルが `PT_INTERP` 経由で exec したのと同じ初期スタック/auxv** (`AT_PHDR`/`AT_ENTRY`=本体, `AT_BASE`=`ld.so` の load base) を組んで `ld.so` のエントリへ分岐する (proot loader 相当)。これで musl は「インタプリタとして起動された」と判定し本体を relocation して起動する。振り分けは **interp が `ld-musl*` かつ対象が ET_EXEC のときだけ**で、glibc `ld.so` (ET_EXEC を明示起動で受ける既存 Arch claude 経路) は非対象として温存。あわせて `scripts/z2root-cmdtest.sh` を git 未導入時は skip し、末尾 echo による rc マスクを外して実 rc を非ゼロ一覧へ反映するよう修正。

Previously (0.8.76-alpha, versionCode 84) — 端末コピーの改行修正 + クリップボード履歴 + z2version に実行中OS + 掴めるスクロールバー.** (1) 選択コピー: ハード行の行末スペース埋めを除去し、ソフト折り返し行は改行を入れず連結するよう `getRangeText` を修正 (旧: 全行が右端まで空白埋めで改行が埋もれ、コピー結果が空白だらけだった)。 (2) クリップボード履歴: システムクリップボードの変化を監視 (`OnPrimaryClipChangedListener` + 前面復帰時取り込み) して `ClipboardHistoryStore` に永続化。📋 貼付ボタンの**ダブルタップ**で履歴シートを開き、過去のコピーを選んで貼り付け可能。 (3) `z2version`: ゲストの `/etc/os-release` (PRETTY_NAME) と `uname -srm` から**実行中の OS と kernel** を追加表示。 (4) 端末右端に**掴めるスクロールバー**を追加 (scrollback がある時のみ表示、つまみドラッグで遡れる)。

Previously (0.8.75-alpha, versionCode 83) — z2root の最長一致 bind 修正 (項目4 再発の根治).** 0.8.73 で `.claude/downloads` を per-distro 隔離したが、**z2root エンジンでは隔離が効かず**、Alpine で musl 版 claude を入れると Arch の glibc 本体まで上書きされ双方起動不可になる現象が再発していた。真因は `z2root.c` のパス変換 (`translate_abs`/`host_to_guest`) が bind を**登録順の最初一致**で解決しており、先に登録される親 bind `/root` が子 bind `/root/.claude/downloads` を覆い隠していたこと (proot は最長一致なので効いていた engine 差)。両変換関数を**最長一致 (最も具体的＝guest_len 最長の bind 優先)** に修正。
...
Previously (0.8.72-alpha, versionCode 80) — Per-distro HOME isolation (fixes cross-distro native breakage).** `/root` (HOME) stays a single `shared_home` for ordinary files (documents, git repos, most of `~/.claude`, …), but a few **arch-dependent subdirectories are now overlaid per-distro**: `.local .cache .npm .npm-global .nvm .cargo .rustup .config .claude/downloads`. Each is bound from `filesDir/home_overlay/<distroId>/<sub>` over `/root/<sub>` (proot adds them after `-b shared_home:/root`; the real-chroot path does the same after its `/root` bind). This root-causes the long-standing problem where installing native HOME tools (npm-global `node`/`claude`, the `~/.claude/downloads` native binary, compiled `~/.cache` addons, nvm's `node`) under musl/Alpine broke the glibc/Arch copy — and vice versa. Migration note: existing `shared_home/<sub>` contents become shadowed (hidden, not deleted) from each distro's view. See `docs/ja/DESIGN-SPEC.md` §HOME 隔離.

Previously (0.8.71-alpha, versionCode 79) — Japanese IME: frequency-learned dynamic block segmentation.** The block boundaries used for per-block prediction are no longer fixed by dictionary cost alone — they now learn from how often you confirm a given reading-block. A reading that kept mis-splitting (e.g. こまんど → こ / まんど, forcing you to manually re-merge to コマンド every time) now **auto-merges into one block from the next use** once you've confirmed it once or twice. Mechanism: confirmed `(reading → surface)` pairs already tracked in `ImeHistoryStore` feed back into the `KkcConverter.nbest` lattice as a per-span cost reduction (frequency + recency scaled), strong enough to beat the katakana penalty after 1–2 confirmations; readings absent from the dictionary get a synthesized node with the learned surface. Unlearned readings are unchanged. See `docs/ja/DESIGN-SPEC.md` §6.2.1.

Previously (0.8.70-alpha, versionCode 78) — UX fixes after Alpine/z2root startup was confirmed working.** Three changes: (1) a new **`z2version`** terminal command prints the running app's version (`versionName`/`versionCode`/flavor/package/execution engine/rootfs generation), rewritten on every launch so APK↔guest version mismatches are trivial to spot (`z2version --short` for one line); (2) the app now **always starts with a single fresh tab** instead of reopening every previously-open tab; (3) after the hidden 7-tap engine toggle fires, the version row is **non-tappable for 3s** (previously taps were accepted but ignored, which felt unnatural). Note on multi-distro use: `/root` (HOME) is a single `shared_home` bound across *all* distros, so tools installed into HOME (npm global, `~/.local`, `~/.cache`, …) are shared — installing e.g. `claude` under musl/Alpine can break the glibc/Arch copy. Per-distro isolation of those dirs is a known follow-up.

Previously (0.8.69-alpha, versionCode 77) — actually root-causes the musl/Alpine guest startup crash (`Alpine Linux` → `exitCode=-1`).** Both 0.8.67 (`ld.so` double-reloc) and 0.8.68 (`AT_PHDR` pre-bias) only patched *parts* of the symptom. A real-device trace nailed it: just before the `SIGSEGV` (`si_addr=0x132c0`, i.e. base computed as 0) the loader `open`s `ld-musl-aarch64.so.1` **itself**. Because `ld-musl` has **no `PT_INTERP`** (it *is* the interpreter), `plan_exec`'s `read_elf_interp` returns 0, so it falls out of the dynamic path (case 2) into the **static path (case 3) with `skip_reloc=0`** — the loader then applies its RELATIVE/RELR relocation help **and** the `AT_PHDR` pre-bias to a binary that self-relocates. Per musl's `ldso/dlstart.c`, `base = aux[AT_BASE]; if(!base) scan AT_PHDR for PT_DYNAMIC, base = runtime &_DYNAMIC − ph->p_vaddr` — with the pre-biased phdr the `PT_DYNAMIC.p_vaddr` is counted twice → `base = 0` → `SIGSEGV`. glibc bootstraps GOT-relative and ignores `AT_PHDR`, so Arch/Kali (glibc) never hit it. The true root cause is that the loader was treating *self-relocating glibc/musl ELFs* as if they were bionic static-PIE. Fix: in `load_elf_and_jump`, detect bionic ELFs via the `.note.android.ident` PT_NOTE (owner `Android`; only Android NDK output carries it) and gate **both** the relocation help and the pre-bias on `apply_loader_reloc = (!skip_reloc && is_bionic)`. glibc/musl get no loader help and self-relocate correctly. Compile-verified; e2e needs an APK with this fix installed via the app UI. See `docs/Z2ROOT-BUILD-PARITY-HANDOFF.md` §10.

Previously (0.8.67-alpha, versionCode 75) — truly root-causes and fixes the guest startup regression (`Arch Linux ARM` → `exitCode=-1`).** The 0.8.62–0.8.64 stat-spoof changes below were a **misdiagnosis**: that line of work is real (it fixes B-3 git-clone hardlinks) but had nothing to do with the startup death. A diagnostic trace with a full register dump on SIGSEGV pinned the real cause: z2root's custom ELF loader (`load_elf_and_jump`, added in 0.8.59) applies `R_AARCH64_RELATIVE`/RELR relocations to **`ld-linux-aarch64.so.1`**, which the dynamic-binary launch path loads for every guest program. But glibc/musl/bionic `ld.so` *self-relocates* in `_dl_start`, so applying the load bias a second time doubles every RELATIVE-relocated pointer → `blr x8` with `x8 = real_ptr × 2` → instruction-fetch SIGSEGV. Decisive evidence: `pc == si_addr == x8 == 0xf8789f5640 == 0x7c3c4fab20 × 2`. Fix: gate the loader's relocation with `skip_reloc` — the dynamic path (loading `ld.so`, which self-relocates) passes `--loader-noreloc` to suppress it; the static-PIE direct-load path keeps `--loader` (the 0.8.59 behavior for non-self-relocating bionic static-PIE). Compile-verified; e2e needs an APK with this fix installed via the app UI. See `docs/Z2ROOT-BUILD-PARITY-HANDOFF.md` §10 "0.8.67 で起動退行を真に根治".

Previously (0.8.64-alpha, versionCode 72) — *believed* to fix the 0.8.62 startup regression but did not (the regression was the loader double-relocation above, unrelated to this stat work; this change is still valid for B-3):** 0.8.63 tightened the B-3 `linkcopy` stat-spoof match key from inode-only to the full `(dev, ino)`, but that gave near-zero added discrimination: the copied dest lives in the rootfs bind, i.e. on the *same host `/data` partition* as every other guest file, so `st_dev` is a constant across the whole rootfs and `(dev, ino)` reduces to inode-only. Unrelated boot-time files whose inode collided with a recorded dest were still falsified, killing the guest. Fix: replace inode matching with **path correlation** — record the copy's destination *host path*, and at `newfstatat`/`statx` *entry* resolve the stat target's host path; only when it equals a recorded dest does the *exit* handler spoof `st_dev`/`st_ino` to the source's. False positives on unrelated files are now structurally impossible. (`fstat`-by-fd can't be path-correlated at entry, so it no longer spoofs inode — git's hardlink check uses the `lstat`/`newfstatat` path anyway.) B-3 hardlink spoofing is preserved. Compile-verified; e2e needs an APK with this fix installed via the app UI. See `docs/Z2ROOT-BUILD-PARITY-HANDOFF.md` §10.

Previously (0.8.63-alpha, versionCode 71): attempted to fix the 0.8.62 startup regression by tightening the `linkcopy` match key from inode-only to `(dev, ino)` — but `st_dev` is constant across the rootfs bind, so this was ineffective (see 0.8.64 above).
