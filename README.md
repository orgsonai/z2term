...
**0.8.27-alpha (versionCode 35) — z2root: fixed `pwd` / relative `ls .` leaking the host cwd path inside the real app. Root cause: an `untrusted_app`'s mount namespace resolves `/data/user/0/<pkg>` to `/data/data/<pkg>`, so after `chdir` the kernel's `getcwd` returns the `/data/data` canonical form while the bind host (from `context.filesDir`) was the `/data/user/0` form — the reverse-translation lookup missed and exposed the host path. Fix: `realpath()` the rootfs and every bind host at startup (inside the app's namespace) so they match what `/proc/<pid>/cwd` / `getcwd` report. Verified on-device (`untrusted_app`): `pwd` = `/root`, `ls` resolves the cwd with no error. PRoot stays the default; z2root remains experimental behind the hidden engine selector (tap the version 7x). FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
