...
**0.8.26-alpha (versionCode 34) — z2root now boots an interactive Alpine shell inside the real app for the first time (non-root, `untrusted_app`). Four fixes landed the `nativeLibraryDir`-resident in-house ELF loader: (1) build `libz2root.so` as `-static` (not `-static-pie` — bionic's static-PIE C runtime self-relocation segfaults before `main` on NDK r28c; plain `-static` starts cleanly and still avoids the bionic linker's `/proc/self/exe` crash under ptrace); (2) eat the artificial initial `SIGSTOP` of `TRACEFORK`-born children so forked grandchildren aren't left stopped (this was why interactive commands hung); (3) suppress `SIGSYS` from Android's seccomp filter for syscalls it forbids `untrusted_app` (e.g. `setfsuid`/`setfsgid` that a fakeroot `-0` guest calls) by faking success instead of dying; (4) honor job-control group-stops (don't re-inject stop signals). The loader maps the rootfs `ld-musl` into anonymous executable memory and jumps to it, so it never `execve`s files in the app's own data dir (which `untrusted_app` forbids under W^X / SELinux). Verified on-device: `localhost#` prompt, `id` = `uid=0(root)`, external commands run. Known remaining: `pwd` / relative `ls .` still show the host cwd path (getcwd reverse-translation / relative-path handling under the `:/root` bind — separate path-translation work). PRoot stays the default; z2root remains experimental behind the hidden engine selector (tap the version 7x). FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
