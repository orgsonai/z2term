...
**0.8.22-alpha (versionCode 30) — The in-house ptrace engine "z2root" gained fakeroot (`-0`) emulation: `getuid`/`geteuid`/`getgid`/`getegid` report 0, `getgroups` reports no supplementary groups (so the host app's gids no longer leak into the guest — `id` now shows just `groups=0(root)`), failing `set*id`/`chown` calls are masked as success, and `newfstatat`/`fstat`/`statx` report `st_uid`/`st_gid` as 0 so files appear root-owned. Verified in the dev environment (host gid leak removed, no regressions). The engine is still experimental and selectable from the hidden engine selector (tap the version 7x); it falls back to PRoot when not bundled. FOSS flavor still excludes the Alpine rootfs from the APK and downloads the upstream minirootfs at runtime.**
