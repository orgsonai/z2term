...
**0.8.449-alpha (versionCode 457).** 最新の APK と全リリース履歴は **[GitHub Releases](https://github.com/orgsonai/z2term/releases)** にあります。
...
- **持ち出し / 引き継ぎ** — 設定・SSH 接続先・スニペット・自動化ルール・マクロ・自作テーマ・タイルの割り当て・アイコン・ユーザー辞書・キーボードの学習を 1 ファイルにまとめて別の端末へ。OS 本体は含めません。**SSH の秘密は既定で含めず、含めるときは合言葉が必須**です。その場合は、秘密が入り得るスニペット・マクロ・学習履歴を含め、**目録以外のバックアップ全体を暗号化**します。**決まった日時に自動で積むこともできます**（間隔・時刻・保存先・残す世代を指定。自動で作ったぶんだけを古いものから片付け、手で作ったバックアップは消しません）。
