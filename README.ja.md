...
**0.8.223-alpha (versionCode 231).** 最新の APK と全リリース履歴は **[GitHub Releases](https://github.com/orgsonai/z2term/releases)** にあります。
...
- **ホーム画面ウィジェット 2 種** — *状態＋ランチャー*: 状態（ssh の接続先 / 常駐サーバー数 / 有効な自動化ルール数 / 電池）を出し、選んだマクロをタップで**アプリを開かずバックグラウンド実行**（実行中のマクロはもう一度タップで停止）。*ライブ tail*: `~` の下のログの末尾をホーム画面に出し続けます。
