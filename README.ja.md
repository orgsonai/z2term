...
| Linux GUI | **アプリ内に GUI タブ**（Xvnc + 内蔵 RFB クライアント）。音声・動画つき。同じビューアで**リモートの VNC** も開ける | 別途 X11 / VNC ビューアアプリ |
...
**0.8.425-alpha (versionCode 433).** 最新の APK と全リリース履歴は **[GitHub Releases](https://github.com/orgsonai/z2term/releases)** にあります。
...
- **日本語・英語・簡体字中国語** — アプリの画面だけでなく **`z2-*` コマンドの表示も言語設定に追従**します（ヘルプ・usage・メッセージまで）。訳の無いところは英語で出ます。⚠ キーボードは別の話で、**中国語の入力方式は内蔵していません** — 中国語を打つときは OS の入力方法に切り替えてください。
...
- **持ち出し / 引き継ぎ** — 設定・SSH 接続先・スニペット・自動化ルール・マクロ・自作テーマ・タイルの割り当て・アイコン・ユーザー辞書・キーボードの学習を 1 ファイルにまとめて別の端末へ。OS 本体は含めません。**SSH の秘密は既定で含めず、含めるときは合言葉が必須**です。**決まった日時に自動で積むこともできます**（間隔・時刻・保存先・残す世代を指定。自動で作ったぶんだけを古いものから片付け、手で作ったバックアップは消しません）。
