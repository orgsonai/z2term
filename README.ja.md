...
**0.8.584-alpha (versionCode 592)、リリースビルド・lint検証済み。単体テスト1,091件成功（2件スキップ）。** コマンド一覧の上部タブは以前の均等幅・角丸ボタンにし、「操作自動化／自動化ルール」の小タブは下線付きの新しいデザインにしました。エッジメモの空欄見出し・罫線切替・下部アイコン・保存をまたぐ履歴にも対応しています。常駐サーバーと自動化をコマンド一覧へ集約しました。自動化は「操作自動化／自動化ルール」、設定の「権限・通知」は「権限／通知・ログ」に分かれます。必要な権限の状態と設定入口をまとめ、Android設定から戻ると更新します。通知ログの長文・重複対策と座標だけの取得も含みます。[設計仕様](docs/ja/DESIGN-SPEC.md)。公開済みAPKは[GitHub Releases](https://github.com/orgsonai/z2term/releases)にあります。
