...
**0.8.460-alpha (versionCode 468).** 最新の APK と全リリース履歴は **[GitHub Releases](https://github.com/orgsonai/z2term/releases)** にあります。
...
- **Linux GUI** — Xvnc + openbox と内蔵 RFB クライアント。`z2gui` でデスクトップを起動し、`z2run <アプリ>` で GUI アプリを起動（GUI タブも自動で開く）。音声・動画つき。**同じビューアでリモートの VNC / RDP サーバにも繋がります** — 保存した接続先に **[VNC]** / **[RDP]** ボタンが付き、そのサーバのデスクトップをタブで開けます（VNC は RFB 3.3〜3.8 / None・VNC パスワード認証、RDP はネットワークレベル認証 (NLA) と証明書の初回確認つき。RDP はまだ画面を見るだけで入力は送れません）。
