...
**0.8.577-alpha (versionCode 585)、作業版・ビルド未検証。** エッジパネルの編集画面の見た目を作り直し、配色を端末テーマから導くアプリ共通のパレット（アクセントはZTSグリーン）へ揃えました。「項目／見た目／管理」を下線付きの見出しに、板・タブを枠付きの札にし、入力欄・一覧・スライダー・スイッチの配色を1か所へ集約、ボタンは主要・通常・控えめ・削除の4種類に整理しています。通常メニューの＋はアプリ選択画面へ直接進むようになり（従来は設定を開いており⚙と区別できませんでした）、0.8.574で入った座標選択オーバーレイのビルドエラーも直しました。[設計仕様](docs/ja/DESIGN-SPEC.md)。公開済みAPKとリリース履歴は[GitHub Releases](https://github.com/orgsonai/z2term/releases)にあります。
