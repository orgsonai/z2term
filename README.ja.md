...
**0.8.597-alpha（versionCode 605、ビルド未検証）**: 端末上のHTTP(S)サーバーから選択したファイルまたはフォルダを直接配信します。QR・リンクを開いた相手がブラウザーでフォルダ階層をたどり、ファイルごとに確認して保存できます。既定の「自動」では、ファイルまたはフォルダを選ぶと現在のWi-Fi／携帯回線の公開アドレスと空きポートを取得し、サーバー起動からURL・QR表示まで行います。「手動」で公開URL・待受ポート・HTTPSを設定する方法も残します。共有停止・期限切れ・回線変更で共有用サーバーも停止します。回線の着信許可は別途必要で、外部到達性は未確認です。クラウドへのアップロードや中継サービスは使いません。QRツールはコマンド一覧の最上部左側から開きます。カメラ・画像の読み取り、URLの確認、SSH接続先と短いコマンドの取り込み・QR表示に対応します。他アプリの共有先「z2term — QR」から、QR画像や読み取り済みテキストも受け取れます。自動化タブは左に「自動化ルール」、右に「操作自動化」を配置し、初期選択は自動化ルールです。Languageの英語固定と起動時の競合修正も維持します。

操作と公開条件は[QRツールと端末からのファイル共有](docs/ja/QR-TOOLS.md)を参照してください。
