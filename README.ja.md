...
**0.8.219-alpha (versionCode 227).** 最新の APK と全リリース履歴は **[GitHub Releases](https://github.com/orgsonai/z2term/releases)** にあります。
...
- **ホーム画面ウィジェット 3 種** — *状態＋ランチャー*: 状態（ssh の接続先 / 常駐サーバー数 / 有効な自動化ルール数 / 電池）を出し、選んだマクロをタップで**アプリを開かずバックグラウンド実行**（実行中のマクロはもう一度タップで停止）。*ライブ tail*: `~` の下のログの末尾をホーム画面に出し続けます。*SSH QR*: この端末の `ssh://` を QR で出すので、IP を打ち直さず PC のカメラで読めます。
