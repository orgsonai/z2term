...
**0.8.576-alpha (versionCode 584)、作業版・ビルド未検証。** エッジパネルの編集画面の見た目を作り直しました。できること・保存先・CLIとの対応は変えていません。「項目／見た目／管理」を下線付きの見出しに、板・タブを枠付きの札にし、折りたたみは見出し行と `▾`／`▸` で示します。入力欄・一覧・スライダー・スイッチの配色を1か所へ集約し、ボタンは主要・通常・控えめ・削除の4種類に整理して、押せるかどうかが見て分かるようにしました。「項目」は1行に絵・名前・種類・編集・上下移動をまとめます。板そのもの（普段のメニュー）の見た目は変わりません。[設計仕様](docs/ja/DESIGN-SPEC.md)。0.8.574で入った座標選択オーバーレイのビルドエラー（`bottom` がViewのものに解決されていた）も直しました。公開済みAPKとリリース履歴は[GitHub Releases](https://github.com/orgsonai/z2term/releases)にあります。
