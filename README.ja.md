...
**0.8.592-alpha (versionCode 600)**: `z2-audio run コマンド 引数...` で、GUIを開かずにローカルLinuxの音をAndroidへ出力できます。初回は `z2-audio install`。エッジパネルの実行項目には「ON/OFFを表示」を追加し、ONをボタンの枠と背景色で表示します。メニュー内にON/OFFの文字やチェックは追加しません。トーチの外部操作・消灯防止の期限切れに連動し、マクロの実行終了時にOFFへ戻す設定も追加しました。組み込みのベルとハートは、丸みから裾・先端まで自然につながる輪郭へ修正しました。実機の音声出力・表示は未確認です。
