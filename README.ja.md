...
**0.8.581-alpha (versionCode 589)、作業版・ビルド未検証。** 常駐サーバーと自動化をコマンド一覧へ集約しました。自動化は「操作自動化／自動化ルール」、設定の「権限・通知」は「権限／通知・ログ」に分かれます。必要な権限の状態と設定入口をまとめ、Android設定から戻ると更新します。通知ログの長文・重複対策と座標だけの取得も含みます。[設計仕様](docs/ja/DESIGN-SPEC.md)。公開済みAPKは[GitHub Releases](https://github.com/orgsonai/z2term/releases)にあります。
