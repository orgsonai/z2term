...
**0.8.578-alpha (versionCode 586)、作業版・ビルド未検証。** エッジパネルの編集画面とAndroid操作マクロのGUIを同じ部品で作り直し、配色を端末テーマから導くアプリ共通のパレット（アクセントはZTSグリーン）へ揃えました。ページは下線付きの見出し、板・タブは枠付きの札、入力欄・一覧・スライダー・スイッチは1か所で定義した配色、ボタンは主要・通常・控えめ・削除の4種類です。操作マクロは1マクロ1行の一覧、番号を揃えた等幅の手順表示、左の罫線で表す入れ子になりました。通常メニューの＋はアプリ選択画面へ直接進みます。[設計仕様](docs/ja/DESIGN-SPEC.md)。公開済みAPKとリリース履歴は[GitHub Releases](https://github.com/orgsonai/z2term/releases)にあります。
