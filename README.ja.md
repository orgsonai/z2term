...
**0.8.213-alpha (versionCode 221).** 最新の APK と全リリース履歴は **[GitHub Releases](https://github.com/orgsonai/z2term/releases)** にあります。
...
- **ホーム画面ウィジェット** — 状態（ssh の接続先 / 常駐サーバー数 / 有効な自動化ルール数 / 電池）を出し、選んだマクロをタップで**アプリを開かずバックグラウンド実行**。並べるマクロはウィジェットごとに選べます。
