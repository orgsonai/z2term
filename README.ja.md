...
**0.8.596-alpha (versionCode 604)**: コマンド一覧 → サーバー →「QRでファイルを渡す」を追加しました。ファイル選択・共有開始・QR表示・停止を専用画面で操作できます。`z2-share --qr ファイル名` からも開始でき、Linux・同じWi-Fi・VPSは不要です。双方のインターネット接続を使ってCloudflare経由で配信し、リンクは30分で終了します。Cloudflare Quick Tunnelsを使う試験機能で、稼働保証はありません。選択したファイルのコピーを端末内に作るため、その分の空き容量が必要です。
...
**音声録画の調査（0.8.596）**: 通常の動画では録音できる一方、`z2-audio`の音声がAndroid画面録画へ入らないとの報告を受け、再生開始時のAudioTrack用途・種別・録音許可・形式を診断ログへ追加しました。実機の再生属性はUSAGE_MEDIA / CONTENT_TYPE_MOVIEで、アプリの録音許可も有効でした。画面録画中の欠落原因は未特定で、録音修正は含みません。
...
- **OS本体は非同梱・APK は 1 つだけ** — 第三者 prebuilt を一切同梱せず（約 36MB）、初回起動時にディストロを DL して SHA-256 で検証。
...
3 つの生成スクリプトを順に実行し、最後に欠落がないか点検します。

1. `build-z2root.sh` → `libz2root.so` / `libz2accept.so`（NDK が必要）
2. `fetch-fonts.sh` → `IBMPlexMono` / `JetBrainsMono` / `FiraCode` の `-Regular.ttf`
3. `build-qr-tunnel.sh` → `libz2tunnel.so` と `assets/licenses/QR-Tunnel.txt`（GoとNDKが必要）

QR共有の接続部品はCloudflare Tunnel 2026.9.1の固定ソースとSHA-256を使い、Android向けにビルドします。初回はソースとGo依存の取得にネット接続が必要です。GradleもAPK作成時に自動生成するため、個別の実行は不要です。
