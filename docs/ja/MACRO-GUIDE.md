...
z2-icon scale 1 64             # 枠 1 の絵を 64x64 に敷き直す（見た目はそのまま）
```
...
z2-icon scale 1 64             # いま枠 1 に入っている絵を 64x64 に敷き直す
```

`scale` は**見た目を変えません**（マス目に対する絵の割合をそのまま保ちます）。増えるのは
描き足せる細かさだけなので、これを通してから `z2-icon edit` で角を丸めていきます。
