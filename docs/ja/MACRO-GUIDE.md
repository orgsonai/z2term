...
**24x24 のドット絵**に差し替えられます（0.8.294）。タイルは**枠ごとに別の絵**にできます。
...
- ⚠ **表示は 24px 前後**です。マス目より細かい描き込みは消えます。`show` で見えるものが
  そのまま出ると思ってください。
...
（0.8.273 で、同梱サンプルのうち電池・日報・OTP の 3 系統は常駐版から `z2-when` 版へ切り替えました）。
