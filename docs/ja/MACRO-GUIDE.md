...
1. **トリガーを有効化**: アプリの ⚙設定 → 「**権限・通知**」 → 「**通知・ログ**」 →
...
2. **きっかけを登録する**: `z2-when <トリガー> run <コマンド>`（→ 4 章）。登録したものは
   ...
   **ルールには名前を付けられます**（0.8.303。フォームのいちばん上。端末からは
   ...
   書いてそのパスを登録**するのが確実です）。
3. **常駐させたいとき**（1 章 B の書き方をするとき）: コマンド一覧 → 「**サーバー**」に起動コマンドを登録すると、
   アプリを開かなくても・端末再起動後も動きます（「起動時に自動で常駐」も ON に）。
4. 便利ツール: `jq`（JSON 解析）を入れておくと楽。例: Alpine `apk add jq` / Debian系 `apt install jq`。
5. **白紙から書きたくないとき**: `z2-macro list` で同梱サンプル 11 本を見て、`z2-macro install <名前>` で
...
- **「システム設定の変更」の許可が要ります。** 設定 › 権限・通知 › 権限 › **システム設定の変更** から許可して
...
z2-icon sample                 # 組み込みの絵の一覧（16 種・どれも 64 マス）
...
出ます — ふだん一辺を気にする必要はありません。一辺を上げると出すときの均しも細かくなります
（24 と 48 は 96 相当、64 は 128 相当）。`z2-icon show` が出すのは**描いたままの絵**で、
タイルにはそれより滑らかに出ます。
...
- 通知アクセスの許可が要ります（⚙設定 → 権限・通知 → 権限 → 通知の読み取り）。
...
- ⚠ **省電力モードが ON のときは、起動してもロックを握りません**（電池優先の設定なので正しい動きです）。
  そのときは `z2-server start` がその旨を出します。設定は コマンド一覧 → サーバー。
...
| 型 | どう動かすか | 中身 |
|---|---|---|
| **常駐**（ずっと動き続ける） | `コマンド一覧 → サーバー` に起動コマンドを登録（例: `sh ~/.z2term/macros/watch.sh`）。「起動時に自動で常駐」も ON にすれば、アプリを開かず・再起動後も動く | 5-0 の雛形を使うもの（監視ループがある。同梱サンプルには無い） |
...
- 準備: 設定 → 権限・通知の「権限」で **SMSの受信を許可**し、「通知・ログ」で **SMS検知をON**
...
`z2-macro install rss` で入るマクロは「定期実行 → 新着だけ拾う → 通知 → 記事または一覧を開く」を組み合わせた例です。0.8.635 の案内では定期巡回と通知操作を自動化へ登録し、タイルやウィジェットの設定は行いません。

| やること | 使っている部品 |
|---|---|
| 定期実行 | `z2-when time:every=30m name='RSS 定期巡回' run ~/.z2term/macros/rss.sh` |
| 新着だけ拾う | `seen.txt` を引き算（`grep -Fxv`）。フィードの日付や並び順は当てにしない |
| 知らせる | `z2-notify -b 開く -b 一覧` |
| 見逃さない | `~/.z2term/rss/important.txt` に語を1行1本。当たった記事は別の通知（1回5件まで） |
...
登録済みなら、追加せず **コマンド一覧 → 自動化 → 自動化ルール** で既存の実行コマンドを編集します。古い「常に記事を開く」ルールを残すと「一覧」でも記事が開きます。通知から画面を開くには「他のアプリの上に表示」の許可が必要です。

- **使い切り**です（5-3）。常駐サーバーには登録しません。
- 解析は python3 の標準ライブラリだけ（pip 不要）。1本落ちても他のフィードは続き、壊れた XML は飛ばします。
- 電池を消費するので、巡回間隔は **30分より短くしない**でください。
- 設定手順は `docs/ja/HANDBOOK.md` の「フィードを購読する（RSS / Atom）」を参照。
- `rss-open` は次の1本を開く別のサンプルとして引き続き使えますが、案内の設定には不要です。

**「引き算で新着を出す」は定石です。** 相手の日付や並び順が当てにならなくても、「前に見た分を覚えておいて引く」ことで新着が分かります。
...
**曜日・朝／夜の指定（0.8.609）**: 通知の「いつ？」にも、以下の書き方をそのまま入力できます。

| 入力 | 予約 |
|---|---|
| `土曜日の夜九時` / `土曜日 夜9時` / `土曜21:00` | 次の土曜日21:00に1回 |
| `月曜日の朝七時` | 次の月曜日07:00に1回 |
| `朝七時` / `夜九時半` | 次の07:00 / 21:30に1回 |
| `明日の夜九時十五分` | 明日21:15に1回 |
| `毎週 土曜日 夜九時` | 毎週土曜日21:00 |

7曜日に対応し、`日曜`・`日曜日`・`日`、英語の `sun`〜`sat` を受け付けます。曜日だけなら今と同じ時分です。同じ曜日で指定時刻がまだ未来なら今日、時刻が同じか過ぎていれば7日後です。毎週の繰り返しにする場合だけ `毎週` を付けます。日付をまたぐ計算は曜日指定では現地の暦日を使い、夏時間切替も考慮します。

時刻は数字・漢数字・全角数字、`○時`・`○時半`・`○時○分` と `HH:MM` に対応します。`朝` は0〜11時、`夜` の1〜11時は13〜23時として扱います。`夜21時` のように24時間表記も使えます。`午前十二時` は00:00、`午後十二時` は12:00です。`夜十二時` は日付の解釈が曖昧なため受け付けません。深夜は `日曜日の00:00` や `明日の00:00` のように日付を明示してください。任意の自然文全体を読む機能ではありません。

**既存マクロへの反映**: APKを更新してローカルの端末タブを開き直し、`z2-macro diff remind` で現在のコピーと新版を確認してください。自分で編集した箇所がなければ `z2-macro install -f remind` で更新できます。編集がある場合は先に退避し、差分を取り込んでください。マクロ本体の更新で保存済み予定を削除したり、setupをやり直したりする必要はありません。
...
**日時と本文は分けて渡します**（`明日 09:00 打ち合わせ`、`土曜日の夜九時 打ち合わせ` のように）。上記の日時表現は読めますが、任意の自然文全体は解釈しません。共有シートから流れてきた文のような自然文を扱うなら、⚠ **時刻を書いていない文は
...
着信通知に電話番号が表示されたら、その番号を通知に出し、**「コピー」ボタン 1 つで
...
**電話帳の登録有無は判定しません。** 題名、本文の順に番号そのものを探すので、
登録済みの相手でも「題名＝名前、本文＝番号」なら対象です。どちらにも番号がなく、名前しか
表示されない場合はコピー通知を出しません。名前と番号が同じ欄に混在する文章からの抽出はしません。
使う権限は既存の通知アクセスだけです。

0.8.607から案内と通知の名称を「電話番号のコピー通知」にしました。
既存ルールとの互換性のためファイル名 `unknown-call.sh` は維持します。
**導入済みのマクロはアプリ更新だけでは置き換わりません。** 更新後に
`z2-macro diff unknown-call` を確認し、独自の編集がなければ
`z2-macro install -f unknown-call` で入れ直してください。編集済みなら必要な変更だけ取り込んでください。
...
z2-notify -h -c "$num" "${what}: 電話番号のコピー通知" "$num"
```
...
- **電池の減りが速くなった** → **常駐サーバーが動いていませんか**（📜 → サーバータブ）。常駐が 1 本でも
  ...
  広げてください（15 秒以上）③ `コマンド一覧 → サーバー → 省電力モード` を ON にすると WakeLock/WifiLock を握らなくなります
  （そのぶん画面消灯中の反応は遅れます）。
...
`z2-edge toggle` で表示を切り替え、取っ手の `--open swipe|tap|both` と `--run` で実行入口を作れます。
`z2-app pick` は選んだアプリのパッケージ名を返し、取消・120秒経過では失敗します。
...
パネルでは「項目追加」で入力欄・ボタン・文字表示を作り、ボタンのコマンドに翻訳マクロを指定します。引数は文章・翻訳先・元言語の順に関連付けます。言語は選択欄や固定値にもできます。[部品と動作の設定](EDGE-MACRO-FORMS.md)。
