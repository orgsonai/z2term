...
稀ですが、原理的な穴です）。なお、ロック画面の「プライベートな通知内容は表示されません」設定は
**表示のされ方だけ**を制御するもので、通知検知（通知アクセス）はそれに関係なく本文の実体を受け取れます。
拾えないのは、本文がタイトル・本文・メッセージのどのフィールドにも入らない**完全カスタム表示だけ**の
通知（ごく一部のアプリ）に限られます。SMS のワンタイムコードは MessagingStyle の本文に入るのが普通で、
0.8.185 以降はこれも取得します。
