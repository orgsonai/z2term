...
最終更新: 2026-09-11 / 対象バージョン: 0.8.587-alpha (versionCode 595)

**バーとメモの配色（0.8.587）**: バーは近くの背景の明暗に合わせて白・黒を自動選択し、反対色の細い縁取りを付けます。「見た目 → 取っ手 → バーの色」で自動／白／黒を選べます。自動判定はAndroid 11以降と「z2term Android 操作」のユーザー補助が必要です。メモごとに背景色・文字色を色見本または#RRGGBBで指定でき、未指定なら従来の配色を使います。背景だけを指定した場合の自動文字色、罫線・カーソルも合わせます。ビルド未検証・実機未確認。
