...
最終更新: 2026-08-23 / 対象バージョン: 0.8.388-alpha (versionCode 396)
...
#### 通信量の上限（`service/NetGuard`、0.8.388）

**何ができるか**: 今期のモバイル通信量が決めた量に達したら、**z2term 自身の通信を止める**。使いすぎに気付くのはたいてい**回線が絞られてから**で、z2term は SSH やダウンロードで黙って通信し続けられる。
...
**測り方**: `NetworkStatsManager` に**自分の UID ぶんだけ**を聞く。⚠ 自分の UID を聞くのに「使用状況へのアクセス」は要らない（他人のぶんを聞くときだけ要る）ので、**追加の権限を 1 つも増やさずに済む**。⚠ それでも断られる端末があるので、**測れないときは止めない**（`measurable = false`）— 測れないことを理由に通信を止めると、直しようのない締め出しになる。測れているかどうかは設定画面に出す。⚠ OFF のときは測りに行かない（問い合わせは軽くない）。
...
- ⚠ **自動で書き出すものに秘密は含めない**。含めるには合言葉が要り、自動化すると**合言葉を端末に置く**ことになる。「合言葉なしで秘密を出す経路は作らない」という上の約束を、自動化のために崩さない。秘密ごと運ぶときは手で 1 本作る。
