...
最終更新: 2026-08-28 / 対象バージョン: 0.8.422-alpha (versionCode 430)
...
- **訳し忘れの守りは 2 つ。** ①`app/build.gradle.kts` の lint で `MissingTranslation` / `ExtraTranslation` を明示的に error にし、CI (`lintDebug`) で落とす ②`bash scripts/i18n-status.sh` が両系統の埋まり具合を数え、`--missing <言語>` で未訳キーを英語の原文つきで出す。⚠ **多言語化は一度の作業ではなく毎回の税** — 文言は版ごとに増えるので、埋める道具と落とす守りをセットで持つ。
