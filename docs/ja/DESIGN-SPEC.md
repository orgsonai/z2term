...
最終更新: 2026-09-23 / 対象バージョン: 0.8.645-alpha (versionCode 653)
...
- **タブを段の積み重ねで組む**: 項目の `row=N` が段。同じ段は横に並び（幅は等分、項目の `width` があればその幅）、段は番号順に上から積む。親の `flow` は見ないので、アプリのタブを変えてもターミナルのタブは巻き込まれない。
...
- **並べ替えは矢印2つで全部できる**（`EdgeRows.move`）: 段の中で隣と入れ替え、段の端ではその項目だけの段に分かれ、1個だけの段からさらに押すと隣の段に入る。長押しドラッグは落とした先の項目の段に入る（設定・通常メニューとも）。
