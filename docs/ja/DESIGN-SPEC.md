...
最終更新: 2026-09-23 / 対象バージョン: 0.8.647-alpha (versionCode 655)
...
- **段の高さと横幅はドラッグで決める**: 見本の段の下の線を上下に動かすと、その段の項目すべてに `height=N%`（パネル高さに対する割合）を書く。項目の間の線を左右に動かすと、その段の項目すべてに合計100になる `width=N%` を書く。指を離したときだけ保存し、動かしている間は見本の下に値を出す。段の見出しの「大きさを戻す」で `width` / `height` を空に戻す。
