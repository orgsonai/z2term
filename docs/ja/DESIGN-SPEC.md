...
最終更新: 2026-09-09 / 対象バージョン: 0.8.565-alpha (versionCode 573)
...
**板の余白を長押しすると設定を開きます。取っ手は300ms長押し後、動かさず離すと設定、動かすと従来どおり移動です。タップで開くボタンはドラッグですぐ移動します（0.8.565）。** 設定の「完了」または戻るで板へ戻ります。
設定は板の幅から独立した画面です。設定を開いても項目のコマンドや定期更新は実行しません。
「見た目を調整」の寸法・配置は画面図、取っ手は実物で仮表示し、保存時だけ定義へ反映します。取消・閉じる・タブ切替・消灯・再読込で未保存の調整を破棄します。
...
| タブ列 | `tabbar=off\|on\|auto`（既定off、autoは子タブがあるとき表示） |
...
＋と⚙は有効にした場合だけアイコンで末尾に出ます。タブ追加・メモ追加・項目編集・並べ替えは設定内にあります。
...
「＋項目」と各項目の「項目を編集」から全6種類の定義を編集できます。省略可能な欄は空欄で設定を解除します。取消は未保存の入力を破棄し、外部で変更された項目への保存は拒否します。
...
| オプション | 変更する起動条件 |
|---|---|
| `--reuse-task` | 複数タスク作成のフラグを外し、既存タスクの再利用を許可する |
| `--os-bounds` | 窓の位置・大きさを指定せず、OSに任せる |
| `--bounds-only` | 非公開の窓モードキーを送らず、公開APIのサイズ指定だけで要求する |

無指定時は従来どおり複数タスク・画面内側80%のサイズ・窓モードキーを要求します。`--reuse-task --os-bounds` は併用できます。`--bounds-only` と `--os-bounds` の併用、freeform以外での指定はエラーです。
これらは端末の標準経路との違いを比較するための切替で、画面遷移時の描画崩れや窓の装飾が直るかは実機で確認が必要です。OS設定を書き換えるものではありません。

既定のフリーフォームは新しいタスクに窓サイズを要求します。分割は端末画面を前面に出し、その隣に新しいタスクを起動します。対象アプリが単一タスクを強制する場合はOSの制限を受けます。
外部アプリ・rootは使いません。フリーフォームは `ActivityOptions.setLaunchBounds` とAOSPの非公開Bundleキー `android.activity.windowingMode=5` で要求します。このキーの対応はOS依存です。分割は `FLAG_ACTIVITY_LAUNCH_ADJACENT` による要求です。
...
設定内で項目名を長押ししてドラッグし、移動先の前後へ落とすと全項目の `order` を保存します。欄外で離した場合は変更しません。設定では項目削除とアプリの起動方式変更もできます。
