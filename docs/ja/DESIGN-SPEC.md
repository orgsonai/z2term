...
最終更新: 2026-08-13 / 対象バージョン: 0.8.332-alpha (versionCode 340)
...
- **一覧の時点で分かるようにする**。`z2-macro list` に状態列（`未導入` / `導入済` / `要更新`）を足す。
...
- **`install` は「同じ」と「違う」を言い分ける**。一律「既にあります (上書きするには -f)」だと、
  上書きしてよい理由があるのかどうかが分からない。違うときは `z2-macro diff <名前>` と
  `z2-macro install -f <名前>` を続けて出す。
