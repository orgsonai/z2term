...
最終更新: 2026-09-07 / 対象バージョン: 0.8.531-alpha (versionCode 539)
...
- ⚠ **かなモードの印はツールバーの ⌨ を「あ」に変えて示す（0.8.530・利用者の指摘）。**
  ⛔ **入力メソッドの窓に印を置いてはいけない** — その窓は端末の上に浮くので、印の下にある行が
  読めなくなる（0.8.529 で候補バーの席に置いて指摘を受けた）。⭐ 状態は `ime/KanaModeState`
  1 つで渡す（入力メソッドと端末画面は同じプロセス）。⚠ 外付けが外れたらかなモードも畳む
  （印だけが嘘になるため）。
- ⚠ **使い方（Tips）は日本語のときだけ出す**（利用者の指示「日本人仕様なので」）。外付けの
  かな入力は日本語を打つ人にしか関係がなく、他の言語では「切り替えると何が起きるのか」から
  説明が要る。res も訳を持たない（`translatable="false"`）。
- ⛔ **変換中は入力メソッドが自分から出る（0.8.531・利用者の指摘「予測変換が表示されない」）。**
  物理キーボードがあるとき、相手のアプリは「キーボードを出して」と要求しない（端末画面も
  0.8.527 でそう変えた）。**要求が無い限り `onEvaluateInputViewShown` が true を返しても窓は
  開かず、候補バーが 1 度も見えない**。⇒ 変換中だけ `requestShowSelf(0)` で自分を出す。
- ⚠ **外付けかどうかは打鍵のデバイスで判定する（0.8.531）。** ⛔ `onKeyDown` が来たら無条件に
  「外付けあり」としていたが、端末側のハードキー（指紋センサー等も `KEYBOARD` として数え
  られる）が 1 度来ただけで成立してしまい、**以後キーの絵が描かれなくなる**。判定は端末画面と
  同じ `isPhysicalKeyboard`（仮想を除く・`SOURCE_KEYBOARD`・`KEYBOARD_TYPE_ALPHABETIC`）。
