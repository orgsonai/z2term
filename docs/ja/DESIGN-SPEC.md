...
最終更新: 2026-09-10 / 対象バージョン: 0.8.581-alpha (versionCode 589)

**管理画面の集約（0.8.581、ビルド未検証）**: 常駐サーバーと自動化はコマンド一覧ツールから設定します。「サーバー」に定義・起動停止・起動時の自動常駐・省電力・通信量上限を集約しました。「自動化」は「操作自動化」と「自動化ルール」を切り替えます。操作自動化は既存の管理画面で作成・編集・実行・停止・履歴を扱い、ルールはその場で編集できます。
...
設定グループはPERMISSIONSへ改名しますが、開閉状態の保存IDはautomationのまま維持します。サーバー・ルール・検知・ログの保存形式とCLIは変更しません。共通権限画面はAndroid 10のストレージ実行時許可とAndroid 11以降の全ファイルアクセス、Android 13以降の通知表示許可を分岐します。USB許可は既存のUsbHostAccessを利用します。XML・文字列参照・差分を確認済み。ビルド未検証・実機未確認。
...
**実装メモ（0.8.580）**: 初回の設定読み込み前に来た通知は読み込み完了まで待ちます。通知内容と保存設定をスナップショットにし、差分計算・時刻整形・保存を単一executorで直列化します。通知トリガーは別executorで保存後に処理し、遅い自動化でログ保存を塞ぎません。切断時は検知ONなら再接続を要求します。過去の通知を再生する処理は追加しません。回帰テスト14件を更新・追加。ビルド未検証・テスト未実行・実機未確認。
...
**自動化タブ（0.8.581）**: コマンド一覧 → 自動化で「操作自動化／自動化ルール」を切り替えます。操作自動化はActionMacrosActivityの管理画面へ進み、作成・編集・実行・停止・履歴を共用します。自動化ルールはWhenRulesBodyの一覧・編集・有効無効・実行ログ・試行・削除・一時停止です。設定内の重複入口は削除しました。上部タブを固定し、横スクロールで省略せず表示します。ツール画面とタブ選択はrememberSaveableで復帰時に保持します。
...
項目名もアイコンも出さない設定（`labels=off` かつ `icon=` 空）では見出し行を作らず、本文が板の上端から始まります（0.8.565）。
