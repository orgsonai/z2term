...
最終更新: 2026-09-10 / 対象バージョン: 0.8.584-alpha (versionCode 592)
...
**本文の抽出（NotificationText、0.8.580）**: titleはEXTRA_TITLE。会話はEXTRA_MESSAGESとEXTRA_HISTORIC_MESSAGESを読み、配列間の重なりを出現回数を保って統合します。時刻・本文に送信者識別も加え、Android 11以降は標準Messageデコーダー、Android 10はBundleの同じフィールドを使います。最後の会話本文が展開本文・通常本文の先頭と一致する場合は、省略記号も考慮して長い本文へ補完します。長い要約文というだけでは置換しません。

会話が無い場合はEXTRA_TEXT_LINESを通常本文より先に扱い、まとめ通知の途中の行も残します。単一行は同様に補完します。その後はEXTRA_BIG_TEXT（通常本文がその続きなら補完）→ EXTRA_TEXT → EXTRA_SUB_TEXT / EXTRA_INFO_TEXT → tickerTextの順です。受け取った本文をz2term側で文字数制限しません。既存のtextへ合流し、JSON・テンプレートの項目は変えません。z2-noti listも同じ抽出を使い、差分にはせず現在の全内容を返します。
...
**重複排除（NotificationText.History、0.8.580）**

- 会話は通知keyごとに時刻・送信者・本文のハッシュと出現回数を保持し、過去の発言を再保存しません。最後の1件の印だけで切る方式を置き換え、同時刻・同文でも配列内の出現数が増えた分を残します。
- 新着抽出後に本文だけの判定を重ねません。別時刻・別送信者の新着は記録し、同じ通知グループの別keyに載った時刻付きの同一メッセージは重複除外します。
- 補完時は短文・長文双方の印を保持し、短文が履歴に戻っても重複保存しません。先に短文しか届かず、後から初めて全文が届いた場合は全文を追加で1回保存します。
- 会話配列の無いメッセージ通知はイベント時刻（無ければpostTime）も比較します。通常通知の同一key再送と、同じアプリ・同じ内容が別keyで10秒以内に届く再送の除外は維持します。
- 通知削除では通常通知の記録だけを忘れ、会話履歴は保持します。通知ごと256件・各128個の印までのLRUです。プロセス終了・LRUからの脱落後まで完全な重複防止を保証するものではありません。
...
**実装メモ（0.8.580）**: 初回の設定読み込み前に来た通知は読み込み完了まで待ちます。通知内容と保存設定をスナップショットにし、差分計算・時刻整形・保存を単一executorで直列化します。通知トリガーは別executorで保存後に処理し、遅い自動化でログ保存を塞ぎません。切断時は検知ONなら再接続を要求します。過去の通知を再生する処理は追加しません。回帰テスト14件成功。リリースビルド検証済み・通知受信の実機確認は未完了。
