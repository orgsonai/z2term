...
最終更新: 2026-09-14 / 対象バージョン: 0.8.596-alpha (versionCode 604)

**0.8.596-alpha (versionCode 604): QRファイル共有（試験提供）**。コマンド一覧 → サーバー →「QRでファイルを渡す」に専用画面を追加。ファイル選択・開始・QRとリンク表示・状態確認・停止を扱う。`z2-share --qr [FILE]` / `--qr-status` / `--qr-stop` からも操作できる。Androidネイティブで実行し、Linux・同じWi-Fi・VPSは不要。送受信側のインターネットとCloudflare Quick Tunnelsを使い、準備完了から30分で終了する。試用中継サービスのため稼働保証はない。

- `share/QrShareManager` が1件だけの共有状態とサービス所有者を保持。`QrShareService` は独立した `dataSync` FGSで、通知から再表示・停止できる。期限は単調時計で測り、`START_NOT_STICKY` とし、再起動・更新後に自動公開しない。「全部停止して終了」も停止対象に含む。
- 選択ファイルを `cacheDir/qr-share/<UUID>/payload` へストリームでコピーしてから公開する。コピーは共有中に固定し、メモリへ全体を読み込まない。空き容量を確認し、停止・失敗時は接続とトンネルを閉じてコピーを消す。プロセス強制終了後の残存コピーは次の開始時に削除する。
- CLIは引用符・空白を保った絶対パスとHOME、OS IDをAPIへ渡す。共有ホームと共有ストレージ、選択OSのHOME隔離領域だけを許可し、既存の `ProotLauncher` のbind対象一覧を参照する。別のプライベート領域やディレクトリの公開は拒否する。GUIはSAF URIからコピーし、Linux内部の他の場所もファイル選択経由で扱える。
- `QrFileServer` はloopbackの空きポートに限定し、256bitの乱数トークンを含む固定パスだけを配信する。リクエストパスをファイルパスへ変換しない。ファイル名をHTMLエスケープし、ダウンロードの `Content-Disposition` はUTF-8対応。GET/HEADと単一Range、再取得に対応し、no-store・CSPを送る。受信成功回数による自動終了は行わず、プレビューやHEADでリンクが失効する問題を避ける。
- `QrTunnelDns` は新規公開名の不存在応答を長く保持しないよう、Androidの設定済みDNSへキャッシュを使わず照会する。端末のDNS設定は変更しない。外向きIPv6アドレスがない回線ではIPv6宛先を使わず、IPv4を優先する。
- 同梱の `libz2tunnel.so` がHTTP/2で外向きに接続する。公開HTTPS URLの専用healthパスが正しいトークンを返したことを確認してからQRを表示する。接続喪失時はURLを隠し、復帰を確認して再表示する。公開URLやトークン、提供元の生ログは永続化しない。診断は失敗種別・接続部品の終了コードなどに限定する。
- Cloudflareが中継する公開リンクであり、リンクを知る人は期限内に取得できる。パスワード認証や端末間の独自暗号化は追加していない。送信元の通信・アプリ稼働が必要で、共有は1ファイルのみ。ZXing Core 3.5.4でQRを生成する。

**確認結果（0.8.596）**: 全単体テスト1,144件中1,142件成功、既存RDPテスト2件はスキップ。debug・署名付きrelease・実機テストAPKのビルド成功、lintエラー0件。PCのHTTP統合テストに加え、Android実機のQR共有テスト2件と既存シェル・ディストロ削除・エッジパネルの回帰テスト4件が成功した。実機同梱の接続部品で公開HTTPSへ接続し、実機と別のPCで1MiBの合成ファイルのSHA-256一致、Range、元ファイル変更後の内容保持、停止後の取得不可を確認。実機画面のQRをデコードし、検証したURLとの一致も確認した。公開直後のDNS反映待ちで再試行を要した。通常版とデバッグ版604を実機へデータ保持でインストールし、版数を確認した。 iPhone実機、SAFの手操作、回線切替、大容量、30分の実時間失効は未検証。
...
QR共有は `scripts/build-qr-tunnel.sh` がCloudflare Tunnel 2026.9.1の固定ソースをSHA-256検証し、Go 1.26+とNDKでAndroid arm64向けPIEとしてビルドする。生成物は `libz2tunnel.so` と、実際のGo依存・ランタイムの権利表示を集めた `assets/licenses/QR-Tunnel.txt`。両方git管理外で、Gradleのassets/JNIマージとLintに生成依存を宣言する。`build-bundle.sh` からも生成する。初回はソースとGo依存の取得にネット接続が必要。ビルドはPCで実行し、論理CPUの半分を使う。
