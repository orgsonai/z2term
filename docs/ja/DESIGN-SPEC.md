...
最終更新: 2026-07-30 / 対象バージョン: 0.8.291-alpha (versionCode 299)
...
- **`z2-tile` コマンド (0.8.258・クイック設定タイル)**: マクロ / コマンドを**クイック設定パネル**へ 4 枠まで置ける。`z2-tile set <1-4> <マクロ.sh|コマンド...> [-l 表示名]` / `list` / `clear <1-4|all>`。
  ...
  - **枠は 4 つ固定**。タイルは manifest に `TileService` を 1 個ずつ書く必要があり、**実行中に増やせない**（Android の仕様）。D1 ウィジェットのボタンと同数にして「どこに置いても 4 つ」で覚え方を 1 つにする。実装は枠番号だけが違う 4 クラス (`Z2Tile1`〜`Z2Tile4`)。
  ...
  - **表示名は先頭の語から作る**ので、同じマクロを 2 枠に置くと**同じ名前**になる（`remind.sh ask` も `remind.sh peek` も `remind`）。使い分けは `-l`。⚠ 引数まで名前に畳み込まない — 枠は 4 つしかなく、長い名前は機種によって黙って切れる。
...
- ⚠ **入力ビューはナビゲーションバーを避ける** (0.8.279)。Android 15 (targetSdk 35) は
  ...
  **3 ボタンナビゲーションバーの裏**に潜り、押しても「戻る」等になって届かない。
  ...
  読み直す。
- ⚠ **候補バーの領域は日本語のとき常に確保して入力ビューの高さを一定に保つ** (0.8.291)。
  候補バー ([CandidateBar]) は非変換時に高さ 0、変換開始で `CandidateBarHeight` (76dp) に
  なる。IME ではこれが入力ビューの高さを変え、その瞬間に **IME ウィンドウがリサイズ**される。
  リサイズ後、`ComposeView` のタップ座標変換 (スクリーン→ローカル) がリサイズ差分ぶん古い基準の
  まま残り、**実際に触った位置より約 76dp 上のキーを押した判定**になっていた (変換候補が出た
  瞬間から発症)。端末画面では候補バーが下端アンカーの `Column` に乗りキーボードが動かないので
  出なかった。修正は、日本語のとき `Box(height = CandidateBarHeight)` で候補バーの枠を**常に確保**し、
  変換の有無で入力ビューの高さが変わらない ＝ リサイズが起きないようにする。英語では composing が
  無く候補バーも出ない ＝ ズレが起きないので確保しない (無駄な隙間を作らない)。定数 `CandidateBarHeight`
  は `CandidateBar` と共有し、実高さと確保量がズレないようにする。
