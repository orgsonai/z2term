...
最終更新: 2026-09-07 / 対象バージョン: 0.8.527-alpha (versionCode 535)
...
- `terminal/TerminalScreen.kt`: 全体レイアウト。TopBar / TabBar / 描画領域 / キーボードトグル / キーボード領域。`KeyboardMode = CUSTOM | SYSTEM`。**横画面**は `LocalView.OnLayoutChangeListener` で向きを検知し、`landscapeKeyboardPosition`/`Width`/`Height` 設定に従って Row レイアウト (`SideKeyboardColumn`) に切替。`landscapeScaledStyle()` で keyHeight/font が横画面高さに比例拡縮。
  ...
  - **コマンド単位の頭出し（∧∨・0.8.525）**: シェルが出す `OSC 133 ; A`（プロンプトの頭）を印として記録し、∧∨ でその行へ飛ぶ。⭐ **印は絶対行番号ではなく `TerminalRow` そのものに持たせる**（`promptMark`）— スクロールバックが溢れると絶対行番号は全部ずれるが、行に付けておけば行が捨てられるときに一緒に消える（`TerminalBuffer.scrollUp` は行オブジェクトをそのままスクロールバックへ移すので、印も付いて回る）。⚠ **行を丸ごと消したら印も落とす** — 使い回された行に前の印が残ると、何も書かれていない行が「コマンドの頭」として拾われる。⚠ **alt screen では印を付けない** — 全画面を描く TUI が出した `OSC 133` を拾うと履歴に残らない画面に印が付き、その TUI を抜けた後もずっと空振りする。⭐ **拾うのは `A` だけ**（`B` / `C` / `D` は受け取っても何もしない。⚠ **知らない字で例外を投げない** — 投げると受信ループごと落ちて、そこから後の出力が丸ごと消える）。頭から次の頭の手前までが 1 コマンド分なので、`A` だけで「1 つ前のコマンドへ」は成立する。終了コードで拾い分けたくなったら `D` を足す。⚠ **飛び先はその行を画面の一番上に置く**（`TerminalSession.scrollTopToAbsRow`。検索の `scrollToAbsRow` は真ん中に置くが、頭出しではその行から**下**を読みたい）。⚠ **端では何もしない** — 黙って一番上/下へ飛ぶと、押しすぎたのか印が無いのか分からなくなる。⛔ **スクロールバーに目盛りは出していない** — 検索の目盛りと二重になるうえ、描画のたびに全行を走査することになる。
  - **印はシェルに出させる（`ProotLauncher.ensureOsc133PromptConfig`）**: 多くの distro の既定のプロンプトはシェル統合の印を出さないので、`OSC 7`（cwd）と同じ形で `/etc/bash.bashrc` と `/etc/zsh/zshrc` へ `PROMPT_COMMAND` / `precmd` のフックを仕込む。⚠ **別マーカーにする**（既に osc7 のブロックを持つ rootfs にも後付けされるように）。⚠ **効くのは rootfs のシェルだけ** — SSH で入った先や、自分でプロンプトを組んでいる環境では、その環境が `OSC 133` を出さない限り印は付かない（出す設定を使っている人はそのまま効く。印が 2 回付いても同じ行なので害はない）。⚠ **仕掛けが入るのはタブを開いた時**なので、この版を入れてから**開き直した**タブで使える。
  - **コマンド 1 回分を選ぶ（0.8.526）**: 選択中のフローティングに「コマンド全体」を出し、押すと**打ったコマンドの行から出力の終わりまで**へ選択を広げる（`TerminalBuffer.commandRangeAt` → `TerminalSession.expandSelectionToCommand`）。⭐ **コマンドの行も含める** — この機能の動機は「さっきの失敗を人に見せたい」なので、出力だけ渡しても何を打った結果か伝わらない。⭐ **頭にはその行自身も含める** — 印の付いた行を選んで「1 回分」を求めたときに 1 つ前のコマンドが返ると意図と逆になる。⚠ **最後のコマンドは最終行まで**（次の頭が無いところで打ち切ると出力が半分しか選べない）。⚠ **印が無いときはボタンを出さない** — 押しても何も起きないボタンは、壊れているのと区別が付かない。⛔ **共有ボタンは足さない**（コピーして貼れば渡せる。ボタンを増やさない）。
