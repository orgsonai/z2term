...
最終更新: 2026-09-09 / 対象バージョン: 0.8.567-alpha (versionCode 575)
...
既定のフリーフォームのサイズ・位置・移動・リサイズと枠の装飾はOSに任せます。画面遷移やリサイズ後の最大化で表示が崩れる問題は未解決です。標準の起動経路では正常な場合があり、起動先アプリだけの問題とは断定せず起動条件の差を比較します。外部アプリの枠を追従描画するオーバーレイも追加しません。分割は端末画面を前面に出し、その隣に新しいタスクを起動します。
