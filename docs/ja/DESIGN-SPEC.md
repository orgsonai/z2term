...
- ⭐ **プリセットの幅はすべて `KeyWidth.Fixed`。** `Auto` は「予算 = 枠の数」で分け直すので、
  いまの `ESC 1.4 + 英字 1.0×10 + ⌫ 1.4`（合計 12.8）とは**比率が変わって見た目が動く**。
  一から作るレイアウトは `Auto` を使い、そちらで「1 つ広げたら残りが均等に縮む」が効く。
...
- ⭐ **複製では `Fixed(1.0)` を `Auto` へ読み替える**（`asTemplate`）。プリセットは見た目を
  動かさないために幅を全部固定で書いてある（段階 1b）ので、そのまま複製すると**1 つ広げても
  他が縮まないテンプレート**になり、要望の再配分が最初から成立しない。⚠ 1.4（端の機能キー）/
  1.2（`?#`・ALT）/ 4.0（スペース）のような**意図して広げた幅はそのまま残す**。
  ⚠ その代償として**複製すると幅の比率がわずかに変わる**（Row 1 は `1.4 + 1.0×10 + 1.4` の
  合計 12.8 で描いていたのが、予算 12 枠を分け直して英字キーが 1.0 → 0.92 になる）。
