...
最終更新: 2026-08-20 / 対象バージョン: 0.8.366-alpha (versionCode 374)
...
| `attach <先>` | そのタブに**繋ぎっぱなし**にして普通に打つ（0.8.366）。抜けるのは行頭の `~.` |
...
- **抜けるのは行頭の `~.`**（ssh と同じ）。判定は**繋いだ側**でやり、アプリに届く前に食う。行頭の `~` そのものは `~~`。⛔ **Ctrl キーを奪わない**のがこの綴りを選ぶ理由（画面を使うソフトから取り上げられるキーが 1 つも無い）。
