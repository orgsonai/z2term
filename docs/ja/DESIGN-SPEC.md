...
最終更新: 2026-06-20 / 対象バージョン: 0.8.114-alpha (versionCode 122)
...
- `TerminalEmulator`: バイト列を状態機械 (Ground/Escape/CSI/OSC…) で処理。
  ...
  - DEC モード: 代替画面、カーソルキー (DECCKM)、**マウスレポート** (X10/Normal/Button/Any × Legacy/SGR/urxvt)。マウスレポートが ON（`?1000`/`?1006` 等で TUI 側が要求）の間は、`TerminalInputView` のスワイプも scrollback 操作ではなく**ホイールイベント** (button 64=上 / 65=下) へ振り向け、`encodeMouseEvent` 経由で PTY に送る。指の累積 dy が `MOUSE_WHEEL_STEP_PX (=40px)` を超えるごとに 1 ノッチ送り、長いスワイプはその回数ぶんの多行送りになる（0.8.114。SGR マウスレポート対応の TUI でタップスクロール＝次ページ送りが効くようにした。同時に **mouseEnabled 中はフリングを無効化**＝慣性で勝手にページが進む/scrollback が走る挙動を抑止）。
