...
最終更新: 2026-07-25 / 対象バージョン: 0.8.222-alpha (versionCode 230)
...
**行数はウィジェットの高さから決める（0.8.220）**: 固定にしていた頃は**縦に伸ばすと下に隙間ができた**。`OPTION_APPWIDGET_MIN_HEIGHT`（そのウィジェットが確実に持てる高さ dp）からヘッダー・フッター・余白を引き、1 行 13dp で割る（2〜30 行に丸める）。リサイズは `onAppWidgetOptionsChanged` で拾って描き直す。設定から行数を選ばせるのはやめた。
