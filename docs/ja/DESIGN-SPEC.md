...
最終更新: 2026-08-29 / 対象バージョン: 0.8.438-alpha (versionCode 446)
...
- `ssh/SshProfilesSheet.kt` + `HostKeyVerificationDialog.kt`: SSH / WebDAV / SMB 共通接続先 UI + SSH 鍵検証。方式は接続先ごとに保存し、旧JSONはSSHとして読む。非SSH接続先にはシェル・VNC・常駐トンネル操作を出さない。
- `sftp/SftpSheet.kt`: `RemoteFs` を使う SFTP / WebDAV / SMB 共通ファイルブラウザ (**全画面ページ**)。WebDAV は通常のTLS証明書検証を行うOkHttp、SMBはSMB1を無効にしたjcifs-ngで直接接続する。一覧の下方向スクロールが ModalBottomSheet の「閉じる」ドラッグと競合して勝手に閉じるため、設定ページと同じ別ページ方式に変更した。
