...
最終更新: 2026-09-24 / 対象バージョン: 0.8.653-alpha (versionCode 661)
...
`QrAction` は読み取り内容を純粋関数で分類する。http/https（ユーザー情報なし）と `//` 付きスキーム・`geo:` などの一部は `ACTION_VIEW`＋BROWSABLE（App Linksで対応アプリへ）、`tel:` は `ACTION_DIAL`、`mailto:`・MATMSG はメール作成、`sms:`/`smsto:`・SMSTO はSMS作成、WIFI はAndroid 11以上で `ACTION_WIFI_ADD_NETWORKS`（WPA/WPA2・WPA3・オープン）、それ以外はWi-Fiパネルを開いてパスワードを機密扱いでコピーする。vCard・MECARDは連絡先の追加、VEVENTは予定の追加（UTC・TZID・浮動時刻・終日）。`javascript:`・`file:`・`content:`・`data:`・`intent:`・`z2term:`・`ssh:` と、`//` の無い未知の `語:値` は対象外。起動は `QrActionLauncher` がボタン押下時だけ行い、開けるアプリが無ければ画面に表示する。`z2-qr` は `z2api 0 qr` → `QrToolsActivity.scan`（カメラ権限が無ければ要求、回転では繰り返さない）。0.8.601の `QrEntryButton` は削除し、⚙設定は `SettingsGroup.QR_TOOLS` に置く。
...
QRの入口は `QrEntryButton` としてスニペット・接続先の見出し行に置き、取っ手の行には置かない。QRツールと中継共有は別Activityのまま、`PillButton`（押せない状態を追加）・`Field`（キーボード種別を追加）・`HintBox` と `ToolScreenParts` の見出し・注記・進捗表示で組む。初めてのパネルと案内の仕様は4.14を参照。
...
詳細は[QRツールと自前の中継](QR-TOOLS.md)。共有設定は専用の設定領域にSSHプロファイルID・HTTPS origin・リモートポートだけを保存し、既定の接続先は設けません。既存の認証情報と踏み台をSshSessionFactoryで使用しますが、初期コマンド・通常の転送設定・常駐トンネル設定は実行しません。ホスト鍵は全段で既存known_hostsと厳密に照合し、未知・変更された鍵はバックグラウンドで承認しません。先にSSHタブで接続して確認します。

DirectShareServiceは選択範囲の非公開コピーを作り、127.0.0.1の空きポートにHTTPサーバーを起動します。ShareRelayはサーバー側127.0.0.1の指定ポートから端末の待受へSSHリモート転送を張ります。公開HTTPSはユーザーが用意するリバースプロキシで終端します。公開URLのhealth応答と共有トークンの一致を確認してからQRを表示し、5・15・60分の期間を開始します。接続準備は約2分で打ち切り、再接続・再公開は自動で行いません。15秒ごとの確認で応答失敗時はURLを隠し、4回連続失敗またはSSH切断で終了します。停止・準備中キャンセルはその共有の全SSH段・転送・待受を閉じます。他のサーバーは停止しません。

DirectFileServerは256ビットの共有トークン、ファイルごとの確認POSTとHttpOnly/Secure Cookie、範囲取得、接続数・ヘッダーサイズ制限を使います。Referrer-Policy: same-originを維持し、正しいブラウザーのフォーム送信を許可しながらnull・不正Originを拒否します。フォルダはOpenDocumentTreeの読み取り権限だけを引き渡し、永続権限は追加しません。合計1 GiB、ルートを含む1万項目、ルート下64階層が上限です。階層一覧と非公開コピーの準備失敗・循環・上限超過は公開前に終了します。ルート外への移動や別ファイルの確認Cookie流用は拒否します。停止・期限切れ・既定回線の変更時は接続とコピーを削除します。QRの読み取り・取り込みは実行を伴わず、新しい接続先・コマンド・非表示エッジパネルへ保存します。

外部QR受け取りは独立したQrReceiveActivity（exported）から、QrIncomingで入力を検証してQrToolsActivityの確認画面へ渡します。画像8枚・結果32件／合計32,768文字を上限とし、画像はcontent URIだけを扱います。従来のSharedIntake・share自動化経路へは流しません。アプリロック解除前は解析を始めず、回転時は処理済み状態と結果を復元し、onNewIntentでは前の入力を置き換えます。受け取りテスト6件はビルド済みで、実機実行は未確認です。
...
**起動時の競合修正（0.8.596）**: 実機更新後に検出したテーマの初期化クラッシュへ対応。共有パレットの状態をApplication起動時に先に生成し、保存済みテーマの非同期読み込み後の適用をメインスレッドへ戻す。画面描画と初回のパレット生成が競合してComposeが状態を読めない問題を防ぐ。 修正版の通常版・デバッグ版を再インストール後、それぞれ3回、計6回の新規プロセス起動でクラッシュが出ないことを確認した。
...
**確認結果（0.8.595）**: エッジ関連の単体テスト77件成功（スキップなし）、debug / 署名付きrelease / 実機テストAPKのビルド成功、lintエラー0件。通常版・debug版を実機に更新。実機のAndroidシェルとz2-edge APIを通じた再作成テスト1件成功（引用符付きの設定と項目、親と子タブの復元、既存親設定・他タブ・有効状態の保持）。管理画面の表示・スクロール・コピーの実機操作は未確認。

**0.8.594-alpha (versionCode 602)**: Linux未導入でもAndroid標準シェルが起動し、Android連携の `z2-*`、自動化、エッジパネル、タイル・ウィジェットからのコマンド実行を利用できます。`z2help` で対応範囲を表示します。保存先はLinuxと同じ共有ホームで、Linux導入済みの実行経路は維持します。Linux専用パッケージや音声・画像の補助コマンドには引き続きLinuxが必要です。

選択中のOSも「OSデータの削除」から確認後に削除できます。対象OSのターミナル・GUI・常駐処理を終了してから、rootfs、OS専用ホーム領域、取得済みイメージを削除します。共有ホームと別OSのデータは保持します。対象タブは残っているOSへ移り、最後のOSを削除した場合はAndroidシェルへ移ります。起動・インストール処理中は削除を受け付けず、root chrootの使用中プロセスやマウントが残る場合は解除後の再試行を案内します。削除失敗は画面と診断ログに残します。

**確認結果（0.8.594）**: 全単体テスト1,132件中1,130件成功、既存のRDPテスト2件はスキップ（外部サーバー・fixtureが必要）。削除専用テスト9件を含む。debug / 署名付きrelease APKビルド、lint（エラー0件）、実機テストAPKのビルドに成功。通常版・debug版0.8.594を実機へインストールし、両方の版数を確認。通常版の既存データ領域は保持。追加の実機テスト3件も成功（Androidシェルからのコマンド呼出し、選択中のテスト用OS削除と共有ホーム保持、停止処理の競合と別タブ保持）。OS未導入状態での画面からの自動起動、パネル表示・権限操作・実際のイベント発火、稼働中のLinux GUIとroot chrootの削除は未検証。

通知受信の一時診断 `z2-noti trace` も統合しました。受信データの型・文字数と抽出結果の長さを記録し、本文の値は保持しません。通知本文の欠落原因は引き続き調査中です。

**バーとメモの配色（0.8.587）**: バーは近くの背景の明暗に合わせて白・黒を自動選択し、反対色の細い縁取りを付けます。「見た目 → 取っ手 → バーの色」で自動／白／黒を選べます。自動判定はAndroid 11以降と「z2term Android 操作」のユーザー補助が必要です。メモごとに背景色・文字色を色見本または#RRGGBBで指定でき、未指定なら従来の配色を使います。背景だけを指定した場合の自動文字色、罫線・カーソルも合わせます。2026-09-12に利用者が実機動作を確認済み。

**メモの切り取り・コピー・貼り付け（0.8.632）**: ⚠ **重ねて表示する窓では、長押しで選べてもフローティングの選択メニューが出ないことがある**（利用者の報告）。あれは `PopupWindow` を親の窓にぶら下げて出すもので、Activity ではない窓では黙って出ないことがある。⇒ **同じことをパネル側のボタンからも叩けるようにする**（`EdgeNoteUi.action` ＋ `EdgeToolRow`）。⚠ **切り貼りを自分で書かない** — `TextView.onTextContextMenuItem(android.R.id.copy/cut/paste)` を叩く。選択範囲・クリップボード・取り消し履歴の扱いが本物と同じになる。⚠ **ボタンは焦点を取らない**（`isFocusable = false`）— 取ると押した瞬間に選択が外れ、コピーも切り取りも空振りする。⚠ **システムの選択メニューは殺さない** — 出る端末ではそちらの方が手数が少ないので、こちらは常に使える控えとして足すだけにする。文言は `android.R.string.copy` 等を使い、端末の言語に任せる。
...
**受信診断（0.8.588）**: `z2-noti trace start [package]` で通知検知ON・リスナー接続中に開始し、重複判定前の各受信のextras/tickerの型・UTF-16文字数、配列件数、抽出本文の文字数を保持します。通知・グループkeyはハッシュ化します。本文・タイトル・送信者名の値は保持せず、ファイルへ自動保存しません。5分で収集停止、最新128件だけをメモリに保持し、超過件数も返します。`trace dump` は収集を継続したままJSONを返し、`trace stop` は停止して返します。再開始は結果を初期化し、プロセス終了で消えます。各extrasの走査は深さ4・256要素、Bundle48項目・配列32件まで。元件数と出力件数が異なる場合は上限による省略を考慮します。抽出失敗時も元フィールドを診断し、通常の本文抽出・重複判定は変更しません。送信前に短縮された本文や未発行の通知の復元機能ではありません。ビルド未検証・追加実機テスト未実行。
...
**実装メモ（0.8.580）**: 初回の設定読み込み前に来た通知は読み込み完了まで待ちます。通知内容と保存設定をスナップショットにし、差分計算・時刻整形・保存を単一executorで直列化します。通知トリガーは別executorで保存後に処理し、遅い自動化でログ保存を塞ぎません。切断時は検知ONなら再接続を要求します。過去の通知を再生する処理は追加しません。会話・形式間重複判定の単体テスト19件と、長文Bundleの読み取りから本文出力までの実機テスト2件を用意。リリースビルド・lint検証済み。通知関連の単体テスト28件成功、実機テスト2件はAPKコンパイル済み・未実行。2026-09-12に利用者が重複改善を確認。全文欠落と連続受信時の案内文は未解決で、受信元データの診断待ち。
...
- **変換表は `AndroidKeyMapper` に置く**（`keyBytesFor`）。⚠ 内蔵キーボード（`mapKeyEvent`）と
  ...
  「同じ表を引いていること」まで固定する。
- **修飾なしの矢印は emulator に組ませる**（DECCKM 依存）。修飾付きはモードによらず xterm の CSI パラメータ形式で送る。
- ⛔ **`C-S-a` のような Shift 付きの文字キーは送らずに断る**（ユーザーが選択）。端末では Shift が文字に
  ...
  backtab は `ESC [ Z`、矢印・編集キー・F1〜F12 は xterm の修飾パラメータで区別する。
...
**自動化タブ（0.8.581）**: コマンド一覧 → 自動化で「操作自動化／自動化ルール」を切り替えます。0.8.586から操作自動化はActionMacrosBodyで管理画面をタブ内へ埋め込み、作成・編集・実行・停止・履歴をその場で扱います。ActionMacrosEditorを単独Activityと共用し、保存形式・競合検出・CLIは共通です。下書き・編集中の手順・編集スクロール位置をBundleとしてrememberSaveableに保存し、回転・ロック解除後も復元します。座標・画面要素の取得要求は呼び出し元Activityを復帰先として保持します。編集中のスワイプ閉じは抑止し、閉じるボタン・戻る・タブ切替で破棄確認を通します。自動化ルールはWhenRulesBodyの一覧・編集・有効無効・実行ログ・試行・削除・一時停止です。設定内の重複入口は削除しました。上部タブを固定し、0.8.583からは均等幅の角丸ボタンで表示します（長い名前は省略）。ツール画面とタブ選択はrememberSaveableで復帰時に保持します。
...
- **タップしたら実行する（0.8.314・利用者の判断で変更）**。0.8.313 までは「入力行に入るだけで ⏎ は人が押す」作法だったが、案内は**打ちかけの途中でも押せてしまう**ため、`ls -l` を打った後に押すと `ls -lz2-macro install remind` のような行ができ、⏎ で意図しない行が走る。**先に `Ctrl-C`（0x03）で行を捨ててから**コマンドを一括貼り付けし、その終了マーカーの後にEnterを付けて1回で送る（`runGuideCommand`）。貼り付けモードを使い、非同期書込みでもEnterが先行しないようにする。⚠ `Ctrl-C` の直後に続けて書かないこと — tty は INTR を受けた時点で**入力待ち行列を捨てる**（`ISIG` かつ `NOFLSH` 無しの既定動作）ので、同じ塊で送るとコマンドまで一緒に消える。150ms 空ける。
...
- **RSS の案内は自動化への登録から一覧の確認まで（0.8.635）**。`rss` の導入、フィード URL、通知から画面を開くための重ねて表示の許可、定期巡回、通知操作の順に進む。後ろの2本は名前付きの `z2-when` ルールとして自動化へ登録する。通知は `Z2_WHEN_ACTION` で「開く」を記事、「一覧」を `rss.sh view` へ振り分け、同梱する全言語の一覧ラベルを受け付ける。最後は取得と一覧の確認、ルールの編集場所を案内する。タイル・ウィジェットへの割り当てと `rss-open` の導入は手順から外す（マクロ自体は残す）。登録済みのカードは × で省略し、古い通知ルールは追加せず編集するよう明記する。案内とマクロの説明は共通の `rssNotificationAction` を使う。
...
- ⚠ **カードの並びはスクロールし、スワイプはタップにしない**（0.8.620・利用者の指摘「スクロールできずに投入されてしまう」）。カードの並びは画面の高さの40%までで、それを超えると並びだけがスクロールする（見出しの ✕ と脚注は固定）。長いコマンドを見せるカードが並ぶと端末を押し出して続きを見る手段が無く、しかも `clickable` は**指が枠の外へ出たときしか**タップを取り消さないので、カードの上で上下にスワイプすると離した瞬間にコマンドが送られていた。`GuideCardRow` は押した位置から touch slop を超えて動いたジェスチャーをタップに数えない（Initial パスで見るだけで消費しないので、外側のスクロールは効く）。
...
**Linux未導入時もAndroid標準シェルを起動する（0.8.594）**:

- `TerminalSession.startupPlan()` の `NeedOsInstall` は、ダウンロードせず `startTerminal()` からAndroidシェルを起動する。再起動・`z2-session new` も同じ判定を通る。設定でOSを明示選択した場合は従来どおり導入する。
- `AndroidShellEnvironment` は `filesDir/android-shell/` にCLIの読込用スクリプトとPATH用リンクを置く。リンク先はAPK同梱の `libz2android.so`。Androidの実行制約に合わせ、書込可能領域のスクリプトは `/system/bin/sh` で読む。 [Android 10の実行制約](https://developer.android.com/about/versions/10/behavior-changes-10#execute-permission)。既存のLinux用 `z2ApiScripts` の生成物からブリッジ・補助コマンド・一時ファイルのパスだけを変える。
- `HOME` は `filesDir/shared_home`（Linux内では `/root`）。自動化ルール、マクロ、パネル、操作マクロ、タイルの保存形式・保存先は変えない。両環境で使うスクリプトは `$HOME` または `~/` を使い、手動のシェルマクロは `sh "$HOME/.z2term/macros/name.sh"` で実行する。LinuxのプロファイルはAndroidシェルで読み込まない。
- `HeadlessRun` は **OSが1つも無い場合だけ** Androidシェルへ分岐する。OS導入済みなら選択中distroの従来経路を使い、選択中OSだけが消えている場合やLinux起動失敗をAndroidへ黙って置き換えない。`EdgeRunner` の出力・エラー・終了コードは実行側の `$HOME` に保存する。
- `z2help` が対応コマンドを列挙する。通知、クリップボード、電池・状態、トーチ、音量、Android操作、`z2-when`、`z2-edge`、`z2-action` 等を利用できる。`z2-audio` / `z2-img` はLinux導入が必要と明示して失敗する。Linuxパッケージ、GUI、Linux常駐サーバー、`z2-macro` のサンプル導入は従来どおりLinuxが必要。Androidの権限・ユーザー補助・イベント検知・バックグラウンド制限は変更しない。
- OSなしの案内はAndroidシェルと `z2help` を説明し、Linux導入への入口を残す。端末の起動だけでは消さず、OS導入時に消す。設定上部の案内からはLinuxセクションを開いてスクロールする。Linuxサンプルを含む従来の初回カードは、引き続きOS導入後に表示する。
- 全6言語の生成CLIをシェル構文検査する。追加検査で見つかったスペイン語のmargin除去漏れ・韓国語ヘルプのコメント継続漏れも修正する。
...
#### 家の中での宛先（`channel/LanRoute`、0.8.651）

**なぜ**: 外向きの宛先を家の中から使うと、ルーターの折り返し (NAT ループバック) に頼る。折り返さない・時々しか折り返さないルーターでは、家の Wi-Fi にいるときだけ `ConnectException: failed to connect to /<外向きアドレス>` で VNC も RDP もシェルも繋がらない（利用者の報告。PC から同じ宛先へ繋いでも失敗することで、アプリではなく経路の問題と確認した）。
**何をするか**: `SshProfile.lanHost` / `lanPort`（0 なら `port` と同じ）。`SshSessionFactory.create` の入口で `LanRoute.choose` が次をすべて満たすときだけ、家の中の宛先へ**踏み台なしで**直接繋ぐ。入口が 1 つなので、シェル・SFTP・サービス（`ServiceRoute`）・常駐トンネル・中継（`ShareRelay`）のすべてに効く。
1. 家の中の宛先が名前解決できる。
2. モバイル通信以外の今のネットワーク（`LinkProperties.linkAddresses` のアドレスと接頭辞）のどれかに入っている。⚠ 外向きアドレスは家の中のネットワークに入らないので、家の外で誤って選ばれることはない。別の家の同じ番号の LAN にいる場合は 3 で落ちる。
3. 1.5 秒以内に TCP で応答する。
4. 鍵の確認画面を出せない呼び元（常駐トンネル・中継 = `lanKnownOnly`）では、その宛先の鍵が known_hosts にある。
どれかが欠ければ従来どおり外向きの宛先（と踏み台）を使う。known_hosts は宛先ごとに別に記録される。
...
- **符号化はアプリ側でもできる（`z2-qr encode`・0.8.630）**。⚠ 上の判断の前提は「アプリに符号化する道具が無い」だったが、**0.8.602 の QR ツール画面で ZXing が APK に入った**ので前提が変わった。自前実装を復活させるのではなく、**すでに入っている実績のある実装を端末からも呼ぶ**。⭐ これで「読むだけ・作るだけ」の道具のために `pacman -S` を要求しなくて済む（タブ＝distro を作り直すたびの入れ直しが消える）。⚠ **順番は `qrencode` が先** — すでに入れている人の見た目（訂正レベル・大きさ）を変えないため。⚠ **升目は低層の [`Encoder`] から取る**（`QRCodeWriter` は指定画素へ引き伸ばした結果しか返さず、1 升を何画素で描くか決められない。升が画素の整数倍でないと縁がにじみ、カメラが読み落とす）。⚠ **文字はファイルで渡す**（改行や引用符を含む長い文字列を引数で運ぶと sh のクォートで事故る）。受け渡しは `z2-view` と同じ`/storage/app/z2api/<置き場>` で、アプリへ渡すのは**置き場の名前だけ**。
- **両方欠けたときだけ、このタブでの入れ方を出して止まる**。`qrencode` はどの distro にもあるが既定では
...
**RSS の購読先管理**: 記事一覧の「購読先」からフィード URL を追加・変更・削除できます。変更は次の巡回から反映し、取得済みの記事は残します。「記事一覧」で戻り、更新すると新しい購読先を取得します。購読先の画面は `rss.sh feeds` でも開けます。
購読先は20件ずつ表示し、URL のハッシュで対象を識別します。重複・不正 URL・古い画面からの変更を拒否し、保存は排他と原子的置換で行います。汎用フォームを使用し、追加依存はありません。

**汎用ビューとフォーム（0.8.640）**: ビューは通常画面とエッジパネルで共通です。フォームの項目・表示名・操作ID・固定引数はマクロが別JSONで定義し、アプリは入力値を順序付き引数として渡します。HTMLからシェルコードは渡せません。予約・保存形式・RSS解析などの用途別の処理はアプリ側に追加しません。利用者は同じ公開仕様で新しい用途のマクロを作れます。[仕様](VIEWER.md)。
...
- **`z2-noti` コマンド (0.8.236)**: いま出ている通知を TSV (key / パッケージ / アプリ名 / タイトル / 本文) で返すコマンド。0.8.588のtrace副コマンドは受信フィールドの型・文字数を一時記録します。通知検知は既にあったが、できるのは記録だけで、シェルから「いま何が出ているか」を見る手段が無かった。
...
- **`z2-tile` コマンド (0.8.258・クイック設定タイル)**: マクロ / コマンドを**クイック設定パネル**へ 12 枠まで置ける。`z2-tile set <1-12> <マクロ.sh|コマンド...> [-l 表示名]` / `list` / `clear <1-12|all>`。
  ...
  - **入 / 切が別コマンドのものは `--off` で 1 枚にまとめる（0.8.261）**。`z2-tile set 3 z2-torch on --off z2-torch off`。押すたびに交互に走り、**入の間だけ ON の見た目**。`z2-torch on` を割り当てただけの枠は「押すたびに `on` が走る」＝**タイルからは消せない**ので、消す側を書けるようにした（利用者の提案）。0.8.591から直接のトーチ割当は実態に連動し、OFFコマンドを省略した場合もON中のタップで消灯する。
    - ⚠ **引数を 2 つ並べるだけの形にはできない**。`z2-tile set 1 ls -la` が「入 = `ls` / 切 = `-la`」に化ける（残りの引数を 1 つのコマンドに繋ぐのが元からの読み方）。だから区切りを明示させる。`-l` と同じく **`--off` はどこに書いてもよい**。
    - 通常のコマンド組は切替状態を記憶する。0.8.591から `z2-torch` の直接割当はAndroidの実態に連動し、外部からの点灯・消灯も反映する。`z2-screen keepon` は実際の有効状態を読む。
...
- **`z2-icon` コマンド (0.8.294・ステータスバーとタイルのアイコンを描き替える)**: `z2-icon pick <対象>` / `sample [名前|対象 名前]` / `edit <対象>` / `set <対象> [ファイル|-]` / `show <対象>` / `clear <対象|all>` / `grid [24|48|64]` / `scale <対象> <24|48|64>` / `list`。対象は **`notify`（このアプリが出す通知すべてに使う 1 枚）** と **枠 1〜12（タイルは枠ごとに別の絵）**。
  ...
  - **タイルに置いた中身から絵を自動で当てる（0.8.299・[`IconSamples.guess`]）**。枠が 12 になって「並べるほど既定のアイコンばかりで見分けが付かない」が現実の問題になったので、`z2-tile set` の時点で**名前から分かる範囲だけ**絵を入れる（`remind.sh` → 時計、`battery-alert.sh` → 電池、`z2-screen keepon` → 月）。同梱マクロは全部当たる。
    ...
    - ⚠ **語の並び順が意味を持つ**。`battery-alert` は `battery` が `alert` より先で電池、`unknown-call` は `unknown` が `call` より先で注意 — どちらも**後ろの語のほうが一般的**なので、狭い意味の語を上に置く。`screenshot` も同じで、`shot`（カメラ）を `screen`（消灯）より上に置く（0.8.626）。
  ...
  - **同梱の絵はいちばん細かい一辺で描き起こす（0.8.384・同梱 16 種すべて 64）**。⚠ **24 で描いた絵を機械的に拡大しても粗さは消えない** — 元が 24 段の輪郭なら、いくら大きく敷いても段は 24 のままで、[`IconStore.scale2x`] が均せるのは段の大きさだけ（利用者の指摘: 「サイズを大きくしただけで荒いことは変わらない」）。0.8.379〜0.8.383 は**入れ物だけ広げて中身を描き直していなかった**ので、`pick` で入れた人には何も変わって見えなかった。⇒ 円・弧・多角形として 64 で刻み直す（`qr` だけは 21 モジュールを 3 ドットずつ敷いた元の模様のまま — 均しでモジュールの角が丸まってはいけない）。`IconStoreTest.everySampleIsDrawnAtTheFinestGrid` が 24 へ戻らないことを固定する。
  ...
  - **サンプルを同梱する**（[`IconSamples`]・16 種）。マス目を前にして最初から思いどおりの形を置ける人はほとんどおらず、**白紙から描き始めさせない**入口が要る（利用者の提案）。`z2-icon pick <対象>` が一覧を出して番号で選ばせ、選んだ絵はそのまま `z2-icon edit` で描き直せる — **サンプルと自作の間に段差を作らない**（どちらもただのテキスト）。サンプルは**番号でも名前でも**指せる（一覧を見て選んだ直後の 1 回と、2 回目からの打ち方を食い違わせない）。⚠ 並び順が番号なので**追加は末尾へ**。`IconStoreTest.everySampleIsAValidDrawing` が、壊れた絵を配らないことを固定する（自分で描いた絵は直せるが、同梱のものは直しようがない）。
  - **`sync` を描き直し、`camera` を足した（0.8.626）**。⚠ **`sync` は形として読めていなかった**（利用者の指摘「syncが意味分からないマークになってます」）— ほとんど閉じた円に矢じりが 2 つ乗っているだけで、**どちら回りに何が巡っているのか**が出ていなかった。⇒ **環を 2 本の弧に割り、それぞれの終端に矢じりを付ける**（点対称・時計回り）。⚠ **切れ目は矢じりが次の弧へ届かない幅を空ける** — 狭いと矢じりと弧がつながり、また「でこぼこした輪」に戻る。`camera` は**本体・上の出っ張り・レンズ**の 3 つだけで描く（⚠ 細部を足さない。ステータスバーの実表示は 24px 相当）。⚠ **レンズは地の色で抜いた輪**にする — 単色で塗り直される以上、円の内と外を地の色で分けるほかに「レンズ」を出す手が無く、塗った丸ではただの点になる。
  - **ベルとハートの輪郭（0.8.590）**: 丸い部分からベルの裾・ハートの先端まで連続する曲線で64マスの原画を描き直し、左右対称にした。旧サンプルと完全に一致する選択済みアイコンは、表示・編集用の読み出し時に新しい絵へ読み替える。24/48/64マスの敷き直しにも対応し、設定された一辺は保持する。編集された絵や他のサンプルは一致しないため対象外。保存データの書換えは行わず、更新後の表示へ反映する。
...
**特殊リンクの共通処理（0.8.614・ビルド未検証）**: `/proc/<pid>` と `/proc/<pid>/task/<tid>` の `fd/<n>`・`cwd`・`ns/<種類>`・`map_files/<範囲>` は、カーネル内のオブジェクトを参照する magic link として扱う。末尾の参照はそのままカーネルへ渡し、`pipe:[…]`・`socket:[…]`・`anon_inode:…`・` (deleted)` をファイル名に変換しない。通常のディレクトリ参照の先はゲストのパス解決を維持する。`root` は従来のゲストルートへの変換を維持し、`exe` は別名 symlink や自身の `task/<tid>` 経由でも記録済みゲスト実行ファイルを参照する。削除済み cwd からの相対参照もカーネルの参照を保持する。回帰確認は `python scripts/z2root-proc-links-test.py`（ビルド・外部通信なし）。更新前のエンジンで同種の失敗を再現済み。修正版での確認は未実施。
...
- 載せる 8 件は利用者が 1 件ずつ選んだ: ツールバーのダブルタップ (**個々のボタンを列挙せず
  ...
  タブの並べ替え / ESC の上下フリック / ⌫ の左右フリック / GUI 内のスクロール (3 本指。2 本指はズームとパン) / `z2term` でコマンド一覧と `--help` (0.8.401。当初は「`z2` + Tab で補完」と書いていたが、⚠ **補完はシェルと補完設定に依存する**ので、確実に一覧が出る既存コマンドへ変えた。`z2term` は `z2help` の薄いエイリアスとして `ProotLauncher.ensureZ2HelpScript` が launch 毎に配置している) /
...
- `terminal/TerminalScreen.kt`: 全体レイアウト。TopBar / TabBar / 描画領域 / キーボードトグル / キーボード領域。`KeyboardMode = CUSTOM | SYSTEM`。**横画面**は `LocalView.OnLayoutChangeListener` で向きを検知し、`landscapeKeyboardPosition`/`Width`/`Height` 設定に従って Row レイアウト (`SideKeyboardColumn`) に切替。`landscapeScaledStyle()` で keyHeight/font が横画面高さに比例拡縮。
  - **キーボード表示切替**: 端末・GUI 共通でツールバーの ⌨ ダブルタップに統一する。表示バーと `keyboardToggleBar` 設定は廃止し、旧保存値は参照しない。単タップはキーボード方式の切替、トリプルタップはサイズ調整。
  - **外付けキーボードを繋いだら内蔵キーボードを畳む（0.8.523）**: 文字が打てる物理キーボードが繋がっている間は `keyboardCollapsed` を倒し、画面を端末（GUI タブでは GUI）へ明け渡す。⚠ **打つ手段は消えない** — 物理キーは `TerminalInputView.onKeyDown` / `GuiInputView` が拾うので、キーボード領域の有無と入力経路は無関係。⭐ **見るのは `InputManager` のデバイス一覧**（`ui/terminal/input/PhysicalKeyboard.kt`）。`Configuration.keyboard` は QWERTY / 12 キー / 無しという粗い分類しか持たず、**どの機器がそう名乗ったのか**が分からない。1 台ずつなら 3 条件で選り分けられる: ①**仮想デバイスを除く**（画面上のキーボードは「Virtual」として常に 1 台居るので、数えると常時 ON になる）②`SOURCE_KEYBOARD` を名乗る ③`KEYBOARD_TYPE_ALPHABETIC`（音量キーやゲームパッドも `SOURCE_KEYBOARD` を名乗るが文字は打てない）。⚠ **倒すのは変化した瞬間だけ**（`LaunchedEffect(physicalKeyboard)`）。毎コンポーズで倒すと、畳んだあとに ⌨ で開き直しても即座に畳み返して**開けなくなる**。⚠ **`AndroidManifest` の `configChanges` に `keyboard` を足した** — 無いと物理キーボードの抜き差しで Activity が作り直され、タブの状態が飛ぶ。⛔ **設定は増やさない** — 畳んだあとは ⌨ のダブルタップで開けるので、ON/OFF の選択肢を作るほどの分岐ではない。
  ...
  - **ツールバー ⌨ の 3 段操作（0.8.428）**: 単タップ=キーボード方式、ダブルタップ=表示/非表示、**トリプルタップ=その場のサイズ調整帯**。`multiTapClickable` は 3 回目の待ち時間が終わるまで 1/2 回の動作を確定しないため、トリプルの途中で方式切替や収納が起きない。調整帯は現在の向きの高さ、横画面サイド配置では幅を既存設定へ書き戻す。⭐ **0.8.430 で「使い方のヒント」にも載せた** — ⌨ の 3 段目は**画面のどこにも出ていない**ので、知らなければ一生見つからない（他のヒントと同じ理由でここに置く）。
...
- **GUI メニューの日本語フォント**: `AndroidGuiFonts` が Android の `/system/fonts`・`/product/fonts`・`/system_ext/fonts` の存在するディレクトリを Linux の `/usr/local/share/fonts/z2term-android/` 配下へ bind し、Fontconfig の `conf.d` に探索先を登録する。z2root と chroot の両経路で行い、Android ホスト bind 設定には依存しない。フォントのコピー・APK 同梱・追加ダウンロードは行わない。既存の日本語字形が不足する GUI メニューへの対策で、GUI を終了して開き直すと反映する。探索先の指定は [Fontconfig の dir 設定](https://fontconfig.pages.freedesktop.org/fontconfig/fontconfig-user.html) に従う。
...
- **音声 (`service/AudioBridge.kt`)**: **オプトイン**（GUIは設定「GUI 音声」`guiAudioEnabled` ON、端末は `z2-audio run` の明示実行）。distro 内 PulseAudio(`-n` 方式で起動) → TCP → Android `AudioTrack` でブリッジ。

**端末の音声（0.8.588）**: `z2-audio install` は現在のOSへ音声パッケージを導入する。`z2-audio run COMMAND [ARG...]` は通常端末のセッションに専用の音声出力を割り当て、PulseAudioとAndroid側の接続完了を待ってからコマンドを前景実行する。引数・標準入力・終了コードを保持する。子プロセス専用のPULSE_SERVERと一時設定を使い、GUIの実行時ディレクトリや既存の設定を書き換えない。終了・中断で専用サーバーと出力を解放し、タブ終了・再起動・SSH切替でもAndroid側の出力を停止する。各実行で独立したlocalhostポートを使う。音声データはs16le/48kHz/2chで、TCP受信の分割をまたぐサンプルを保持する。バッファを抑えて追加遅延を減らすが、下流のAndroid出力遅延はプレイヤーの音声時計には含まれない。追加の録音権限は不要。SSHセッションは対象外。 rootのchroot経路にもAPI用の共有ディレクトリをバインドし、起動元OSの識別子を渡す。
...
**状態の連動と自動OFF（0.8.591）**: 「ON/OFFを表示」の下にある「状態の連動先」で選びます。既定の「自動」は、直接登録した `z2-torch` と `z2-screen keepon 時間` を実態に連動させます。トーチはAndroidの通知を受け取り、クイック設定や他アプリからのON/OFFも反映します。消灯防止は期限切れ・手動解除・期限の変更に追従し、パネルを閉じている間に終了しても開き直した表示はOFFです。マクロ内でこれらのAPIを操作した場合も、最後に操作した対応機能を連動先として記録します。初回実行前から連動させる場合や複数機能を操作するマクロは「トーチライト」「画面の消灯防止」を明示指定してください。「実行中はON・終了時にOFF」は、マクロの終了・失敗・タイムアウト・停止でOFFへ戻り、実行中の再タップで停止します。バックグラウンドへ処理を切り離してすぐ終了するマクロや、別の場所で起動した独自処理には、実態を返す状態取得コマンドと更新間隔を設定してください。状態取得コマンドは連動先の指定より優先します。文字やチェック表示は追加しません。 CLIは `button-source=auto|torch|screen|process|remember`。マクロの連動先は定義の署名と共に保存し、実行トークンで要求元を照合する。終了済み・再実行前の古いトークンや、変更・削除済み定義からの更新を拒否する。プロセス状態は実行管理を正本とし、アプリ再起動でONを持ち越さない。トーチは背面のフラッシュを優先し、初期通知前は未確定として扱う。`z2-torch status` は実際の状態を返す。直接トーチを割り当てたクイック設定タイルも同じ状態を使う。
...
`ActionDefinition.CURRENT_TARGET` は対象未指定と `target current` を表し、書式番号1・2とも受理する。`ActionProgram.usesCurrentTarget` は参照する子マクロと全分岐も検査する。GUIからの開始は前面待機を予約し、ユーザー補助の前面パッケージを250ミリ秒間隔で3回確認する。自アプリ・取得不能・対象変更では安定判定をやり直し、5秒で失敗する。待機中の停止・消灯・切断では予約を取り消す。対象を具体化した手順だけを既存のAndroid操作処理へ渡す。CLI・パネル・タイルからの開始は背景移行を行わない。
...
**スクロールは画面に触らずに頼む（0.8.627・[`AndroidNodeScroll`]）**: ⛔ **注入したスワイプは、アプリから見て指と区別が付かない**。払う操作に機能が割り当たっている画面ではその機能が動き、グライド入力のキーボードに線が当たれば文字になる（利用者の指摘:「ジェスチャーすると文字が勝手入力されたりかなり危険です」「ラインを開いてスクロールしようとすると変に送信されると怖い」）。IME のウィンドウぶんは 0.8.5xx から除いているが、**避けられるのは OS が IME だと報告した窓だけ**で、アプリ側の払う操作は避けようがない。⇒ **`ACTION_SCROLL_FORWARD` / `ACTION_SCROLL_BACKWARD` でスクロールできる部品へ直接頼む**。画面へ何も注入しないので、誤入力・誤送信・払う操作の誤爆が**原理的に起こらない**。⚠ **送り量はアプリ持ち**で、端数を頼めるのは Android 14 以降だけ（`ACTION_ARGUMENT_SCROLL_AMOUNT_FLOAT` で 1/4 画面ずつ）。⇒ 速度は**送る間隔**で作る（1 回ぶんの距離 ÷ 速度）。指で速さを変える滑らかさはスワイプに劣るので、`scroll-how=auto|node|swipe` で選べるようにした（既定 `auto`）。⚠ **`auto` の落とし先を例外で書かない** — [`AndroidNodeScroll.start`] は部品が無いとき **false を返す**（例外にすると `auto` がスワイプへ落とせない）。⚠ **ノードは古くなる**。一覧は送るたびに作り直されるので、`refresh()` が false なら探し直す（探し直さないと 1 回動いたきり止まって見える）。⚠ **点を含む部品のうちいちばん内側**を採る（外側から採ると、入れ子の一覧で外側だけが動く）。⚠ **高さ 96dp の下限はスワイプ側だけの制限**（[`EdgeScrollTiming`] が MOVE の取り込み間隔ぶんの長さを要求するため）。部品へ頼む側では確かめない。

**端末で作った頁をアプリの中で読む（`z2-view`・0.8.628・[`ViewerActivity`]）**: ⚠ **通知は「気付く」ためのもので「読む」のには向かない**（利用者の指摘:「rss ですが通知だと購読しにくいですね」）。読み物にするなら HTML が素直だが、⛔ **`file://` は他アプリへ渡せない**（`FileUriExposedException`）ので、端末で書いた頁を既定のブラウザへ渡す道は塞がっている。⛔ **HTTP サーバーを立てるのは常駐が 1 つ増える**（§10-1「常駐を増やさない」）。⇒ **自前で開く**。⚠ **中身は文字列として読み込む**（`loadDataWithBaseURL(null, …)`・`allowFileAccess=false`）— ファイルを開かせないので、ここから置き場をたどる道が無い。⚠ **JavaScript も外部通信も止める**: 書くのは端末側とはいえ、**フィードから来た文字がそのまま載る**ので読み物側には何もさせない（外部画像を取りに行かない = 開いたことが相手に伝わらない）。⚠ **`http(s)` のリンクは外のブラウザへ出す** — 中で開けるようにするとここがミニブラウザになり、通信を止めた意味が無くなる。⚠ **配色は CSS 変数で渡す**（`--z2-bg` `--z2-bg2` `--z2-fg` `--z2-dim` `--z2-line` `--z2-accent` を `<head>` の直後へ差し込む）。読み物ごとに色を決め打ちさせると、明るいテーマの人に真っ黒な頁が出る。⚠ **受け渡しは `z2-share` と同じ**`/storage/app/z2api/<置き場>/page.html` で、**アプリへ渡すのは置き場の名前だけ**（パスをそのまま受けると、端末側から任意のファイルを読ませる口になる）。上限 4 MB。

**`rss.sh` が読み物を作る（0.8.628）**: 取得のたびに `~/.z2term/rss/latest.html` を書き、`rss.sh view` で開く。通知は**そのまま**で、ボタンが「開く」（その記事をブラウザへ）と「一覧」（読み物を開く）の 2 つになる（どちらも `notify_action` で返るので、`Z2_WHEN_ACTION` で振り分ける）。⚠ **通知のボタンから画面を開くには「他のアプリの上に表示」の許可が要る** — Android は背景からの画面起動を止めるため（エッジパネルと同じ許可で、既存の「開く」も同じ条件で動いている）。⚠ **`latest.txt` はそのまま**（ウィジェットの tail と `rss-open.sh` が読む）。読み物には**いつ来たか**が要るので、`articles.tsv`（時刻・URL・題名）を別に持つ。0.8.628 より前から使っている人のぶんは `latest.txt` から起こす。⚠ **頁に載せるのは `http(s)` の URL だけ**（`javascript:` を作らせない）。⚠ 最終行を `[ -t 1 ] && echo` で終わらせない — 端末でないときに**終了コードが 1 になり、`z2-when` の記録が失敗扱いになる**（0.8.628 で直した）。

**Markdown を端末で読む（`md.sh`・0.8.629）**: 同梱マクロの 11 本目。⭐ **アプリ側に文書ビューアを作らない** — 折り返しと色は端末が、リンクは OSC 8 が、絵は kitty graphics（`z2-img`）が、頁として読むのは `z2-view` がすでに持っている。足すのは **Markdown を解く awk 1 本**だけで、用途限定の画面は 1 枚も増えない。⚠ **追加インストールを要求しない**（`sh` と `awk` だけ）。読むだけの道具に `pacman -S` が要ると、タブ（distro）を作り直すたびに入れ直しになる（`qr.sh` の `qrencode`、`rss.sh` の `python3` は「符号化」「XML 解析」という**自分で書くと必ず壊れる処理**が中身なので事情が違う）。⚠ **端末用（ANSI）と読み物用（HTML）で解析を分けない**（`mode=ansi|html` の 1 本）。分けると、表の桁がずれる・引用が抜けるといった差が**片方にだけ**出て気付けない。⚠ **見た目の桁は自分で数える**: `LC_ALL=C` でバイト列として扱い、幅の判定は [`EastAsianWidth`] と同じ範囲にそろえる。`awk` の `length()` は gawk が文字数・busybox がバイト数を返すので、任せると同じ表が端末ごとに崩れる。曖昧幅は既定 1（アプリの既定と同じ）で、`Z2_MD_AMBIWIDTH=2` で全角側へ切り替える。⚠ **折り返しの残り桁を引き算で出さない** — 折り返しの機会が「いま見ている文字の直前」だったとき（行末ちょうどに空白が来た場合）に -1 になり、次の行が 1 桁はみ出す。短い残りを測り直すほうが安い。⚠ **awk はファイルに書いて `-f` で渡す** — 引数として渡すと、gawk が 4096 バイトごとに読む境界に**多バイト文字がまたがったところでプログラムが切れたことにされ**、`unexpected newline or end of string` で落ちる（日本語コメント入りの長いプログラム特有）。⚠ **`*強調*` は下線で出す** — 端末が SGR 3（斜体）を描かないため、そのままでは素の字と区別が付かない。`man` と同じ逃げ方。**端末が斜体を描くようになったらここを戻すこと。**⚠ **コードの中の行頭 `>` を引用と間違えない**（リダイレクトや `>>>` は普通に出てくる）。引用の剥がしは ``` の外でだけ行う。⚠ **絵は頁へ埋め込む**（`data:` URI・1 枚 1.5MB まで）。`z2-view` は外の絵を取りに行かないので、埋め込まなければ出ない。`http(s)` の絵は説明だけにする。⚠ **画面の幅に入らない表は「見出し: 中身」の並びに崩す** — 横に溢れた表は携帯では読めない。

**相手はいま触っている窓（0.8.627）**: 対象の選び方を `isFocused` から **`isActive` 優先**へ変えた。⚠ 入力欄を持たない窓や自由な大きさの窓（フリーフォーム）は**入力フォーカスを持たないことがあり**、「前面に出ているのにスクロールできない」になっていた（利用者の指摘:「フリーフォームウィンドウとかだと上手くスクロールもしません」）。⚠ **自分の窓は相手にしない**（パネルを触った拍子に自分へ送っても何も起きないうえ、取り違えに気付けない）。
...
**項目編集のまとまりと、パネルの一括削除（0.8.620）**: 保存する値・CLIとの対応・未保存の扱いは変えず、並べ方と開閉を直した（利用者の指摘「編集を開いたところがかなり分かりにくい」「閉じられない」「パネルごと削除できない」）。
- **項目編集は名前付きのまとまりに分ける**（`EdgeSettingsUi.group`）: 「部品と動作」（アプリ項目は「アプリの起動」）→「表示」（表示名・アイコン・表示行数・メモの色・罫線）→「コマンド」（実行・OFF・状態取得・状態の連動先・選択時・QR）→「値」（初期値・選択肢・必須）→「接続」（渡す値・標準入力・結果を表示する欄）。見出しは罫線の下に太字で置き、欄の名前（小さく控えめな字）と見分ける。**選んだ部品・動作で表示する欄が1つも無いまとまりは見出しごと隠す**。入力欄・結果欄など値の出し手には「接続はボタン側で設定する」ことを枠付きの補足で示す。
- **まれに使う設定は末尾で折りたたむ**（`EdgeSettingsUi.fold`）: 「詳細設定」「項目の大きさ・配置」。見出しの形と左端は他のまとまりと揃え、字下げした二重の入れ子にしない。
- **開いている編集は、どの項目のものか分かるようにする**: 編集欄の左端に強調色の縦線（`EdgeSettingsUi.attached`）、対象の行は薄く色付けする（`opened`）。保存・取消は罫線の下にまとめる。
- **「編集」で開き、同じボタンで閉じる**: 開いている間は「閉じる」と表示し、2つの文言のうち長い方に幅を合わせて行が動かないようにする。行のタップも同じ開閉。閉じるのは取消と同じ扱いで、未保存の変更があれば破棄を確認する（`EdgeEditorSession.discard`）。閉じた下書きは `untrack` で追跡から外す。「自由な枠を追加」も同じボタンで開閉する。
- **「管理 → パネル・タブを削除」はチェックボックス式**: 全パネルを並べ（編集中のパネルを先頭、「表示中」と表示）、各パネルの下に子タブを字下げで並べる。パネルにチェックを入れると子タブもチェック済みで固定し、パネルごと削除する。タブだけにチェックを入れると、そのタブだけを削除して親と他のタブは残す。**先頭のタブはパネル本体なので単独では選べない**。削除ボタンは選択数を表示し、0件では押せない。押すと削除対象の一覧を赤枠で出し、確認後に `EdgeStore.removePanels` で消す。選択を変えると確認は閉じる。
  - ⚠ `removePanels` は**子タブを先に消す**。途中で失敗しても、親が残った状態でタブ一覧が整合する（先に親を消すと、残ったタブが別々のパネルとして取り残される）。存在しないIDが1つでもあれば何も消さない。0.8.619までの「表示中の板・タブを削除」は1件ずつの `removePanel` で、親を消しても子タブを残すため、パネルごとは消せなかった。
  - CLIの `z2-edge delete ID` は従来どおり、親を消しても子タブを残す。
  - 削除後は「管理」のまま、編集中のパネルが残っていればそれを、無ければ残った最初のパネルを開く。1つも残らなければエッジをOFFにする（従来と同じ）。

**段組み（0.8.645）**: 利用者の指摘「配置に自由度がない」「横並びにするとアプリもメモも横に並ぶ」「並べ方を自動にしないとターミナルのタブが全体にならない」への対応。`flow` は親板に1つでタブ全体に効き、全種類の項目を同じ並びに入れていた。
- **タブを段の積み重ねで組む**: 項目の `row=N` が段。同じ段は横に並び（幅は段の中の割合。下の「段の中の幅」）、段は番号順に上から積む。親の `flow` は見ないので、アプリのタブを変えてもターミナルのタブは巻き込まれない。
- **ターミナル・メモ・ビューのある段は残りの高さに広がる**: パネルの高さが決まっているとき（`fit=fixed`、またはターミナル等の対話部品を置いたとき）だけ。項目に `height` があれば広げない。メモは広がった段の中で本文をスクロールする。
- **項目名**: `labels` 未指定のとき、1個だけの段は名前も出し、複数の段はアイコンだけにする。
- **既存のパネルは見た目を変えずに移る**: `row` が1つも無いタブは今までどおり `flow` で描く。設定の「配置」は `flow` から段を導いて表示し（縦=1項目1段、横=全部で1段、格子=列数ごと、flow未指定の格子=アプリだけ格子で他は下へ1段ずつ、自由配置=1項目1段）、最初に並べ替えたときにタブ全体の `row` と `order` を書く（`EdgeStore.arrange`）。⚠ 横一列だったタブは段になると横スクロールしない。自由配置の座標（`at`）は段では使わない。
- **並べ替え（0.8.646で作り直し）**: 0.8.645の「↑↓で段をまたいで1歩ずつ進む」（`EdgeRows.move`）は、離れた段へ運ぶのに何度も押し、押すたびに一覧が先頭へ戻っていた（利用者の指摘「上下ボタンはかなり面倒」）。⇒ 一覧の ←→ は**段の中だけ**の入れ替え（`EdgeRows.shift`）、段の変更は「段 N ▾」のメニューから1回で選ぶ（選んだ段の末尾、または一番下の新しい段。`EdgeRows.insert` / `newRow`）。操作後はスクロール位置を保って描き直す。
- **見本は実物と同じ縦横比で描き、そこで直接動かす**（`EdgeRowSketch`）: 項目を長押しでつかみ、段へ落とすと指の位置（他の項目の中心より右にいくつあるか）へ、段と段の間の線へ落とすとそこに新しい段を作る。通常メニューと一覧の長押しドラッグは、落とした先の項目の隣へ入る（`EdgeRows.drop`）。
- **見本の高さは通常メニューと同じ条件で見積もる（0.8.649）**: 利用者の指摘「自動なのにプレビューと一致しない」。見本は「高さが決まっているか」を設定画面自身の値（設定は常に決まっていない扱い）で判定していたため、ターミナルを置いたパネルでもメモ・ターミナルの段が「残りの高さ」にならず小さく描かれ、ドラッグ開始時の％も小さかった。⇒ 通常メニューと同じ `EdgePanelLayout.bounded` か `fit=fixed` で判定する。アイコンの段は `addItem` と同じ「最低48dp＋上下余白12dp（名前付きは罫線1dp）」で見積もる（0.8.648までは約8dp低かった）。高さが決まったパネルで段が埋めきらないときは、下の空きも見本に描く。
- **段の高さと横幅はドラッグで決める**: 見本の段の下の線を上下に動かすと、その段の項目すべてに `height=N%`（パネル高さに対する割合）を書く。項目の間の線を左右に動かすと、その段の項目すべてに合計100になる `width=N%` を書く。指を離したときだけ保存し、動かしている間は見本の下に値を出す。段の見出しの「大きさを戻す」で `width` / `height` を空に戻す。見本の下の「すべての大きさを自動に戻す」（0.8.648）は、段組みのタブの全項目の `width` / `height` を空にする（並びはそのまま。大きさが1つも無いときは押せない）。
- **段の中の幅は割合で分ける**（`EdgeRows.weights`）: `N%` はそのまま、dp はパネル幅に対する割合に直す。幅の無い項目は残りを等分し、残りが無ければ指定済みの平均を取る。合計が100に満たない（1個だけの段に幅を付けた等）ときは右を空ける。高さのある段は広がらない。
- **アイコンは段と項目の大きさに合わせる（0.8.647）**（`EdgeRows.iconFit`）: 利用者の指摘「段の幅や高さを変えたらアイコンが入りきらない」。項目の枠は上下左右の余白と最低48dpの高さを必ず持ち、アイコンは `icon-size` 固定だった。⇒ 高さのある段では、段の高さ（と、名前を出さない項目では項目の幅）に収まる大きさへ拡大・縮小する。高さの無い段では、項目の幅が足りないときだけ縮める。大きさを合わせた項目は余白2dpで、48dpの最低高さを持たない。下限12dp、上限192dp。
- **別の段へ移った項目は自分の大きさを捨てる**: 並びの前後で同じ段の相手が変わったら、その項目の `width` / `height` を空にし、移った先の段の大きさに従う。
- **アプリ追加**: 段組みのタブでは、最後の段がアプリだけならそこへ、そうでなければ新しい段へ入れる。CLI で `row` を付けずに足した項目は末尾に1個の段として出る。
- **設定画面は「配置／パネル／操作／管理」**: 「配置」に段ごとの一覧と見本（段の形を小さく描いたもの）、「パネル」に大きさ・配置・アイコン・見出しとボタン・取っ手、「操作」にジェスチャーとスクロール、「管理」は従来どおり。タブの追加は「配置」と「管理」の両方から行える。「並べ方」と「格子の列数」の選択欄は廃止した（CLI の `flow` / `columns` は段の無いタブ用に残る）。

表示部品と配置は親板の `panel.conf` に保存し、全タブで共有します（段 `row` だけは項目ごと＝タブごと）。設定だけが持つ保存状態はありません。
...
| タブ列 | `tabbar=off\|on\|auto`（既定off、autoは子タブがあるとき表示。**offなら子タブがあってもタブ切り替え（Spinner）を出さない**。0.8.624） |
| ＋と⚙の位置 | `tools-place=top\|bottom`（既定は自動＝`hasNavigation`に従う。`top`はタブ切り替えが無くてもnavigation行を作ってそこへ、`bottom`はnavigationがあっても下のrowsへ。0.8.624） |
...
**囲んだところだけを撮る（`z2-shot`・0.8.623）**: `RegionShot` が担当する。流れは **パネルと取っ手を隠す → 描画の反映を待つ → 画面を1枚取る → その静止画の上を囲む → 指を離して確定 → プレビュー → 保存／共有**。⭐ **囲む操作は必ず取得済みの静止画の上で行う**（生きている画面の上で囲むと、選択中のスクロールや再描画で対象がずれるうえ、囲むための重ね表示そのものが写り込む）。`EdgeRuntime.suspendForActions(true)` でパネルと取っ手を**両方**隠し、`Choreographer` で3フレーム空けてから `AccessibilityService.takeScreenshot` を呼ぶ。⚠ `GONE` にした直後は合成が追いつかず、**消える前のパネルが写る**（固定のミリ秒で待たず、描画の区切りを数える）。完了・取消・失敗・例外のすべてで表示を戻す。囲みは自由・四角・円の3種類で、いずれも `Path` に落として外接矩形で切り出すため、出力処理は1本で共通。透過は**先に囲みの形を塗って `SRC_IN` で元画像を落とす**（`clipPath` では円や斜めの縁が階段状になる）。⛔ **`ALPHA_8` のマスクを `DST_IN` で重ねてはいけない** — 端末によっては黙って無視され、**どの形で囲んでも外接矩形がそのまま出る**（0.8.622 の誤り、0.8.623 で修正）。塗り → `SRC_IN` なら経路に `ALPHA_8` が出てこないので、この失敗自体が成立しない。⚠ **座標は画像の画素で持ち、表示のときだけ引き伸ばす** — 画面とオーバーレイの寸法は切り欠き・回転で一致しないことがあり、そのままでは「見えていた位置」と「保存された位置」がずれる。保存は `MediaStore` の `Pictures/z2term` へ PNG で、書き終わるまで `IS_PENDING` を立てる。共有は0.8.608と同じ `shared-files` キャッシュと `FileProvider` を使う。市松模様はプレビューの表示だけで、保存画像には焼き込まない。Android 11以降（`takeScreenshot`）で、ユーザー補助の有効化が前提。⛔ **バーの自動配色は約1秒ごとに画面を取っているので、その直後に撮ると `ERROR_TAKE_SCREENSHOT_INTERVAL_TIME_SHORT` で断られる**（0.8.624 までは「画面を取得できませんでした」が頻発した）。パネルを隠せば `canSample` が false になり**次の**サンプリングは止まるが、**直前の1枚とは間隔が空かない**。⇒ `EdgeHandleContrast.lastRequestAt`（直近の取得時刻）を共有し、`MIN_INTERVAL_MS` に足りない分だけ待ってから撮る（0.8.625）。待つ長さの根拠は実際の取得時刻で、固定の当てずっぽうではない。

**なぞっている間は操作バーを隠す（0.8.625）**: 案内と形の選択は撮りたい対象に重なるため、`ACTION_DOWN` で `INVISIBLE`、離す・中断で戻す（利用者の指摘「邪魔になります」）。タッチは全画面の選択ビューが受けているので、隠してもなぞりは途切れない。

**真円（0.8.625）**: 形は自由・四角・楕円・真円の4種類。真円は対角ドラッグの**短い方の辺**を直径にし、指を動かした向きへ伸ばす。長い辺に合わせると指より大きい円が出てきて狙いが付けられない。日本語表示は楕円と区別するため `oval` を「楕円」に改めた。

**プレビューでの位置直し（0.8.625）**: 切り出し結果を指でドラッグすると囲みが平行移動し、切り抜く位置が変わる（利用者の指摘「特に円は場所が難しい」）。形は変えず位置だけを直す。指の移動量は表示倍率で割って画像の画素へ直す。⚠ 画像の外へ出すと外接矩形が縮んで**切り出しの大きさまで変わる**ため、動かせる範囲へ丸める。ドラッグ中は画面を作り直さず、切り出しだけ差し替えて描き直す（`recrop`）。
...
- `EdgeStore`: 最大 64 パネル・各 64 項目。`panel.conf` は `handle=bar|button|off`、`side=left|right`、
  `offset` / `length` / `x` / `y`（百分率）、`size`（保存値2〜96 dp、形状ごとに上記範囲へ丸める）、`open`、`alpha`、`bar-color=auto|white|black`（既定auto）、`label`、直接実行用 `run`。
  メモの項目は `note-background` / `note-color`（#RRGGBB、不透明色のみ。未指定・空値で自動）を持つ。
...
- `EdgeHandleContrast`: Android 11以降の[ユーザー補助の画面取得API](https://developer.android.com/reference/android/accessibilityservice/AccessibilityService#takeScreenshot(int,%20java.util.concurrent.Executor,%20android.accessibilityservice.AccessibilityService.TakeScreenshotCallback))を使用。既定の自動色では約1秒ごとに1画像を取得し、ワーカースレッドで1回だけ読み取り用に変換して、各バーの内側4dp幅から27点の輝度を調べる。画像は判定後に解放し、保存・送信しない。自身のバーの画素を除外し、明暗の切替に幅を持たせて点滅を抑える。移動・再生成中の古い結果は破棄し、権限切断・非表示・停止時は更新を終了する。取得失敗では間隔を空け、白黒の縁取りを保持する。
...
- **修飾キーの送出**: `KeyModifiers` に Ctrl／Alt／Shift を保持し、`TerminalKeyboard.onKey` から名前付き機能キーと一緒に渡す。端末・外付けキー・`z2-session key` は `AndroidKeyMapper.namedKeyBytes` を共有する。修飾付きの矢印・Home／End・編集キー・F1〜F12 は xterm のパラメータ（Shift=2、Alt=3、Ctrl=5、併用=4/6/7/8）で一括送信する。例: Alt+↑=`ESC[1;3A`、Ctrl+←=`ESC[1;5D`。修飾なしの矢印だけ DECCKM に従う。文字の Alt は ESC 前置、Shift+Tab は backtab、Ctrl+Backspace は BS。Ctrl／Alt とワンショット Shift は送信後に解除する。カスタム `KeyAction.Chord` は文字と名前付きキーの両方を処理し、画面の修飾状態とも合成する。GUI は修飾 keysym の押下→機能キー→解放へ変換し、検索欄には VT 列を入れない。Ctrl+Shift+文字、Ctrl+Tab、Shift+Enter などを個別に区別する拡張キーボード規格は未実装。回帰テストは追加済み、ビルド未検証。
...
**0.8.618の修正**: 共通キー部品へ統合後も英字の `repeatable=false` が残っていたため、英字・大文字レイヤー・記号のプリセットを `true` に修正しました。連打開始前のフリックは従来どおり連打を取り消します。保存済みの自作配列は変更しません。
...
**設定内の配列エディタ（0.8.413、操作領域改善 0.8.633、プレビュー下固定 0.8.634）。** エディタは別ダイアログでなく既存の
設定ウィンドウ内の専用ページとし、バー全体の戻る操作とシステムバー余白を共有する。見出し・説明とキー設定をスクロールさせ、配列プレビュー・複数選択・保存・
キャンセルは下部に固定する。プレビューの高さは画面の45%か320dpの小さい方を上限とし、
入りきらないキーの段だけをプレビュー内でスクロールする。保存配列は対象種類 `face` を持ち、省略した旧JSONは
`ascii` として互換を保つ。内蔵の英字・日本語・数字と各自作配列は独立した面として個別に
ON/OFFでき、`next_face` の順番も自由に入れ替えられる。選択は単体を既定とし、明示トグルを
ONにした間だけ複数選択を蓄積する。⚠ **そのトグルはプレビュー（タップして選ぶキーボード）の側に置く**（0.8.631）。スクロールする設定欄に置くと、キーの設定を見に行った時点で画面から消え、**切り替えたい場面でだけ見えない**という形になる（利用者の指摘）。切り替えと選択は同じ手の動きなので、押す場所の隣から離さない。
幅の0.2〜5.0スライダーは設定画面と同じ意匠・0.1刻みとし、
...
**入力メソッドの出口**: 名前付き機能キーは修飾情報を含む Android `KeyEvent` で送る。通常のテキスト欄の修飾なし Enter は従来の改行／入力確定を使う。`onBytes` は通常の欄では [`ImeKeyTranslator`] で下表の操作へ読み替える。z2term 自身の端末に制御コードを送る場合は `performPrivateCommand(TERMINAL_KEY_BYTES_ACTION)` で一つのバイト列を渡し、Ctrl+C・Ctrl+J・Alt+文字を捨てたり改行変換したりしない。
...
| `0x17` / `0x15` (⌫ の左右フリック) | 単語削除 / 行頭まで削除。`getTextBeforeCursor` を見て長さを出す (`readline` の `unix-word-rubout` と同じ数え方)。端末へは専用のキー入力経路で制御バイトを送る。旧版向けのフォールバックは Ctrl+W / Ctrl+U のキーイベント |
...
端末は編集中の文字列を入力欄の形で保持していないため、単語／行削除は文字数を数えず、制御コードをキー入力として送る。`EditorInfo.privateImeOptions` の `TERMINAL_IME_OPTION` があるときだけ専用のバイト送信を使う。通常の入力欄は従来の `ImeKeyTranslator` に従う。専用コマンドの送信が受け付けられなかった場合の既存経路では、Ctrl+W／Ctrl+U を修飾付き Android キーイベントで送る。
...
| 補助キーバー | specialKeyBar | true | キー配列設定内のトグル。OS キーボード使用時に表示し、従来の ON/OFF 保存値を引き継ぐ。端末・GUI 共通 |
| 補助キーバーの配列 | specialKeyLayoutJson | 空文字 | 空・不正値は既定の ESC/TAB/CTRL/矢印/Enter/^C/^D/^L。キー配列エディターで配置・幅・分割・タップ・フリック・長押しを編集でき、通常の面巡回とは独立して保存 |
...
- **F-Droid のビルドではフォントを同梱しない (0.8.639 で手当て)。** `assets/fonts/*.ttf` は `scripts/fetch-fonts.sh` がネットワークから取るもので git に入っていないため、clone したてのツリーをビルドする F-Droid では存在しない。`verifyBundledFonts` は同梱漏れを**ビルドエラーで止める**ので、yml の prebuild で `echo allowMissingBundledAssets=true >> ../gradle.properties` を渡して**警告へ格下げ**する。⛔ **検査そのものを消さないこと** — 手元と CI は `fetch-fonts.sh` を必ず流しており、フォント無しの APK を配らないための守りとして効かせ続ける。フォント不在時は [`TerminalFonts.isAvailable`] が端末の monospace へ落とすので、選択肢が 1 つになるだけで動作に支障はない。
