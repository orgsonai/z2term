...
最終更新: 2026-08-13 / 対象バージョン: 0.8.335-alpha (versionCode 343)
...
- ⚠ **一覧に並ぶのはマクロの名前そのもの**（`watch-basic` / `battery-alert` / `daily-report` / `otp-clip` / `otp-sms` / `unknown-call` / `remind` / `rss` / `qr`。0.8.335・利用者の指摘）。0.8.334 までは説明文を並べていたが、「入門: できごとに反応する」が**何をしたいものか読めず**、どのマクロの話かも分からなかった。名前は短いまま出し、**何をするものかは開いた案内の見出しで言う**（`rss — フィードの新着を通知して、1 本ずつ読む`）。
