...
最終更新: 2026-09-10 / 対象バージョン: 0.8.571-alpha (versionCode 579)
...
選べる動作は、このパネルの開閉、戻る、ホーム、履歴、通知欄、アプリ起動、待機、コマンド、上下の単発スワイプ、可変速／固定速オートスクロール、停止、加速、減速、反転です。アプリはインストール済みの一覧から選びます。「アプリ起動 → 待機 → 上へ1回スワイプ」のように組み合わせられます。コマンドは終了、単発スワイプはAndroidの完了通知を待ってから次へ進み、失敗・取消では残りを実行しません。アプリ起動は起動要求の受付までなので、画面準備には明示的な待機を挟みます。要素出現を待つ機能はありません。
...
画面要素や入力文字は取得しない。オートスクロール用にウィンドウのID・位置・フォーカスとウィンドウ変更イベントを取得する。`status` は利用可能な操作、`permission` は許可画面。
