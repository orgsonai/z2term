...
最終更新: 2026-08-14 / 対象バージョン: 0.8.337-alpha (versionCode 345)
...
- ⚠ **`watch-basic` は「気付くまで最大 N 秒」まで説明に出す**（0.8.337・利用者の指摘: 反応が 10 秒近く遅れる）。あれは**ログを見に行く形**の雛形なので、抜き差しの瞬間ではなく次に見に行ったときに気付く（`POLL` = 既定 15 秒）。「反応する」とだけ書いてあると、遅れを**不具合として受け取る**。⚠ 数字は `MACRO_POLL_SECONDS`（`Z2MacroScript`）から `guideDesc()` で流し込み、**サンプルの `POLL` と 2 か所に書かない**。⚠ **間隔を詰めて縮めない** — 0.8.273 で計った通り、待っているだけでエンジンが CPU を使い電池に出る。遅れが困るきっかけは `z2-when`（アプリ側が待ち受けるので、`SystemEventService` がブロードキャストを受けたその場でルールが走る）で拾うのが正しい。
