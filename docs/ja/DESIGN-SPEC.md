...
最終更新: 2026-09-09 / 対象バージョン: 0.8.568-alpha (versionCode 576)
...
既定のフリーフォームのサイズ・位置・移動・リサイズと枠の装飾はOSに任せます。0.8.568では、MotorolaのAndroid 16以降で標準の縮小窓と画面遷移時の継承を要求する端末専用フラグを追加しました。確認した端末のフレームワークでは、窓モードだけを指定したタスクは標準の縮小窓として扱われません。通常のfreeform起動時だけ適用し、`--bounds-only`では追加しません。寸法や倍率は固定しません。画面遷移・リサイズ後の最大化の表示崩れについて、修正版の実機での解消確認はまだです。外部アプリの枠を追従描画するオーバーレイは追加しません。分割は端末画面を前面に出し、その隣に新しいタスクを起動します。
