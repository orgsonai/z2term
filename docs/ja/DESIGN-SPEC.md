...
最終更新: 2026-07-27 / 対象バージョン: 0.8.255-alpha (versionCode 263)
...
- **帯をアクセント色で強調する（0.8.255）**。0.8.254 までは背景 `ZtsBgSecondary` ＋枠 `ZtsBorder` で**周囲と同系の暗色**、「貼る」も**緑の文字だけ**だったため、**出ていることに気付けず貼らずに進んでしまう**（実機報告）。枠を緑 2dp、背景を緑 12%、先頭に 📋（ツールバーの 📋 と結び付ける）、「貼る」を**緑で塗りつぶしたボタン**（暗色の太字）にした。⚠ **出る場所は変えない**（`SearchBar` と同じ位置）— 動かすと「どこに出るか」の慣れが無駄になるので、強めるのは色と押しやすさだけに留める。
