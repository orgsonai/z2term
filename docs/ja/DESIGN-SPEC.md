...
最終更新: 2026-09-10 / 対象バージョン: 0.8.578-alpha (versionCode 586)
...
**操作マクロGUI（0.8.572）**: 設定 → 自動化から一覧・作成・手順編集・並べ替え・複製・実行・停止・履歴を開けます。タップ・長押し・スワイプは対象アプリ上で座標を取得し、pxまたは百分率へ戻せます。取得中のタッチは背後へ送らず、取消・画面変更・消灯・切断・2分経過で終了します。テキスト編集とCLIで同じ定義を使い、未保存の変更確認と保存競合の検出を備えます。アプリロックも引き継ぎます。ビルド未検証・実機未確認。
