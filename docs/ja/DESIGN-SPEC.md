...
最終更新: 2026-06-23 / 対象バージョン: 0.8.125-alpha (versionCode 133)
...
- `TerminalEmulator`: バイト列を状態機械 (Ground/Escape/CSI/OSC…) で処理。
  ...
  - DEC モード: 代替画面、カーソルキー (DECCKM)、**マウスレポート** (X10/Normal/Button/Any × Legacy/SGR/urxvt)。マウスレポートが ON（`?1000`/`?1006` 等で TUI 側が要求）の間は、`TerminalInputView` のスワイプを**画面種別と方向と scrollback 位置で振り分ける**: **alt screen** (lazygit/vim/htop/btop/nano 等。`buffer.primaryActive == false`) は scrollback が無いので**両方向**を PTY へ wheel として送る (0.8.119。指を上=wheel-down=button 65、指を下=wheel-up=button 64)。これをしないと下方向スワイプが scrollback フォールバックに落ちて scrollbackSize==0 ゆえ無反応となり「下にしかスクロールできない」状態になる。**primary 画面**では従来通り、**指を上方向 (=次へ進めたい) かつ scrollback が最下端 (scrollOffset==0)** のときだけ wheel-down を送る。**scrollback で過去ログを見ている途中 (scrollOffset > 0)** は上方向でも wheel ではなく scrollback の「最新側へ戻る」操作として吸収する (0.8.116。これをしないと wheel 送信時の `TerminalSession.writeBytes` が scrollback を 0 にリセット＝「いきなり最下端へジャンプ」する違和感の原因になる)。**指を下方向 (=過去を見たい)** は primary では常に scrollback 操作にフォールバック (多くの読み物 TUI が wheel-up を「端末 scrollback に任せる」設計で無視するため上向きのみ TUI へ届ければよい)。指の累積 dy が `MOUSE_WHEEL_STEP_PX (=40px)` を超えるごとに 1 ノッチ送り、長いスワイプはその回数ぶんの多行送りになる (alt では符号付きで蓄積し方向反転も自然に吸収)。フリングも同じ条件分岐で、primary では `mouseEnabled && velocityY < 0 && scrollOffset==0` のときだけ no-op、それ以外は scrollback 慣性スクロール、alt では `sendMouseWheelRows` で慣性ぶんも PTY へ wheel 変換 (lazygit 等で慣性スクロールが効く)。
...
- **primary 画面のスワイプを scrollback 専用に変更 (0.8.125)**: 0.8.124 で DECRST 1049/1047/47 経由のマウスレポート auto-OFF を入れたが、nvlg のように alt screen をまったく使わない (rmcup を `\e[2J\e[H` 等の自前リセットで代替する) TUI では auto-OFF が発火せず、exit 後も primary 上で `mouseEnabled = true` のまま残り、スワイプが `TerminalInputView.onScroll` の primary 分岐 (`mouseEnabled && atBottom && distanceY > 0`) で `\e[<65;col;row M` を PTY に流出させ続けていた (`65;53;28M65;53;27M…` がシェル prompt にリテラル入力される再発報告)。修正は **primary 画面の wheel 送信分岐そのものを削除**し、`mouseEnabled` の真偽に関わらず primary では常に scrollback 操作へフォールバックする (`if (mouseEnabled && !primaryActive)` に簡約)。lazygit/vim/htop など alt screen で動く TUI は 0.8.119 で入れた alt-screen 分岐で従来どおり wheel が届く。primary で wheel を使う TUI (mc/w3m の一部設定) は事実上見ない & primary シェルの readline 破壊の方が被害が圧倒的に重いので切り捨て。click 送信 (`sendMouseClick`) は変更なしで primary でもタップ操作は維持。これで「行儀の悪い TUI が exit 時にマウス OFF を送り忘れる」全パターン (1049 経由 / 47 経由 / そもそも alt screen 不使用) を端末側で吸収する。0.8.124 の auto-OFF (A) はそのまま残し二重防御。 2 件まとめての修正。(1) `nvlg` などの TUI が exit 時に DECRST 1049 (rmcup) だけ送って DECRST 1000/1006 (マウス OFF) を送り忘れるため、primary シェルに戻った後も `emulator.mouseEnabled = true` のまま残り、スワイプすると `TerminalInputView.kt` の `onScroll` の primary 分岐 (`mouseEnabled && atBottom`) が wheel イベントを PTY に流出させ、`\e[<65;70;39M65;69;36M…` が prompt にリテラル入力されるユーザー報告。修正は `TerminalEmulator.kt` の DECRST 1049/1047/47 (alt→primary) の各分岐で `mouseProtocol = MouseProtocol.OFF` を強制し、行儀の悪い TUI が暴発させた stale 状態を端末側で掃除する (xterm 仕様では mouse mode は alt screen 状態と独立だが、現実には rmcup 経由で抜けるアプリがマウスを切り忘れるケースが多く、被害は primary シェルの readline 破壊に集中するため救う方が実用的)。lazygit のように自分で DECRST 1006/1000 を送るアプリは二重掃除になるだけで挙動不変。`MouseEncodeTest` に DECRST 1049/1047/47 で OFF に戻ることを固定するテストを追加。(2) lazygit でフォーカス枠 (例: コミット欄) を高速スワイプすると、慣性に入った瞬間から右上の diff/詳細枠もスクロールするユーザー報告。原因は `flingRunnable` が `sendMouseWheelRows` を呼ぶときの (col,row) を画面中央 (`rows/2, cols/2`) で固定していたため、フリング段階の wheel が常に画面中央のセル＝多くの場合 diff 枠に届いて lazygit がそのペインをスクロールさせていた (スワイプ中は `sendMouseWheelFromSwipe(e2.x, e2.y, ...)` が指の位置を渡すので正しくフォーカス枠だけが動く)。修正は `onFling` で `e2.x`/`e2.y` を `flingPxX`/`flingPxY` に保存し、`flingRunnable` がそれを `sendMouseWheelRows` に渡す。`sendMouseWheelRows` 内で `pixelToAbsCell` でセルへ落とし、null (view 外/未設定の `-1f`) の場合のみ従来の画面中央へフォールバック。あわせて `onFling` で `mouseWheelAccumDy` をリセットし、連続フリングの端数持ち越しを断つ。
