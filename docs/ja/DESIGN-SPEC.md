...
「編集 → 見た目を調整」で、取っ手の太さ・長さ・不透明度と板の幅・高さ上限を数値とスライダーで仮表示できます。幅・高さはdpまたは%を入力し、スライダーは画面比率です。保存時だけ定義へ反映し、取消・閉じる・タブ切替・消灯・再読込では未保存の調整を破棄します。仮表示中は取っ手のドラッグ保存を止めます。
...
板・タブの削除は「編集」内で対象を確認して実行します。削除前にメモを保存し、板内の本文は板と一緒に削除、外部本文と子タブは維持します。最後の板を削除するとパネルを停止します。
板がない状態で有効化すると、右端にメインのバーを作り、空の板を開きます。既存の板は上書きしません。
板上の「編集」から表示中の板・タブの名前変更、親板の追加、子タブの前後移動、別の親板への移動ができます。名前とタブ順は明示的な保存・移動操作で定義へ書き戻します。
...
板の「＋タブ」で名前を入れるとタブを追加できます。CLIは `z2-edge tab main work 仕事`。
...
タブごとに「アイコンのみ／名前とアイコン」を選べます（`layout=grid|list`）。gridはrun型だけを並べ、
他の型はその下へ通常の行として表示します。項目のアイコン・名前を長押ししてドラッグし、移動先の前後へ落とすと
全項目の `order` を保存します。欄外で離した場合は変更しません。
「編集」では項目を削除でき、登録済みアプリの起動方式も変更できます。「完了」で通常表示へ戻ります。

「＋メモ」で現在のタブへメモを追加できます。`type=note` は本文を表示し、タップで編集、戻す／やり直すを備えます。
