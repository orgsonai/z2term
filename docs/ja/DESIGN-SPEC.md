...
最終更新: 2026-09-14 / 対象バージョン: 0.8.597-alpha (versionCode 605)
...
詳細は[QRツールと端末からのファイル共有](QR-TOOLS.md)。DirectFileServerは256ビットの共有トークン、ファイルごとの受信確認POSTとHttpOnly Cookie、範囲取得、接続数・ヘッダーサイズ制限を備えます。DirectShareServiceが選択範囲の非公開コピー（合計最大1 GiB）と5・15・60分の期限を管理し、停止・期限切れ・回線変更で共有用サーバーと転送中の接続を閉じ、コピーを削除します。他のサーバーは停止しません。自動は暗号化されない旨を表示したHTTP、手動のTLSは持ち込みPKCS#12、手動のHTTPは明示選択です。DirectShareNetworkはAndroidの既定ネットワークとアドレスの有効性を監視します。自動は端末に割り当てられた公開IPv4、次いでネイティブのグローバルIPv6を候補とし、プライベート・CGNAT・リンクローカル・特殊用途・無効なアドレスを除外します。アドレス照会サービスは使いません。選んだアドレスのポート0へ一度だけbindし、OSが割り当てた実ポートからURLとOriginを確定します。候補がなければ公開用サーバーを起動せず理由を表示します。フォルダはOpenDocumentTreeの選択範囲を再帰取得し、階層一覧とファイルごとの非公開コピーを作ります。上限はルートを含む1万項目とルートの下64階層です。空フォルダ・同名の別ファイルを保持し、読み取り失敗・不完全な一覧・循環・上限超過時は公開前に終了してコピーを削除します。受信URLは共有トークンと一覧内の数値IDだけで解決し、受信者が指定した端末内パスやURIは使いません。親へのリンクは選択ルートまで、受信CookieはファイルごとのHMACとパスで分離します。フォルダのprefix読み取り権限とHTTPS証明書のexact読み取り権限は別のサービス起動Intentで引き渡し、証明書の権限到着を待ってから準備します。永続権限は追加しません。カメラ権限は読み取り時だけ要求し、画像はSAF経由で取得します。QRの取り込みは実行を伴わず、新しい接続先・コマンド・非表示エッジパネルへ保存します。単体テストのソースを追加しましたが、ビルド・テスト実行・実機での通信確認は未実施です。

外部QR受け取りは独立したQrReceiveActivity（exported）から、QrIncomingで入力を検証してQrToolsActivityの確認画面へ渡します。画像8枚・結果32件／合計32,768文字を上限とし、画像はcontent URIだけを扱います。従来のSharedIntake・share自動化経路へは流しません。アプリロック解除前は解析を始めず、回転時は処理済み状態と結果を復元し、onNewIntentでは前の入力を置き換えます。受け取りテスト6件を追加、実行・ビルド未検証。
