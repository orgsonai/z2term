...
最終更新: 2026-07-25 / 対象バージョン: 0.8.219-alpha (versionCode 227)
...
#### SSH 接続 QR ウィジェット（`widget/QrWidgetProvider` / `widget/QrEncoder`、0.8.219・D3）

**何ができるか**: いまこの端末に入るための `ssh://root@<IP>:<ポート>` を QR で出す。隣の PC のカメラで読めば IP を目で写して打ち直す手間が要らない。Wi-Fi が変わって IP が動いても ⟳ か定期更新で追従する。設定画面は持たない（見せるものが 1 つしかないため）。§10-2 の D3。

**QR は自前実装**（外部ライブラリを足さない・ユーザー方針）。用途を割り切って範囲を絞ってある:
- **8bit バイトモードのみ**（UTF-8）。数字/英数モードの詰め込みはしない。
- **型番 1〜10・誤り訂正 M 固定**（バイトモードで最大 122 バイト）。ssh URI は長くても 60 文字程度なので十分。超えたら `encode` が null を返し、呼び元はテキストだけ出す。
- マスクは 0〜7 を全部試し、JIS X 0510 のペナルティ評価で最小のものを採用。
- 型番 7 以上で要る型番情報（BCH(18,6)）も入れてある。

**正しさの担保（重要）**: QR は**間違っていても「それらしい模様」が出る**ので目視では壊れに気付けない。そこで
1. **規格の既知値**を `QrEncoderTest` で突き合わせる（型番 1-M "01234567" の誤り訂正語 10 個 / 形式情報 8 通り / 型番情報 7〜10 / タイミングパターン / 位置検出パターン）。
2. 開発時に**既存実装（`qrencode`）と 1 マスずつ比較**した。マスクの選び方は実装ごとに変わり得るので `QrEncoder.encode(text, forcedMask)`（internal・テスト用）で揃える。**型番 1 / 7 / 10 のいずれも差分 0 で一致**することを確認済み。

**実装で踏んだ罠**: 形式情報の予約を `for (i in 0..8)` で回すと **`(8,6)` と `(6,8)` を白で塗り潰す**。この 2 つは形式情報ではなく**タイミングパターン**なので、潰すと読み取り機がタイミングを追えなくなる（`QrEncoderTest.timingPatternAlternates` が検出した）。また位置合わせパターンを「予約済みだから飛ばす」と書くと、**型番 7 以上でタイミング線と交差する位置のパターンが消える**。飛ばすのは 3 隅（位置検出パターンと重なる組み合わせ）だけ。

**ビットマップ化**: 余白 4 モジュール（規格の下限）を含めて 1 辺 360px に収め、**1 モジュールが整数ピクセルになる倍率に丸める**（半端だと滲んで読み取り機が迷う）。白地は背景色ではなく**画像に焼き込む**（ウィジェットの土台は暗色なので、背景任せだと余白が黒くなって読めない）。

**キャプション**: `sshd` が動いていなければ「(sshd 停止中)」を添える。**QR が読めても入れない**状態を黙って見せないため。`ServerDaemonService` の起動/停止から `QrWidgetProvider.refresh()` を呼んで追従する。
...
**何ができるか**: ホーム画面に**いまの状態**（ssh の接続先 / 常駐サーバーの稼働数 / 有効な `z2-when` ルール数 / 電池残量）を出し、下段に並べたボタンで `~/.z2term/macros/*.sh` を**アプリを開かずバックグラウンド実行**する。`z2-when`（トリガー駆動）に対して、こちらは**人が押して起こす**入口。
...
- `✓ 名前` … 一度走って終わっている。**すぐ終わるマクロで `■` が一瞬で消えるのが「勝手に停止された」ように見える**という指摘への対応で、正常終了だと分かるようにした。
- `名前` … まだ一度も走っていない（時刻は `––:––`）。
