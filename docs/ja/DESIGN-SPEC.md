...
- **長文の自動ブロック分割**: 文が 2 文節以上に分かれる長文は、変換キーを押さなくても自動で先頭文節にスプリットしブロック毎に予測する (`KkcConverter.bunsetsu` で判定)。打ちかけの 1 語では分割しない。**文節境界は正確なラティス最短経路 (`nbest` 1 位) の分割を使う** (0.8.29)。位置 DP の `segments` は単一右文脈しか持たない近似で接続コスト次第で誤分割していたため、`nbest` 1 位の分割へ切り替えた。
