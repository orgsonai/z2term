...
最終更新: 2026-09-18 / 対象バージョン: 0.8.633-alpha (versionCode 641)
...
**設定内の配列エディタ（0.8.413、操作領域改善 0.8.633）。** エディタは別ダイアログでなく既存の
設定ウィンドウ内の専用ページとし、バー全体の戻る操作とシステムバー余白を共有する。上部と配列も含めた単一スクロールとし、保存と
