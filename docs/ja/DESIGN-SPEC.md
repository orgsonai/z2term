...
最終更新: 2026-07-28 / 対象バージョン: 0.8.261-alpha (versionCode 269)
...
- **`z2-tile` コマンド (0.8.258・クイック設定タイル)**: マクロ / コマンドを**クイック設定パネル**へ 4 枠まで置ける。`z2-tile set <1-4> <マクロ.sh|コマンド...> [-l 表示名]` / `list` / `clear <1-4|all>`。
  ...
  - **約束は D1 ウィジェットのボタンと同じ**: 押すと実行、実行中は緑 (`Tile.STATE_ACTIVE`)、もう一度押すと停止。入口ごとに違う操作感を作らない。
  - **入 / 切が別コマンドのものは `--off` で 1 枚にまとめる（0.8.261）**。`z2-tile set 3 z2-torch on --off z2-torch off`。押すたびに交互に走り、**入の間だけ緑**。`z2-torch on` を割り当てただけの枠は「押すたびに `on` が走る」＝**タイルからは消せない**ので、消す側を書けるようにした（利用者の提案）。
  ...
  - **状態を持つコマンドは、その状態を緑にする（0.8.260）**。`z2-screen keepon 1h` は設定を書いて**すぐ終わる**ので、「実行中だけ緑」で描くと**押した瞬間しか光らず、シェードを開き直すと掛かっているのか切れているのか分からない**（実機で指摘）。割り当てが `z2-screen keepon <時間>` の枠だけは、緑が**「消灯しないが掛かっている間」**を指す（[`TileStore.isScreenKeepOn`]）。正本は `ScreenTimeout` が持つ保存ファイル 1 つなので、**端末から `z2-screen keepon off` を打ってもタイルが揃う**。⚠ `keepon off` と `status` は対象外 — 状態を持たない一度きりの操作なので、緑にすると「押すと消える緑」になる。
    - **外すのはアプリ内で完結させ、掛けるのは今までどおりコマンドを走らせる**。`1h` の読み方をタイル側へ書き写すと、端末の `z2-screen` と 2 か所で解釈がずれる（秒への変換は sh 側が正本）。
    - **残り時間を副題に出す**。⚠ **シェードを下ろした時点の値で止まる** — `TileService` は開いている間に描き直されないので、秒読みはできない。だから分より細かくは出さず（[`TileStore.remaining`]・**切り上げ**）、`keepon 1h` の直後に「残り 59 分」と出て短く見えることも避ける。
