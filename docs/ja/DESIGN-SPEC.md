...
最終更新: 2026-08-31 / 対象バージョン: 0.8.454-alpha (versionCode 462)
...
- **RDP はまだ利用者向け機能ではない（0.8.450〜0.8.454）**: `gui/rdp/` は X.224/TLS、CredSSP/NTLMv2 に加え、T.124 GCC・T.125 MCS 接続、Client Info、valid-client license 処理、Demand/Confirm Active の capability 交換までを依存追加なしで実装。opt-in live test は端末内の NLA 強制 FreeRDP server に対して全経路を完走し、最初の connection finalization PDU 受信まで確認した。Surface Commands / Bitmap Codecs は意図的に広告せず、次段を従来型 bitmap update に絞る。⚠ bitmap decode・connection finalization・入力・接続 UI は未接続のため、現時点で `[RDP]` は表示しない。
