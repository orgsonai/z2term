...
最終更新: 2026-09-11 / 対象バージョン: 0.8.585-alpha (versionCode 593)

**出荷前確認（0.8.584）**: リリースビルド・lint成功。単体テスト1,091件成功、2件スキップ。署名済みAPKを実機へデータ保持で更新し、版数・起動を確認済み。

**コマンド一覧のタブ（0.8.583、0.8.584）**: 上部タブを均等幅の角丸ボタンへ戻しました。「操作自動化／自動化ルール」の小タブは下線付きの新しいデザインです。上部タブの選択中は緑の枠と薄い背景、文字は等幅12sp・1行で表示します。上部の固定表示とタブ選択の復元は引き続き有効です。

**エッジメモの改善（0.8.582）**: 空欄の表示名では見出しを省き、本文から表示します。罫線のON/OFF、本文下の戻す・やり直す・文字サイズのアイコン、保存と再起動をまたぐ上限付き履歴を追加しました。通知ログのユーザーID取得を公開SDKのAPIへ置き換え、既存のコンパイルエラーも修正しました。リリースビルド・lint・関連テスト48件成功。

**管理画面の集約（0.8.581）**: 常駐サーバーと自動化はコマンド一覧ツールから設定します。「サーバー」に定義・起動停止・起動時の自動常駐・省電力・通信量上限を集約しました。「自動化」は「操作自動化」と「自動化ルール」を切り替えます。操作自動化は既存の管理画面で作成・編集・実行・停止・履歴を扱い、ルールはその場で編集できます。

設定の旧グループは「権限・通知」へ改め、「権限」と「通知・ログ」を切り替えます。権限は通知表示・通知読取・ユーザー補助・他アプリ上への表示・共有ストレージ・SMS・電池最適化除外・システム設定変更・使用状況アクセス・APKインストール・端末管理者・IME・接続USBをまとめています。開発者用の実行エンジンを解放済みならrootの再確認もできます。フォルダー選択による許可は各機能で対象を選びます。

通知・SMS・イベント・解除失敗の検知とログ書式は「通知・ログ」に残します。許可が不足する検知をONにすると「権限」へ案内します。Android設定からの復帰・許可ダイアログの結果で状態を再取得し、USB一覧は画面表示中だけ更新します。権限を一括で要求せず、各ボタンで必要な項目だけ設定します。

設定グループはPERMISSIONSへ改名しますが、開閉状態の保存IDはautomationのまま維持します。サーバー・ルール・検知・ログの保存形式とCLIは変更しません。共通権限画面はAndroid 10のストレージ実行時許可とAndroid 11以降の全ファイルアクセス、Android 13以降の通知表示許可を分岐します。USB許可は既存のUsbHostAccessを利用します。XML・文字列参照・差分・リリースビルド・lintを確認済み。管理・権限操作全体の実機確認は未完了。
...
**省電力設定の入口（0.8.581）**: コマンド一覧 → サーバーに集約します。常駐サーバーと🔒バックグラウンド常駐の双方に効くことを説明し、DataStoreキーservers_low_powerとTerminalService.startの再評価を維持します。サーバーには次回起動から反映し、常駐がどちらも動いていない間は変更してもロックの状態は変わりません。電池最適化の除外とphantom processの案内は設定 → 権限・通知 → 権限です。
...
OS の「通知アクセス」を許可し、通知検知とログ保存をONにすると、Androidから届く通知・更新を1件ずつJSONL（ts / time / pkg / app / title / text / category / key）へ保存します。ポップアップ・音の有無では除外しません。同じ通知に複数の新着メッセージが含まれる場合は本文を改行で連結します。保存OFFでも通知トリガーは利用できます。

**取得範囲（0.8.580）**: 通知未発行、送信元の本文省略、Androidが伏せた本文は、通知ログ経由では復元できません。Android 15以降ではOTPを含む通知をリスナー向けに伏せる場合があります。音やポップアップを抑えるクールダウンとは区別し、OS設定を一律に変更すれば解除できるとは扱いません。[通知リスナーAPI](https://developer.android.com/reference/android/service/notification/NotificationListenerService)、[機密通知保護](https://developer.android.com/about/versions/15/behavior-changes-all#otp-redaction)、[クールダウン](https://support.google.com/android/answer/9079661)。

**本文の抽出（NotificationText、0.8.585）**: titleはEXTRA_TITLE。会話はEXTRA_MESSAGESとEXTRA_HISTORIC_MESSAGESを読み、配列間の重なりを出現回数を保って統合します。時刻・本文に送信者識別も加え、全対応OSで受信Bundleのtext/time/sender_person/senderを直接読みます。標準Messageデコーダーは再構築時に本文を1,024 UTF-16単位で切るため使いません。最後の会話本文が展開本文・通常本文の先頭と一致する場合は、省略記号も考慮して長い本文へ補完します。長い要約文というだけでは置換しません。

会話が無い場合はEXTRA_TEXT_LINESを通常本文より先に扱い、まとめ通知の途中の行も残します。単一行は同様に補完します。その後はEXTRA_BIG_TEXT（通常本文がその続きなら補完）→ EXTRA_TEXT → EXTRA_SUB_TEXT / EXTRA_INFO_TEXT → tickerTextの順です。受け取った本文をz2term側で文字数制限しません。ただし送信元・OSで既に短縮された本文は復元できません。[AndroidのMessage再構築と長さ制限](https://android.googlesource.com/platform/frameworks/base/+/android16-qpr2-release/core/java/android/app/Notification.java)。本文の代わりに新着を知らせる案内文だけが届いた場合も記録し、特定の文言による除外や、後から届いた本文による過去ログの置換は行いません。既存のtextへ合流し、JSON・テンプレートの項目は変えません。z2-noti listも同じ抽出を使い、差分にはせず現在の全内容を返します。
...
**重複排除（NotificationText.History、0.8.585）**

- 会話は通知keyごとにメッセージ自身の時刻・送信者・本文のハッシュと出現回数を保持します。既存履歴を除外し、同文でも別時刻・別送信者や配列内で増えた出現を残します。同じグループの別keyに載る同一の時刻付きメッセージも除外します。
- 会話配列のない通知はタイトルと本文を比較します。通知全体のwhen/postTimeが変わっただけでは新着にしません。同じkeyの同内容が続く間は再保存しません。
- 同じアプリ・ユーザー・タイトル・本文は、通常通知/会話/時刻なしのまとめ行をまたいで10秒以内のコピーを除外します。異なる明示的なグループは分け、両方にメッセージの時刻・送信者があれば異なる新着を残します。識別情報が無い同文新着と再送は完全には区別できません。
- 補完時は短文・長文双方の印を保持します。先に短文しか届かず、後から全文が初めて届いた場合は全文を追加で1回保存します。
- 通知削除では通常通知の直前内容だけを忘れます。会話履歴と10秒の再作成判定は保持します。通知/グループkey各256件・key内128印、形式をまたぐ本文の印は256件までのLRUです。プロセス終了・LRU脱落を越える重複防止は保証しません。
...
**実装メモ（0.8.580）**: 初回の設定読み込み前に来た通知は読み込み完了まで待ちます。通知内容と保存設定をスナップショットにし、差分計算・時刻整形・保存を単一executorで直列化します。通知トリガーは別executorで保存後に処理し、遅い自動化でログ保存を塞ぎません。切断時は検知ONなら再接続を要求します。過去の通知を再生する処理は追加しません。会話・形式間重複判定の単体テスト19件と、長文Bundleの読み取りから本文出力までの実機テスト2件を用意。リリースビルド・lint検証済み。通知関連の単体テスト28件成功、実機テスト2件はAPKコンパイル済み・未実行。通知受信の実機確認は未完了。
...
- ⛔ **プルダウンは文字だけで置かない（0.8.522）**: キーと演算子の入口を「文字 + ▾」で置いていたところ、利用者から**「ただの白文字でプルダウンだと分からない」「三角も小さくて気付けない」**と指摘が出た。⭐ **押せるものの意匠を 1 つに揃える** — `PillButton` と同じ枠 1dp + 背景 + 角丸 6dp を与え、ボタンと違うのは**右端の ▾ だけ**にする（`DropdownAnchor`）。⚠ **▾ は本文より大きくする**（12sp の文中に 12sp の ▾ を置くと文の一部に見え、印にならない）。タップ領域も文字の高さから 縦 8dp + 横 10dp の余白つきへ広げた。
...
**自動化タブ（0.8.581）**: コマンド一覧 → 自動化で「操作自動化／自動化ルール」を切り替えます。操作自動化はActionMacrosActivityの管理画面へ進み、作成・編集・実行・停止・履歴を共用します。自動化ルールはWhenRulesBodyの一覧・編集・有効無効・実行ログ・試行・削除・一時停止です。設定内の重複入口は削除しました。上部タブを固定し、0.8.583からは均等幅の角丸ボタンで表示します（長い名前は省略）。ツール画面とタブ選択はrememberSaveableで復帰時に保持します。
...
1. **エンジン: `--wait-tracees`（0.8.253）**。渡されたときはメインの tracee が終わってもループを抜けず、残った tracee の syscall を翻訳し続け、全員が居なくなって `waitpid` が `ECHILD` を返した時点で終わる。終了コードはメインのものを使う（後から終わる tracee に上書きされない）。**子孫の寿命まで追う単発実行（`HeadlessRun`）と GUI アプリ起動（0.8.512）で渡す** — 端末タブは既定のままで「シェルを抜けたらタブが終わる」を変えない。
...
**OSS ライセンスのスクロール下端（未リリース）**: 一覧・全文ダイアログは `decorFitsSystemWindows=false` とし、各ダイアログの Android ウィンドウのインセットと可視範囲から余白を取得します。スクロール領域は見出しを除く高さを `weight(1f)` で確保し、下端の隠れる分＋24dpをスクロール内容の末尾に含めます。最終行の「ライセンス全文」「ソースを開く」が画面内まで移動できるようにします。0.8.566 の `safeDrawing` と固定余白だけでは最下部が見えないとの報告に対応した変更です。
...
大量出力の安定性（未リリース）: PTYの1チャンクの処理、画面サイズ変更、描画はバッファの共通ロックで排他し、履歴の削除・追加の途中を描画しません。履歴は設定された行数で頭打ちになります。端末ログの書き込み待ちは約1MiBに制限し、遅いストレージではPTYの読込を待たせます。記録停止時は受け付け済みの出力を順番に書き切ります。
...
改行スクロール（LF/IND）と明示的な上スクロール（SU）は、指定領域内の行だけを動かします。通常画面で領域の上端が画面最上行なら、下端が画面途中でも押し出された本文をscrollbackへ保存します（0.8.567）。これにより、下部の入力欄を固定して本文を更新するTUIでも画面外の本文が消えません。上端が画面途中の領域、代替画面、行削除（DL）は履歴を増やしません。領域外の固定行は動かさず、領域より下の行での改行も本文をスクロールさせません。
...
- `terminal/TerminalScreen.kt`: 全体レイアウト。TopBar / TabBar / 描画領域 / キーボードトグル / キーボード領域。`KeyboardMode = CUSTOM | SYSTEM`。**横画面**は `LocalView.OnLayoutChangeListener` で向きを検知し、`landscapeKeyboardPosition`/`Width`/`Height` 設定に従って Row レイアウト (`SideKeyboardColumn`) に切替。`landscapeScaledStyle()` で keyHeight/font が横画面高さに比例拡縮。
  ...
  - **外付けキーボードを繋いだら内蔵キーボードを畳む（0.8.523）**: 文字が打てる物理キーボードが繋がっている間は `keyboardCollapsed` を倒し、画面を端末（GUI タブでは GUI）へ明け渡す。⚠ **打つ手段は消えない** — 物理キーは `TerminalInputView.onKeyDown` / `GuiInputView` が拾うので、キーボード領域の有無と入力経路は無関係。⭐ **見るのは `InputManager` のデバイス一覧**（`ui/terminal/input/PhysicalKeyboard.kt`）。`Configuration.keyboard` は QWERTY / 12 キー / 無しという粗い分類しか持たず、**どの機器がそう名乗ったのか**が分からない。1 台ずつなら 3 条件で選り分けられる: ①**仮想デバイスを除く**（画面上のキーボードは「Virtual」として常に 1 台居るので、数えると常時 ON になる）②`SOURCE_KEYBOARD` を名乗る ③`KEYBOARD_TYPE_ALPHABETIC`（音量キーやゲームパッドも `SOURCE_KEYBOARD` を名乗るが文字は打てない）。⚠ **倒すのは変化した瞬間だけ**（`LaunchedEffect(physicalKeyboard)`）。毎コンポーズで倒すと、畳んだあとに ⌨ で開き直しても即座に畳み返して**開けなくなる**。⚠ **`AndroidManifest` の `configChanges` に `keyboard` を足した** — 無いと物理キーボードの抜き差しで Activity が作り直され、タブの状態が飛ぶ。⛔ **設定は増やさない** — 畳んだあとは ⌨ かトグルバーで開けるので、ON/OFF の選択肢を作るほどの分岐ではない。
  - **外付けキーボードのときは OS の入力方法に変換させる（0.8.527・利用者の指摘「日本語が打てない」）**: ⛔ **内蔵キーボード (CUSTOM) では `TerminalInputView.onCreateInputConnection` が null を返す**ので、OS の入力方法は一切関与せず、物理キーは `onKeyDown` から**生のまま PTY へ**行く。内蔵の変換エンジンは画面のキーを押したときしか通らないため、**外付けを繋ぐと日本語を打つ手段が無くなる**（0.8.523 で内蔵キーボードを畳むようにして、この穴が表に出た）。⇒ **外付けが繋がっている間は `imeEnabled` を立てる**（keyboardMode は変えない）。⚠ **`requestKeyboard` は呼ばない** — 外付けを繋いだのに画面がソフトキーボードで埋まっては、畳んだ意味が無くなる。⭐ **変換だけ OS に任せて表示は出さない**のが、物理キーボードを繋いだ Android の標準的な形。⚠ 制御キーは今までどおり通る（入力方法が消費しないキーは `TerminalInputConnection.sendKeyEvent` から `AndroidKeyMapper` へ流れる）。⚠ 英字を打つときに変換が挟まるかは**入力方法側の ON/OFF 次第**（半角/全角キーなど、PC と同じ作法で切り替える）。
...
- **ローダー予約領域とメモリ上限（0.8.516、ビルド未検証）**: 0.8.515の互換入口に到達する前に画像ローダーの子がSIGABRTで終了することを実機で確認。自前ローダーが残すAndroid Scudo primary領域（当該端末で8.25GiB）が`RLIMIT_AS`へ算入され、数GiBの上限では次のexecのbionic初期化が失敗する。GUI非依存のfork/setrlimit/execでも5GiBでは失敗し、Scudo予約量を実測加算すると成功した。z2rootのgetrlimit/setrlimit/prlimit64へ予約分の加減算を追加し、ゲストのsoft/hard上限・取得値・fork/スレッド継承を維持する。無制限化やアプリ名判定は行わない。回帰確認は`scripts/z2root-rlimit-test.py`。修正版エンジンのビルドと実機GUI再検証が済むまで、GUI問題の解消済みとは扱わない。
...
- **GUI 一式の導入 (`ensure_pkgs`)**: Xvnc / openbox / D-Bus を GUI 基盤とし、それらが揃っていれば**無通信で即起動**（Alpine は下記のbashとdesktop schemaも判定する）。**GUI 端末は自動導入も自動起動もしない (0.8.505)**。デスクトップはアプリ表示面とし、導入済みの GUI アプリを ☰ / `z2menu` から直接起動する。**未導入の基盤があるときだけ** `install_pkgs`（apk add / apt install / pacman -S）で取得する。app 側 (`TerminalScreen`) のダウンロード確認ゲートが同意を取ってから走り、`clean` のときだけ cache から入れ直す。
  ...
  - ⭐⭐⭐ **OS×アプリの組合せでGUIが開かない共通原因（0.8.514）**: (1) `GuiSession` が起動・一覧・アプリ起動のたびに全体設定のdistroを読み直しており、OS切替後も残る同じdisplayのXvncへ別OSのアプリを送っていた。⇒ `Z2_DISTRO_ID`を全prootへ注入し、`z2run`の`OPEN display distro`からGUIセッションを起動元OSへ一度だけ固定。OS切替時は同displayの古いGUI/Xvncを閉じる。(2) z2rootのfakerootは`getuid()`と`SCM_CREDENTIALS`を0へ揃えていたが、DBusが認証に使う`getsockopt(SO_PEERCRED)`だけAndroid実UIDのままで、クライアントが名乗るUID 0と不一致になりsession bus認証が全アプリ共通で失敗していた。⇒ syscall 209の成功時に`struct ucred.uid/gid`を0へ補正する。(3) GUI基盤にXDG Desktop PortalとGTK backendを全PM共通で含め、DBus経由の設定・ファイル選択・URI連携を提供する。Alpine DolphinのSIGSEGVはDolphin本体でなくKDE Frameworks 6.8.0の`libKF6GuiAddons`（Settings portalのエラー応答で空`arguments().first()`を読む既知バグ）と実機PC/map照合で確定。個別Dolphinラッパーは置かない。(4) 今後の未知クラッシュは`Z2ROOT_CRASHLOG`へ致命信号時だけPC/LRと該当mapを記録し、通常時の全syscall trace負荷なしで共通ライブラリまで一度で特定できる。
...
- ⭐ **☰ アプリ一覧（0.8.499）— GUI の中にアプリを起こす「常設の入口」**: 右クリックメニューが戻っても、それは**押し方を知らないと出ない入口**でしかない（デスクトップの何もないところを長押し）。利用者の要望は **Windows のスタートボタンにあたる常設のボタン**だったので、GUI タブのツールバーに `ToolbarButtons.APPS`（☰・`guiOnly = true`）を足し、`ui/gui/GuiAppsSheet.kt` のシートに一覧を出す。行をタップすると `GuiSession.launchApp` が `z2run` 経由で起こす。
  ...
  - ⚠ **一覧の取り直しは「更新」を押したときだけ。** パッケージを入れた直後に出ない場合も、この入口から再取得できる。
  - ⭐⭐ **OS / アプリによって開いたり開かなかったりした根本原因（0.8.512）**: ☰ からの各アプリは独立した z2root で起こすが、従来は `--wait-tracees` を渡していなかった。前景プロセスが残るアプリは動く一方、起動直後に fork して親が終了する（デーモン化・単一起動化する）アプリは、z2root がメイン tracee の終了で抜けた瞬間に `PTRACE_O_EXITKILL` で**残った窓のプロセスまで殺されていた**。同じアプリでもパッケージ版や OS のビルド設定でプロセス形態が違うため、OS とアプリの組合せ問題に見えていた。⇒ `GuiSession.launchApp` は `waitTracees = true` を渡し、子孫が全て終わるまでトレーサを生かす。個別アプリ名や OS ごとの例外は要らない。
  - ⛔ **起こしたアプリの PTY も閉じないこと。** z2root は `--kill-on-exit` なので、ルートの PTY を閉じると**配下の GUI アプリごと殺される**（`setsid` しても z2root の管理下からは逃げられない）。`waitTracees = true` は「親が自然終了した後も子を追う」対策、`GuiSession` が PTY を `appPtys` に持ち続けるのは「Android 側から不用意に親を閉じない」対策で、両方が必要。タブを閉じる (`stop`) ときだけまとめて閉じる。
...
**追加・タブ操作（未リリース）**: 「＋」から項目編集を開き、アプリ追加または「自由な枠を追加（マクロ・コマンド）」を選べます。自由な枠は tile と同様に保存済みマクロを選択でき、コマンドの直接入力も可能です。アイコン欄では z2-icon の同梱・保存済みアイコンを絵付き一覧から選びます。追加と設定の両ボタンは高さ48dpを維持し、バーの実幅に合わせて各24〜32dp幅へ詰めて横に並べます。通常のメニュー上では、縦並び・グリッドなら左右、横並びなら上下へのスワイプで前後のタブに移動できます（左／上が次、右／下が前）。タブの事前タップは不要で、端では移動せず、入力欄の操作・長押し・項目の並び方向のスクロールは維持します。バーをタップしてメニューを開き、**メニューの外側を長押し**するとバー編集を開けます。設定の「小技」にもこの操作を掲載しています。

**名前付きAndroid操作マクロ（0.8.571）**: CLIでテキスト定義を保存し、座標タップ・長押し・スワイプ、待機、アプリ起動、シェル実行、時間指定スクロールを共通実行できます。実行管理は同時1本・完了待ち・停止・タイムアウト・履歴を備えます。パネル・タイル・既存マクロ・z2-whenから同じ定義を呼べます。書式・制限・例は[Android操作マクロ](ACTION-MACROS.md)を参照してください。ビルド未検証・実機未確認。
...
**座標だけの取得と設定画面の復元（0.8.579）**: 「座標を取得」はz2termの設定・編集タスクを保持したまま背景へ移し、座標選択を開始します。アプリ起動・対象指定の手順は不要です。「画面操作」で通常操作へ切り替え、目的の画面で取得を再開できます。取得時の対象窓チェックを外し、数値だけを下書きへ戻します。保存・実行時の対象指定と対象窓検査は維持します。設定画面の開閉・スクロール位置をバックグラウンド復帰とアプリロック解除後に復元し、マクロ編集のスクロール位置もロック画面で上書きしません。ビルド未検証・実機未確認。

設定状態はロック分岐の外側の rememberSaveableStateHolder と、端末／GUI双方の settingsOpen の rememberSaveable で保持します。設定内の rememberScrollState も復元対象になります。ロック中は画面を組み立てず、解除後だけ復元します。

**操作マクロの反復・分岐・再利用（0.8.573）**: version=2 に回数指定／無限反復、端末状態や前面パッケージによる条件分岐、保存済みマクロの呼び出しを追加。呼び出し先を開始前に検査・固定し、循環を拒否します。親と子の制限時間、停止、全体10000命令の上限を共有します。GUIは反復／分岐の雛形と呼び出し先の一覧選択を備え、ブロックの階層GUIは0.8.574で追加しています。実行位置・反復回数・分岐結果も表示します。詳細は[Android操作マクロ](ACTION-MACROS.md)。ビルド未検証・実機未確認。
...
**座標選択の窓で `bottom` が View のものに解決されていた（0.8.576）**: `ActionCoordinatePicker` は `FrameLayout(service).apply { ... }` の中で `Session.bottom`（操作パネルを上下どちらへ寄せるか）を裸で書いていた。暗黙のレシーバは内側が勝つので、この名前は **`View.getBottom()`（Int）** に解決され、`if (bottom)` が型不一致でコンパイルできなかった（0.8.574 はビルド未検証のまま入っていた）。⇒ `this@Session.bottom` と修飾する。⚠ **View を組み立てる `apply` の中で外側のプロパティ名が View の持ち物（`bottom` / `top` / `left` / `right` / `alpha` / `visibility` / `id` 等）と重なったら必ず修飾する。** 同じ名前でも型が違うと**エラーになるので気付ける**が、`Boolean` 同士のように型まで一致すると**黙って別の値を読む**ので、名前が重なった時点で疑う。
**操作マクロGUIを編集画面と同じ部品で作り直す（0.8.578）**: 独自の配色と素のボタン列をやめ、`EdgeSettingsUi`（地・罫線・アクセント・ボタン4種）をそのまま使う。⚠ **0.8.577 で色だけがテーマ追従になり、形は取り残されていた** — 同じアプリの中で作法が2つあると、どちらが正しいのか決められなくなる。
...
**共通アクションとジェスチャー（未リリース）**: 「見た目」→「ジェスチャー」で、タップ・ダブルタップ・上／下／内側／外側スワイプに動作の列を割り当てます。動作はGUIで追加・削除・上下移動でき、すべて削除するとそのジェスチャーは無効になります。長押しは設定／位置変更用に残します。移動ボタンでも割り当て済みの方向は動作を優先し、位置変更には長押しを使います。編集中の仮表示では動作を実行しません。

選べる動作は、このパネルの開閉、戻る、ホーム、履歴、通知欄、アプリ起動、待機、コマンド、上下の単発スワイプ、可変速／固定速オートスクロール、停止、加速、減速、反転です。アプリはインストール済みの一覧から選びます。「アプリ起動 → 待機 → 上へ1回スワイプ」のように組み合わせられます。コマンドは終了、単発スワイプはAndroidの完了通知を待ってから次へ進み、失敗・取消では残りを実行しません。アプリ起動は起動要求の受付までなので、画面準備には明示的な待機を挟みます。要素の出現待ちは名前付き操作マクロの wait-ui から使います。

動作は1列16個まで、待機は1回0〜30000ms・合計60000msまで、コマンドは1回30秒、列全体は3分までです。取っ手への新しいタッチ、消灯、回転、再読込、サービス停止で実行中の列を取り消します。新しい列は前の列を取り消して開始し、遅れて届いた完了通知では再開しません。オートスクロール開始の動作は開始後に次へ進み、継続中のスクロール自体は列の完了後も動きます。

左端の可変速スクロールは、取っ手を左のバーにし、上／下スワイプに「移動量で速度を変更」を割り当てます。指を離して開始し、上で先へ、下で戻ります。最高速度に達するスライド距離は `gesture-range` で32〜2000dp（既定160）に設定できます。見た目のジェスチャー設定でも変更できます。開始点からタッチスロープを超えた移動量に比例して速度が増え、設定距離で最高速度に達します。最低5%の速度への切り上げは行いません。例: `z2-edge panel ID gesture-range=640` なら、最高速度40000に対して160dpで10000、640dpで40000になります。タップなど移動量のない操作から開始する場合は設定速度で先へ進みます。最高／固定速度は50〜40000dp/秒（既定600、送出スワイプの目安）。加速・減速は1.5倍／1.5分の1、反転は符号を逆にします。変更後の速度は2.5〜40000dp/秒に制限します。スクロール中の取っ手へのタッチでは一旦止め、スワイプに割り当てた速度変更・反転を離したときに適用します。タップは停止を優先するため、速度変更・反転はスワイプへ割り当てます。

「スクロール位置・横／縦」は入力フォーカスのあるアプリウィンドウ内の10〜90％（既定50％）です。短い縦スワイプの経路がウィンドウ外へ出ないよう位置を補正します。単発スワイプも同じ位置を使います。画面要素は検索せず、ウィンドウのID・位置・フォーカスだけを取得します。対象を取得できない場合は開始せず、フォーカス・寸法変更で後続のスクロールを止めます。速度変更時も元の対象と一致することを確認します。ユーザー補助の有効化が必要です。

キーボード使用中は、z2term内蔵キーボードを除く端末本文の表示領域と、AndroidのIMEを除いたウィンドウ領域に限定します。キーボードの開閉・サイズ変更で対象領域が変わった場合は停止します。`z2-key permission` はユーザー補助の個別設定画面を開き、端末が権限で拒否した場合は通常のユーザー補助設定画面へ切り替えます。

タップ停止では取っ手を開きません。取っ手外も送出中・待機中を問わず画面へのタッチで停止します。自動送信のイベントだけを除外し、実際のタッチ通知を処理します。タップは背後のアプリへ届く場合があります。送出済みのスワイプ（通常最大120ms、旧Android等の100ms間隔では最大301ms）と対象アプリの慣性は残る場合があります。高速時は距離だけでなく送出時間も短縮しますが、Androidの入力間隔に合わせて途中の移動イベントを3回以上確保します。短すぎてタップとして扱われることを防ぎます。設定速度は目標値で、実際の速度は画面高さ・更新頻度・対象アプリの処理に制約されます。

保存キーは `actions-tap` / `actions-double-tap` / `actions-up` / `actions-down` / `actions-inward` / `actions-outward`。動作を `|` で並べ、引数は `種類:UTF-8フォームエンコードした値` にします。例: `actions-double-tap=launch:org.example.app|wait:500|swipe-up`、`actions-up=scroll-variable`。コマンド中の `|`、`+`、改行などはエンコードして保持します。GUIで自動保存形式に変換され、1列16KiBまでです。未知の動作・不正引数は保存／読込時に拒否します。空文字は明示的な無効化、キー省略は旧 `open` / `run` / `gesture-*` 設定の互換読込です。`gesture-speed` は引き続き速度、`scroll-x` / `scroll-y` は対象位置です。CLIとGUIは同じ `panel.conf` を使います。

0.8.568から、エッジパネルの常駐通知はアプリアイコンの通知数・通知ドットに含めません。更新後のサービス起動時に、バッジを無効にした新しい通知チャネルへ自動移行します。旧チャネルの通知の無効化・重要度などを引き継ぎ、通知欄の稼働表示と停止ボタンは引き続き使えます。

0.8.562 から、板は項目だけを表示します。空の板には案内文も操作ボタンも出ません。既存の定義も保持したまま新しい既定表示になります。
**板の余白を長押しすると設定を開きます。取っ手は300ms長押し後、動かさず離すと設定、動かすと従来どおり移動です。タップで開くボタンはドラッグですぐ移動します（0.8.565）。** 設定の「完了」または戻るで板へ戻ります。未保存の変更があれば破棄するか確認します（0.8.567）。戻るを押した時にIMEが表示中なら、まずIMEだけを閉じて設定を維持します。
設定は板の幅から独立した画面です。設定を開いても項目のコマンドや定期更新は実行しません。
0.8.566 から編集画面を「項目」「見た目」「管理」に分けました。「項目」はアイコンと名前の一覧で、タップまたは「編集」から変更し、上下の矢印または長押しドラッグで並べ替えます。アプリの開き方と表示名は基本フォーム、コマンドや更新間隔などは「詳細設定」にあります。項目削除は編集内で名前を確認します。メモ本文ファイルは残ります。
「見た目」は大きさ・配置、アイコン・並べ方、見出し・操作ボタン、取っ手をまとめて折りたたみます。寸法・配置の画面図と保存・取消ボタンは固定表示し、取っ手は実物で仮表示します。IME表示中は画面図を畳み、入力欄をIMEより上の残り領域でスクロールできます。保存時だけ定義へ反映し、取消・閉じる・ページ／タブ切替では未保存の変更の破棄を確認します。消灯・外部からの再読込は従来どおり未保存の調整を破棄します。「項目／見た目／管理」と板のタブは同じオーバーレイ窓の内容を差し替えて遷移し、背後の画面を挟みません（0.8.567）。「管理」では名前変更、パネル・タブの追加と削除を行います。
**編集画面の見た目を作り直す（`EdgeSettingsUi`、0.8.575）**: できること・保存先・CLIとの対応・未保存の扱いは一切変えず、**見せ方だけ**を1か所へ集約した。オーバーレイにはActivityのテーマが無く、部品ごとに色と余白を書くと画面の中で作法がばらつくため、地・罫線・強調色・警告色と字の大きさは`EdgeSettingsUi`だけが決める。
- **移動の段は2つ、別々の見た目にする**: 題名と「完了」の下に「項目／見た目／管理」を下線付きの見出しで置き（現在地は下線と色で示す）、その下に板・タブを枠付きの札で並べる。同じ形の行を2段重ねると、どちらがページでどちらがタブなのか読めなくなる。
- **折りたたみは見出し行と `▾`／`▸`**。入れ子（ジェスチャーの各操作・項目の詳細設定）は枠を二重にせず、**左の罫線と字下げ**で深さを表す。
- **ボタンは4種類だけ**: 主要（塗り）・通常（枠線）・控えめ・削除（赤枠）。⚠ 使えないボタンは薄くする（`StateListDrawable`で無効時の枠と字の色も持つ）。押せるかどうかが色で分かるようにし、押せない理由を推測させない。
- **入力欄・一覧・スライダー・スイッチも同じ配色**。⚠ **スイッチはOFF側の色も指定する**（既定のままだと暗い地に溶けて「そこに何も無い」ように見える）。一覧（`Spinner`）は矢印を残したまま色だけ揃える。
- **「項目」は1項目1行**: アイコン・名前・種類・「編集」・上下移動を同じ行に収める。長い説明は一覧の下に枠付きの補足として置く（毎回読むものではないため、操作の前に立ちはだからせない）。
- **配色はアプリのパレット（`AppColors`）から取る（0.8.577）**: 編集画面は独自の色を持たず、選択中のターミナルテーマから導いた地・カード・罫線・文字と、ブランドのアクセント（ZTS グリーン）・`error` をそのまま読む。板と自動化画面（`EdgeEditorUi`）も同じ出所へ揃えたので、**設定だけ配色が違う**ことは起こらない。テーマを変えれば板も追従する。
  - ⚠ **`AppColors.applyFrom` を呼んでいたのは端末画面と IME だけ**だったので、再起動直後のように**アプリ画面を一度も出していない**状態ではオーバーレイが既定配色のまま描かれていた。⇒ `Z2TermApplication.onCreate` でプロセス開始時に 1 回適用する（画面を持たない入口＝オーバーレイ・ウィジェット・タイルのため）。独自テーマは別 DataStore を非同期で読むので、**組み込みに無い名前のときだけ**最大 3 秒待つ。端末画面はテーマ変更のたびに上書きするので二重適用は無害。
- ⚠ **通常メニュー（設定以外）の配置は変更していない。** `EdgeEditorUi`は板と自動化画面が引き続き使う。

表示部品と配置は親板の `panel.conf` に保存し、全タブで共有します。設定だけが持つ保存状態はありません。
以下の表示設定はすべて `z2-edge panel ID key=value ...` で同じように変更でき、`z2-edge get ID` で保存値を読めます。

| 設定 | 対応フィールド／コマンド |
|---|---|
| 板名・閉じる・＋・⚙を表示 | `title` / `close` / `add` / `settings` = `on\|off`（既定off） |
| タブ列 | `tabbar=off\|on\|auto`（既定off、autoは子タブがあるとき表示。列を隠しても子タブがある板にはタブ選択を表示） |
| 項目名 | `labels=on\|off`（未指定・空欄はタブのlayoutに従う。本文と実行結果は消さない） |
| 幅・高さ | `width` / `height`（dpまたは%、既定360dp・72%） |
| 高さの扱い | `fit=content\|fixed`（既定content。空の板は操作用に最低48dp、指定上限以内） |
| 配置 | `place=handle\|left\|right\|top\|bottom\|center`（既定handle） |
| 自由配置 | `at=X%,Y%`（0〜100、配置より優先。空欄で解除。板を除いた残り領域に対する割合） |
| 並べ方 | `flow=vertical\|horizontal\|grid`（未指定・空欄はタブのlayoutに従う） |
| 格子の列数・アイコン寸法 | `columns=auto` または1〜16、`icon-size=16..192` dp（既定40） |
| 取っ手の形・位置・開き方 | `handle` / `side` / `offset` / `x` / `y` / `size` / `length` / `alpha` / `open`（`z2-edge handle`でも変更可能） |
| タブ追加・名前・順序 | `z2-edge tab PARENT ID LABEL`、`panel ID label=名前`、`panel PARENT tabs=a,b` |
| 板の追加・削除 | `panel ID label=名前 handle=bar side=right` / `delete ID` |
| 項目追加・編集・削除・並べ替え | `set ID:項目 key=value ...` / `remove ID:項目` / `set ID:項目 order=N` |
| アプリ起動方式・メモ追加 | 項目の `run=z2-intent -p PACKAGE --window MODE` / `type=note` |

子タブがある板は通常メニューから直接切り替えられます。タブ列を非表示にしている場合は、先頭の現在のタブ名をタップして一覧から選択します（0.8.567）。

＋と⚙は有効にした場合だけアイコンで末尾に出ます。「見た目」→「見出し・タブ・操作ボタン」の「＋アプリ追加ボタンを表示」で通常メニューに＋を設置でき、設定を開かずにアプリを追加できます。追加の取消・戻るでも元のメニューへ戻ります。タブ追加・メモ追加・項目編集は設定内にあります。
⛔ **0.8.562〜0.8.576 の＋は設定の「項目」ページを開いていた（0.8.577 で修正）。** 隣の⚙と同じ画面に着くため、**2 つ並んでいるのに違いが分からない**（実機で指摘）。本文がうたう「設定を開かずにアプリを追加できます」とも食い違っていた。⇒ ＋は `AppPickerActivity` を直接開く（`editPanel` は設定から押したときだけ true にし、選択・取消のあと戻る先を決める）。⚠ **見分けは字だけに頼らない** — 並ぶ 2 つはどちらも 32dp の記号 1 文字なので、＋は**強調色＋太字**、⚙は**控えめな色**にして役割の重さで分ける。⚠ 「自由な枠」の追加は設定 →「項目」に残る（＋から消えたのは経路であって機能ではない）。
縦一列・横一列・格子を選べます。横一列は横スクロールします。格子の列数は未指定ならアイコン寸法から決まり、明示したflowは全種類の項目に適用します。
`layout=grid|list` はタブごとの既定表示として残ります。flow未指定のgridはrun型だけを格子にし、他の型は下へ並べます。

アイコンだけの縦バーは次の設定で作れます（GUIでも同じ項目を選択できます）。
...
バーは300ms長押しで移動できます。タップで開くボタン（`open=tap`。ボタンの既定）は長押し不要で、触れたまま動かせば移動します（0.8.565）。`open=swipe|both` のボタンは開く操作と重なるため長押しのままです。バーの当たり判定は見た目の太さとは別に最低24dpを確保します。
掴むと判定幅まで太く不透明になり、点と振動で知らせます。ドラッグ中は吸着先の辺に線を表示し、
離すと近い左右の辺へ吸着して `side` / `offset` を保存します。取消時は元の位置・見た目に戻ります。
横画面でも表示領域に合わせて位置を再計算します。上下の壁への吸着は対象外です。
...
「＋項目」と各項目の「編集」から全6種類の定義を編集できます。省略可能な欄は空欄で設定を解除します。取消は未保存の入力を破棄し、外部で変更された項目への保存は拒否します。
消灯時は板と取っ手を隠し、点灯・解除通知の後、解除状態を再確認して取っ手を復元します。待機は消灯・停止時に終了します。一時的な描画失敗で通知の受信まで停止しません。

板・タブの削除は設定内で対象を確認して実行します。削除前にメモを保存し、板内の本文は板と一緒に削除、外部本文と子タブは維持します。最後の板を削除するとパネルを停止します。
板がない状態で有効化すると、右端にメインのバーを作り、空の板を開きます。既存の板は上書きしません。
設定から表示中の板・タブの名前変更、親板の追加、子タブの前後移動、別の親板への移動ができます。名前とタブ順は明示的な保存・移動操作で定義へ書き戻します。

細いバーも指定幅で背景全体を描き、操作領域は最低24dpを維持します。
フリーフォームの開き方は `z2-intent --window freeform` に次のオプションを足して調整できます（0.8.563）。板の設定で項目の実行コマンドを編集しても、`z2-edge set ID:項目 'run=...'` で書いても同じです。

| オプション | 変更する起動条件 |
|---|---|
| `--reuse-task` | 既存タスクの再利用を許可する（0.8.566から既定動作。保存済みコマンドの互換用） |
| `--os-bounds` | 窓の位置・大きさをOSに任せる（0.8.566から既定動作。互換用） |
| `--bounds-only` | 比較用に画面内側80%の寸法だけを要求し、非公開の窓モードキーは送らない |
| `--no-scale` | Motorolaの縮小フリーフォーム指定をこの起動では送らない |

0.8.566から、無指定時は複数タスクを強制せず、位置・寸法を指定せずに窓モードだけを要求します。再起動時に毎回80%の寸法を渡すことをやめ、既存タスクと保存済みの寸法をOSが利用できるようにしました。`--reuse-task --os-bounds` は併用できます。`--bounds-only` と `--os-bounds` の併用、freeform以外での指定はエラーです。
これらは端末の標準経路との違いを比較するための切替で、画面遷移時の描画崩れや窓の装飾が直るかは実機で確認が必要です。OS設定を書き換えるものではありません。

既定のフリーフォームのサイズ・位置・移動・リサイズと枠の装飾はOSに任せます。0.8.569では、Motorola端末で標準の縮小窓と画面遷移時の継承を要求する端末専用フラグを既定で追加します。項目設定の「縮小しない」またはCLIの`--no-scale`で、窓モードだけを指定する従来表示も項目ごとに選べます。通常のfreeform起動時だけ適用し、`--bounds-only`では追加しません。他メーカーでは縮小設定による追加指定はありません。外部アプリの枠を追従描画するオーバーレイは追加しません。分割は端末画面を前面に出し、その隣に新しいタスクを起動します。
標準側が内容全体を縮小する一方、この起動経路では表示領域だけが狭まり、画面遷移後の移動操作で窓内の表示がフルサイズへ戻る報告があります。`--bounds-only` でも改善しないため、初期寸法の調整だけで直るとは扱いません。
外部アプリ・rootは使いません。フリーフォームはAOSPの非公開Bundleキー `android.activity.windowingMode=5` で要求します。このキーの対応はOS依存です。`ActivityOptions.setLaunchBounds` は明示的な `--bounds-only` 時だけ使います。分割は `FLAG_ACTIVITY_LAUNCH_ADJACENT` による要求です。
Motorola向けの追加指定は非公開Bundleキー `key_moto_flags` の `0x100000`（継承可能な標準フリーフォーム指定）です。Android 16の実装から値とタスク識別・継承の処理を確認しました。将来のOSや機種での対応は保証されません。外部サービス・署名権限・非公開メソッドの呼び出しは使いません。
...
設定の「＋タブ」で名前を入れるとタブを追加できます。CLIは `z2-edge tab main work 仕事`。
タブも板で、親の `tabs=work,home` に板IDを並べます。親自身の項目が先頭タブ、子の取っ手は非表示です。
全体64板・各64項目。タブの入れ子・循環・複数親は拒否します。`list` は `tab:親ID` を表示します。
`delete` で子を消すと親の参照も除去します。親の削除で子の板は消しません。

通常メニューでもアプリなどの実行項目を長押しドラッグして並べ替えられます（0.8.567）。移動先の枠を表示し、縦一列では上半分／下半分、横一列・格子では左半分／右半分に落として前後を決めます。端へドラッグするとスクロールし、欄外へのドロップは変更しません。余白の長押しは設定を開きます。設定内でも項目名を長押ししてドラッグし、移動先の前後へ落とすと全項目の `order` を保存します。欄外で離した場合は変更しません。設定では項目削除とアプリの起動方式変更もできます。

設定の「＋メモ」で現在のタブへメモを追加できます。`type=note` は本文を表示し、タップで編集、戻す／やり直すを備えます。
メモの表示名が空欄（空白のみも含む）なら、内部ID・アイコン・見出し行を表示せず本文から始まります（0.8.582）。
メモの項目設定で「罫線」をON/OFFできます（既定OFF、`note-lines=on|off`）。本文の折り返し行にも罫線が付きます。
編集中は本文の下に戻す・やり直す・文字を小さく・大きくするアイコンボタンを表示します。文字サイズは10〜32sp（既定16sp、`note-size`）で、メモごとに保存し閲覧・編集の両方に適用します。
戻す・やり直すの履歴も保存し、保存後・パネルを閉じた後・アプリ再起動後も使用できます。本文ファイルごとに最大100履歴、過去・未来の本文合計256KiBまで（長いメモでは回数が減ります）。履歴は板内の `.note-history/`（外部本文は共有ホームの `.z2term/edge/.note-history/`）に保持し、外部で本文が変更された場合や履歴が破損していた場合は古い履歴を使用しません。履歴保存だけが失敗しても保存済み本文は保持します。
本文は既定で `~/.z2term/edge/板ID/項目ID.txt`。`file=~/memo.txt` で共有ホームの別ファイルを指定できます。
相対パスは共有ホーム基準、絶対パスはAndroidアプリからアクセス可能なファイルを指します。ゲスト固有の絶対パスを自動変換しません。
UTF-8で64KiBまで。閉じる・戻る・タブ切替・消灯・close/off時に保存し、表示中も変更があれば10秒ごとに保存します。
外部で本文が変わった場合や保存失敗時は元を上書きせず、`.recovery` 下へ編集内容を退避して通知します。
項目の `remove` は本文ファイルを残します。板の `delete` は先に保存してから板内の本文も削除し、外部の `file` は削除しません。
メモはライブ値の `push` 対象外です。ファイルを編集してから板を開き直してください。

`z2-edge toggle` は常駐のON/OFF、`z2-edge open main --toggle` は板の開閉を切り替えます。
単一コマンド `z2-edge toggle` のタイルは実際の有効状態を表示します。有効化には解錠が必要です。
設定の「＋アプリ」または板の「＋」はアイコン・名前の一覧と検索欄を表示します。名前とパッケージ名の部分一致で絞り込み、
選ぶと定義へ保存されます。検索文字は回転時にも保持します。`z2-app pick` はパッケージ名を標準出力へ返し、
取消・120秒経過は失敗として返します。選択中も他のAPIは動きます。板のコマンドから呼ぶ場合は `timeout=130` などを指定してください。

ユーザー補助は「重ねて表示」と別の許可です。`z2-key permission` は詳細画面を開き、対応しない端末では一覧へ戻ります。
`z2-key app-info` は制限付き設定を許可するためのアプリ情報を開きます。スイッチ自体は利用者が操作します。
...
設定画面も同じ定義を読み書きし、正本は `~/.z2term/edge/<panel>/panel.conf` と `<item>.item`。
...
- `EdgeStore`: 最大 64 パネル・各 64 項目。`panel.conf` は `handle=bar|button|off`、`side=left|right`、
  ...
  `width` / `height` は `%` 付きで画面の利用可能領域比（0より大きく100%まで）、数値のみで dp（0より大きく10000まで）。
  既定は幅360dp・高さ上限72%。画面内に収め、中身が少なければ高さを縮め、超過分は縦スクロールする。
- `EdgeRuntime`: `TYPE_APPLICATION_OVERLAY` のバー／丸ボタンとパネルを主スレッドで管理する。
  全画面の透明ウィンドウ内に板を置き、欄外タップは背後へ渡さず閉じる。
  パネルは外側タップ・戻る、または任意表示の「閉じる」で閉じる。消灯時は取っ手も隠し、ロック解除後に戻す。
...
| `note` | UTF-8本文ファイルを表示・編集。Undo/Redo、閉じる時と10秒ごとの保存、64KiB上限 |
...
全体操作とオートスクロールはウィンドウのID・位置・フォーカスと変更イベントを使う。要素指定マクロ・選択・inspect では対象窓の表示文字・説明・IDも取得する。編集可能な入力欄・パスワード欄は対象外。`status` は利用可能な操作、`permission` は許可画面。
...
[ジェスチャー送出](https://developer.android.com/reference/android/accessibilityservice/AccessibilityService#dispatchGesture(android.accessibilityservice.GestureDescription,android.accessibilityservice.AccessibilityService.GestureResultCallback,android.os.Handler))、
[ウィンドウ情報](https://developer.android.com/reference/android/view/accessibility/AccessibilityWindowInfo)。
...
| | 置き場 | 量 | 持ち方 |
|---|---|---|---|
| アプリ画面 | `res/values[-<言語>]/strings.xml` | 1,129 件 | Android 標準 |
| 端末に出る文言 | `proot/*.kt` の `t(en = …, ja = …)` | 351 件 | `CliText`（rootfs へ書き出すシェルスクリプトの中身なので res では持てない） |
...
#### スペイン語を追加（B3 の 3 言語目・0.8.517）

**res 1,129 件と端末に出る文言 351 件を訳しきり、`cliComplete` を付けた 5 言語目。**
中国語 2 種で通した道がそのまま使え、**使う側のコードはまた 1 行も変えていない**（触ったのは
名簿の 1 行・`res/values-es/`・`t(…)` への `"es" to …` だけ）。

- **地域は分けない。** `es-ES` / `es-MX` / `es-419` はどれも `es` 1 つへ寄せる（`AppLanguages.matchIn` の 4 段目）。⚠ 中国語で書き方（簡体字・繁体字）を分けたのは**字が読めなくなる**からで、スペイン語の地域差は語彙に留まる。分ければ訳す量が倍になり、埋まらない方は英語へ落ちるので**同じ画面に 2 言語が混ざる**。**この判断は名簿に載せる前からテストで縛ってあった**（`AppLanguagesTest.spanishRegionsCollapseToTheBaseLanguage`。仮の名簿で書いてあったものを、載せた今回、実名簿を見る形へ直した）。
- **中南米でも通じる語を選ぶ。** `ordenador`（スペイン）ではなく `PC` / `dispositivo`、`fichero` ではなく `archivo`、画面の操作は `pulsar` ではなく `tocar` に寄せた。⚠ 用語は端末の慣習に合わせる（`portapapeles` / `pestaña` / `mosaico` / `disparador` / `servidores permanentes`）。
- ⛔ **打つ語は訳さない。** `z2help` の一覧や `remind.sh` の構文説明では、利用者が打つ語（`daily` / `weekday` / `every` / `tomorrow`）を英語のまま残し、説明だけを訳す（zh-CN / zh-TW と同じ扱い）。⚠ ただし `remind.sh` が**その言語で case で拾う語**（`pDaily` 等）は訳す — 訳さないとその言語では書けないため。**1 語で書けるものを選ぶ**（`entre semana` ではなく `laborables`）: 引数は空白で区切って読むので、2 語の訳語は語数がずれて拾えない。
- **`z2scan` の基準ファイルに書く言語コード**にも `es` を持たせた（これも `t(…)` の 1 つ）。忘れると「言語を変えた瞬間に全項目が変化扱い」になる。
- **桁で揃えた表は組み直す。** `z2help` の一覧と `z2scan` の usage は説明の開始桁を揃えてあり、訳語は英語より長いので入れ替えるだけでは列がずれる。**1 行の最大長を原文と同じ 91 桁に収める**ところまで調整した（端末の幅で折り返すと一覧として読めなくなる）。
- ⭐ **アクセント付きの字（`ñ` `á` `¿`）は入力方法の話**で、既存のフリック機構（キーごとに上下左右へ割り当てる）でそのまま賄える。⛔ **今回は配列を足していない** — 端末に打つのはコマンド（ASCII）で、スペイン語の文章を打つのは OS の入力方法の仕事。

#### 韓国語を追加（B3 の 4 言語目・0.8.520）

**res 1,131 件と端末に出る文言 351 件を訳しきり、`cliComplete` を付けた 6 言語目。**
⚠ **0.8.501 でいったん名簿から外した機械翻訳は捨てて、英語から訳し直した。**
使う側のコードはまた 1 行も変えていない（触ったのは名簿の 1 行・`res/values-ko/`・`t(…)` への `"ko" to …` だけ）。

- ⛔ **前回外した訳は下訳にも使っていない。** 0.8.501 で外した理由は誤訳の**量**ではなく**種類**で、コマンド名・識別子・プレースホルダまで訳されていた（`z2-when` / `known_hosts` / `{time}` `{from}` `{body}`）。そこだけ直しても語の選び方が信用できないので、**原文から引き直した**。
- **用語を先に決めてから訳す**（前回の失敗はここだった）: 터미널 / 탭 / 셸 / 매크로 / 클립보드 / 툴바 / 상주 서버 / 플릭 / 배포판 / 암호 문구。⚠ **tab と tap が韓国語ではどちらも「탭」になる**ので、タブは「탭」、タップする操作は「누르기」と書き分ける（「탭을 탭하세요」を避ける）。
- ⛔ **plurals は `other` だけ。** 韓国語に文法上の複数形は無く、`one` を足すと lint の `ExtraTranslation` が鳴る（中国語 2 種と同じ扱い）。
- ⛔ **打つ語は訳さない。** `daily` / `weekday` / `every` / `tomorrow` は英語のまま残す。⚠ `remind.sh` が case で拾う語だけは訳す（매일 / 평일 / 매주 / 매달 / 매년）。空白で区切って読むので**1 語で書けるもの**を選ぶ。
- ⚠⚠ **Kotlin のテンプレート参照は `${'$'}{…}` で閉じる。** `${'$'}tiles번` と書くと**ハングルまで識別子の一部**として読まれ、`Unresolved reference 'tiles번'` でコンパイルが落ちる。⭐ **日本語・中国語では起きない**（全角の漢字・かなは識別子に使えない）ので、**韓国語で初めて出る罠**。今回 8 か所で踏んだ。
- **`z2scan` の基準ファイルに書く言語コード**にも `ko` を持たせた（これも `t(…)` の 1 つ）。忘れると「言語を変えた瞬間に全項目が変化扱い」になる。
- **桁で揃えた表は組み直す。** `z2help` の一覧、`z2scan` / `z2-icon` / `z2-when` / `z2-tile` の usage は説明の開始桁を揃えてある。ハングルは全角なので**表示幅で計算し直す**（1 行の最大長は原文と同じ範囲に収めた）。
- ⭐ **キーボードは足していない。** ハングルの入力は字母の**組み立て**（오토마타）で、中国語のような変換辞書は要らないが組み立ての実装が要る。端末に打つのはコマンド（ASCII）なので、ハングルを打つ場面は OS の入力方法に切り替えれば済む（中国語と同じ判断）。
...
**物理キーボードから日本語を打つ（0.8.529・利用者の指摘「日本語打てない」）**

⛔ **内蔵キーボードには物理キーボードの入口が無かった。** フリック入力なので「かなが直接入る」
前提で組まれており、打鍵がアルファベットで届く外付けキーボードには変換の入口がどこにも無い。
⚠ **0.8.527 の「OS の入力方法に変換させる」では解決しない** — この端末では**有効な入力方法が
z2term 自身の 1 つだけ**で、変換を任せる先が自分だった（実機の `settings get secure
enabled_input_methods` で確認）。⇒ **`Z2ImeService` が物理キーを直接受ける**。

- ⭐ **新しく書くのはローマ字 → かな（`ime/RomajiKana.kt`）だけ。** 変換・候補・確定・学習は
  画面のキーボードと同じ `ComposingState` がそのまま受け持つので、**両者の挙動がずれない**。
- ⚠ **ヘボン式と訓令式を両方受ける**（si/shi・tu/tsu・hu/fu・zi/ji…）。どちらで打つかは人に
  よるので、片方しか受けないと「打てない」と受け取られる。促音（`kk` → っ）、「ん」（`n` の
  次が母音でも y でもなければ確定）、小書き（`xa` / `ltu`）も表に入れる。
- ⛔ **知らない打鍵で溜まったローマ字を捨てない** — 黙って消すと、打った字が消えて「打鍵が
  飛んだ」ようにしか見えない。そのまま文字として出す。
- ⚠ **かなモードでないときは 1 打鍵も食べない。** 端末に打つ文字を横取りするとコマンドが化ける。
  `Ctrl` / `Alt` / `Meta` 付きも同じ理由で必ず素通しする（`Ctrl+C` など）。
- ⚠ **切り替えは半角/全角・かな・変換・無変換キーと `Shift+Space`。** 英語配列の外付けには
  半角/全角キーが無いので逃げ道が要る。⛔ `Ctrl+Space` は使わない（端末で使う組み合わせを
  入力メソッドが奪うことになる）。⚠ **既定は英数** — 端末で最初に打つのはほぼコマンド。
- ⚠ **物理キーボードがあるときはキーの絵を出さない**（打つ手はもう指の下にある）。出すのは
  候補バーだけで、`onEvaluateInputViewShown` を変換中とかなモード中に限って true にする。
  ⚠ **かなモード中も開けたままにする** — 物理キーボードがあると相手のアプリはキーボードを
  要求しないので、変換が始まってから開こうとしても窓が出ず、**候補バーが 1 度も見えない**
  （0.8.530 で変換中だけに絞ったときに踏んだ）。席は塗らないので、開いていても画面には何も出ない。
- ⭐ **かなモードの印は端末画面のツールバーの ⌨ に出す**（`ime/KanaModeState.kt`・0.8.546・
  利用者の指摘「左下の『あ』が見にくい」）。物理キーボードでは画面にキーが出ないので、印が無いと
  「英数のつもりでかなが出る」を毎回踏む。⛔ **入力メソッドの窓には置かない** — 窓は端末の上に
...
- ⛔ **確定前のインライン表示は `imeEnabled` と同じ条件で出す**（0.8.546・利用者の指摘
  ...
  ⚠ ところが `TerminalRenderer` へ渡す `composingText` だけが `keyboardMode == SYSTEM` を見ていた
  ため、**届いた変換中テキストを捨てて空の `composing.text` を描いていた** = 確定するまで画面に
  何も出ず、打ちかけのかなが見えなかった。⇒ 出所の判定を `imeEnabled` と揃える。⚠ **抜き差しでも
  打ちかけを捨てる** — 出所が `composing.text` ⇄ `systemComposing` で入れ替わるので、跨ぐと
  片方に取り残される。
...
- **設定の置き場は「キーボード・入力」グループ** (0.8.277)。0.8.276 では「権限・通知」の
