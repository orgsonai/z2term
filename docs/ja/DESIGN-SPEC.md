...
**固定画面・複数面の配列エディタ（0.8.412）。** エディタをドラッグで閉じる一時シートから、
外側タップでは閉じない全画面ダイアログへ変更した。保存配列は対象面 `face` を持ち、省略した
旧JSONは `ascii` として互換を保つ。英字・日本語・数字の既定配列と、選択中の自作配列を複製・
編集でき、選択配列は対象面だけを置き換える。複数選択では外枠幅・見た目項目・明示的にコピー
したジェスチャを全キーへ一括適用し、幅には 0.2〜5.0 のスライダーも置く。数値の下書きは解析後
の幅でなく選択キーに紐づけるため、入力途中の `1.` から小数点が消えない。
