...
最終更新: 2026-07-23 / 対象バージョン: 0.8.199-alpha (versionCode 207)
...
| | 動作 |
|---|---|
| `list` | 1 行 1 タブの TSV（`番号 / id / 種別 / 印 / 名前`）。印は `*`=表示中 `!`=動作中 `-`=その他 |
| `new [名前]` | 端末タブを 1 枚開き、`番号\tid` を返す（続けて `send` する材料になる） |
