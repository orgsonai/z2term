...
最終更新: 2026-06-21 / 対象バージョン: 0.8.117-alpha (versionCode 125)
...
- `TerminalEmulator`: バイト列を状態機械 (Ground/Escape/CSI/OSC…) で処理。
  ...
  - DEC モード: 代替画面、カーソルキー (DECCKM)、**マウスレポート** (X10/Normal/Button/Any × Legacy/SGR/urxvt)。マウスレポートが ON（`?1000`/`?1006` 等で TUI 側が要求）の間は、`TerminalInputView` のスワイプを**方向と scrollback 位置で振り分ける**: **指を上方向 (=次へ進めたい) かつ scrollback が最下端 (scrollOffset==0)** のときだけホイールイベント wheel-down (button 65) を `encodeMouseEvent` 経由で PTY へ送る。**scrollback で過去ログを見ている途中 (scrollOffset > 0)** は上方向でも wheel ではなく scrollback の「最新側へ戻る」操作として吸収する (0.8.116。これをしないと wheel 送信時の `TerminalSession.writeBytes` が scrollback を 0 にリセット＝「いきなり最下端へジャンプ」する違和感の原因になる)。**指を下方向 (=過去を見たい)** は常に scrollback 操作にフォールバック (多くの読み物 TUI が wheel-up を「端末 scrollback に任せる」設計で無視するため上向きのみ TUI へ届ければよい)。指の累積 dy が `MOUSE_WHEEL_STEP_PX (=40px)` を超えるごとに 1 ノッチ送り、長いスワイプはその回数ぶんの多行送りになる。フリングも同じ条件分岐で、`mouseEnabled && velocityY < 0 && scrollOffset==0` のときだけ no-op、それ以外は scrollback 慣性スクロール。
