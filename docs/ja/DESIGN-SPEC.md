...
最終更新: 2026-09-09 / 対象バージョン: 0.8.569-alpha (versionCode 577)
...
**OSSライセンス画面の下端（0.8.566）**: 一覧と全文の両ダイアログを `decorFitsSystemWindows=false` と `WindowInsets.safeDrawing` の組み合わせに揃え、システムバー・カットアウト・窓のキャプション・IMEの領域を避ける。スクロール部は `weight(1f)` で見出しを除いた残りの高さに割り当て、末尾に24dpの余白を設ける。長いタイトルは重み付きで折り返し、閉じるボタンを押し出さない。
