...
最終更新: 2026-07-27 / 対象バージョン: 0.8.252-alpha (versionCode 260)
...
#### ルールが起こしたデーモンが即座に殺されていたのを直す（`HeadlessRun` / `PtyProcess.detach`、0.8.251）
...
**原因**: 単発実行の後始末が `PtyProcess.close()` で、これは**ルートプロセス（proot/z2root）へ SIGHUP → SIGKILL を送る**。ランチャは常に `--kill-on-exit`（z2root では `PTRACE_O_EXITKILL`）付きで起動するので、**ルートを kill するとカーネルが配下のプロセスを丸ごと道連れにする**。ルールが `sshd --lan` を呼ぶと dropbear はデーモン化して本当に listen し（ラッパーは pidfile を `kill -0` で確かめてから `listening` を出す）、`sh` が終わった瞬間に PTY が EOF → 後始末 → dropbear ごと消える。ログが「起動できた」で終わっているのは**その時点では本当に起動できていた**ためで、ログを疑うと原因に辿り着けない。常駐サーバー経由が無事なのは、スクリプトが回り続けて PTY が EOF にならず後始末が動かないから（GUI が `while x_running; do sleep 2; done` でルートをブロックさせているのも同じ回避）。

**対策**: EOF は「**前景スクリプトが終わった**」だけで「配下が終わった」ではない、として扱う。後始末を `waitFor()` → `detach()`（**シグナルを送らず fd を閉じるだけ**）に変え、ルートが自分から終わるのを待つ。背景に何も残さない普通のルールでは `waitFor` は即座に返るので挙動は変わらない。明示的に止める `HeadlessRun.stop()` は従来どおり `close()` で丸ごと畳む（人が止めたのだから道連れが正しい）。

⚠ `detach()` を**ルートが生きているうちに呼んではいけない**。マスタ fd を閉じるとカーネルが端末のフォアグラウンドプロセスグループへ SIGHUP を送り、結局ルートごと落ちて同じ道連れになる。
⚠ こうして生き残るデーモンは**アプリのプロセスが生きている間だけ**生きる（proot ルートの子であるため）。恒久的に上げ続けたいものは常駐サーバーに登録する。
