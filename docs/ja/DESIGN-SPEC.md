...
最終更新: 2026-09-18 / 対象バージョン: 0.8.622-alpha (versionCode 630)
...
**囲んだところだけを撮る（`z2-shot`・0.8.622）**: `RegionShot` が担当する。流れは **パネルと取っ手を隠す → 描画の反映を待つ → 画面を1枚取る → その静止画の上を囲む → 指を離して確定 → プレビュー → 保存／共有**。⭐ **囲む操作は必ず取得済みの静止画の上で行う**（生きている画面の上で囲むと、選択中のスクロールや再描画で対象がずれるうえ、囲むための重ね表示そのものが写り込む）。`EdgeRuntime.suspendForActions(true)` でパネルと取っ手を**両方**隠し、`Choreographer` で3フレーム空けてから `AccessibilityService.takeScreenshot` を呼ぶ。⚠ `GONE` にした直後は合成が追いつかず、**消える前のパネルが写る**（固定のミリ秒で待たず、描画の区切りを数える）。完了・取消・失敗・例外のすべてで表示を戻す。囲みは自由・四角・円の3種類で、いずれも `Path` に落として外接矩形で切り出すため、出力処理は1本で共通。透過は `clipPath` ではなく `ALPHA_8` のマスクを `DST_IN` で重ねる（クリップでは円や斜めの縁が階段状になる）。⚠ **座標は画像の画素で持ち、表示のときだけ引き伸ばす** — 画面とオーバーレイの寸法は切り欠き・回転で一致しないことがあり、そのままでは「見えていた位置」と「保存された位置」がずれる。保存は `MediaStore` の `Pictures/z2term` へ PNG で、書き終わるまで `IS_PENDING` を立てる。共有は0.8.608と同じ `shared-files` キャッシュと `FileProvider` を使う。市松模様はプレビューの表示だけで、保存画像には焼き込まない。Android 11以降（`takeScreenshot`）で、ユーザー補助の有効化が前提。バーの自動配色取得と同時に撮ると撮影間隔エラーになるため要求を調停する。
