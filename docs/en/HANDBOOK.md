...
- ⚠ **Only `watch-basic` reacts late.** It is the skeleton for polling a log yourself, so it notices at most one polling interval later (15 s by default); the description says "up to 15 s to notice" for that reason. Shortening the interval burns battery while idling, so if the delay matters, pick the event up with `z2-when` instead (the app does the waiting, so it runs the moment you plug or unplug).
