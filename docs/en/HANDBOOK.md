...
| ∧ / ∨ | **Jump to the start of the previous / next command** (0.8.525). Instead of scrolling a long output by finger, you step back through it one command boundary at a time. ⚠ **The shell has to say "a command starts here"**, so this becomes available once you **open a terminal tab again** after installing this version (opening one installs the hook). It does not work over SSH or in a shell without the hook. ⚠ It stops at the ends (if a tap does nothing, there is no further command boundary that way) |
