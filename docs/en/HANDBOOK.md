...
1. Put the APK file (`z2term-0.8.369-alpha.apk`) on your phone.
...
| `z2-session attach <tab>` | **Stay connected to that tab** (0.8.366). After `z2-session attach 2` you just type into it like any terminal (Ctrl+C works, so do full-screen programs). **Leave with `~.` at the start of a line** (as in ssh; type `~~` for a literal `~` there). ⚠ While attached the tab follows **your** window size — on the phone it wraps wrongly and looks broken, but it goes back when you leave. ⚠ **The tab on the phone stays live**, so what you type shows up there too. ⚠ A notification appears while you are attached (it keeps the app from being killed mid-session) |
