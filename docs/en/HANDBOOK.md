...
| 🔅 | Screen-on lock (when ON, the screen won't auto-dim; the icon changes to 💡 while ON). **Double-tap for a slider that dims this app only** (for dark rooms; going home restores it, and Reset clears it any time) |
...
- **See and stop them from the app** (0.8.227): 📜 → the **Automation** tab lists your rules. Each row has an on/off switch, **▶ to run it once without waiting for the trigger**, **▤ for its run log**, and ✕ to delete. The **Pause automatic runs** switch at the top stops **every** rule from firing (nothing is deleted, and ▶ still works). Below the list, **recent fires** show what ran — and what was held back (`paused`), so a rule that seems dead is easy to explain.
