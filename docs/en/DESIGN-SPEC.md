...
Last updated: 2026-08-31 / Target version: 0.8.454-alpha (versionCode 462)
...
- **RDP is not yet a user-facing feature (0.8.450–0.8.454)**: `gui/rdp/` now covers X.224/TLS, CredSSP/NTLMv2, T.124 GCC and T.125 MCS connection, Client Info, the valid-client licensing path, and Demand/Confirm Active capability exchange without adding a dependency. The opt-in live test completes that entire path against the on-device NLA-enforcing FreeRDP server and receives the first connection-finalization PDU. The client intentionally omits Surface Commands and Bitmap Codecs so the next stage can target classic bitmap updates. ⚠ Bitmap decoding, connection finalization, input and connection UI are not wired, so no `[RDP]` action is shown yet.
