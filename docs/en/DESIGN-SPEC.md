...
Last updated: 2026-06-20 / Target version: 0.8.114-alpha (versionCode 122)
...
- `TerminalEmulator`: processes byte streams with a state machine (Ground/Escape/CSI/OSC…).
  ...
  - DEC modes: alternate screen, cursor keys (DECCKM), **mouse reporting** (X10/Normal/Button/Any × Legacy/SGR/urxvt). While mouse reporting is ON (the TUI enabled `?1000` / `?1006` etc.), swipes inside `TerminalInputView` are also redirected from scrollback control to **wheel events** (button 64 = up / 65 = down) and sent to the PTY via `encodeMouseEvent`. One notch is sent every time the cumulative finger dy crosses `MOUSE_WHEEL_STEP_PX (=40px)`, so a long swipe produces a multi-line scroll (0.8.114; makes tap-scroll = next-page work inside TUIs that opt into SGR mouse reporting. Fling is also disabled while mouse reporting is on, so inertial scroll cannot accidentally page through pages or move the scrollback).
