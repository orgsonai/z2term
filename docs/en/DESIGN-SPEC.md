...
- `TerminalEmulator`: processes byte streams with a state machine (Ground/Escape/CSI/OSC…).
  ...
  - DEC modes: alternate screen, cursor keys (DECCKM), **mouse reporting** (X10/Normal/Button/Any × Legacy/SGR/urxvt). While mouse reporting is ON (the TUI enabled `?1000` / `?1006` etc.), swipes inside `TerminalInputView` are **routed by direction and scrollback position**: **finger going up (= "advance")** is sent as wheel-down (button 65) only when **scrollback is at the bottom (scrollOffset == 0)**. While scrollback is non-zero (the user is looking at past lines), upward swipes are absorbed by scrollback and pull the view back toward the latest line instead of sending a wheel notch (0.8.116; otherwise `TerminalSession.writeBytes` would reset scrollback to 0 on the very first wheel byte and snap the view to the bottom in one go — the "jumps to bottom" complaint). **Finger going down (= "look back")** always falls back to scrollback control (many reader TUIs deliberately ignore wheel-up and "leave it to the terminal scrollback", so the app only needs to forward upward swipes). One notch is sent every time the cumulative finger dy crosses `MOUSE_WHEEL_STEP_PX (=40px)`, so a long swipe produces a multi-line scroll. Flings follow the same gate: no-op only when `mouseEnabled && velocityY < 0 && scrollOffset == 0`, otherwise drive scrollback inertial scroll.
