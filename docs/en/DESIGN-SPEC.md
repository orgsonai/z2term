...
Last updated: 2026-08-31 / Target version: 0.8.453-alpha (versionCode 461)
...
- **RDP is not yet a user-facing feature (the 0.8.450–0.8.453 NLA spike)**: `gui/rdp/` implements X.224 Enhanced Security negotiation, TLS upgrade with an explicit certificate-approval boundary, CredSSP `TSRequest` DER framing, all three NTLMv2 messages with SPNEGO/direct NTLM and MIC, connection-oriented SIGN/SEAL, the v5/v6 public-key binding exchange, and encrypted `TSPasswordCreds`, without adding a dependency. Alongside official crypto vectors and a scripted end-to-end CredSSP peer, an opt-in live test now connects to an on-device, NLA-enforcing FreeRDP shadow server. It exposed a SPNEGO-token interoperability mismatch hidden by the scripted peer; after aligning the exchange with FreeRDP's direct-NTLM path, the real server completes TLS, NTLMv2 authentication, public-key binding and credential submission, then waits for MCS. ⚠ MCS, display updates, input and connection UI are not wired, so no `[RDP]` action is shown yet.
