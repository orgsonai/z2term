...
Last updated: 2026-06-15 / Target version: 0.8.95-alpha (versionCode 103)
...
- **Dynamic block segmentation (learned)** (0.8.71): block boundaries are no longer fixed by dictionary cost alone — they are **learned from how often the user confirms a given reading-block**. Confirmed `(reading → surface)` pairs already live in `ImeHistoryStore` with frequency/recency, and `ImeHistoryStore.learnedBlock(reading)` returns `(top surface, cost reduction)` (wired into `KkcConverter.learnedBlock`). During `KkcConverter.nbest` lattice construction, any reading ≥2 chars that matches a learned block has its whole-span node cost reduced (`BLOCK_BASE_BONUS=3000` + `count`-scaled `BLOCK_COUNT_STEP=1500` (capped at count 4) + recent `BLOCK_RECENT_BONUS=1000`). This overcomes the katakana penalty (4000) + connection costs after just 1–2 confirmations, so a frequently used reading that was mis-split (e.g. こまんど → こ / まんど) **auto-merges into a single block こまんど = コマンド from then on**. Even readings absent from the dictionary get a synthesized node (`lc=rc=0`) using the learned surface, so any "frequently used reading chunk" can be remembered as a block (unlearned readings behave exactly as before — the regression surface is limited to confirmed readings). **The cost reduction applies only to the surface the user actually confirmed** (0.8.74). Previously the bonus was applied uniformly to every surface of the reading, so the block stayed together but the dictionary-cheapest surface still won, and the user's chosen kanji was not reflected (e.g. learning きく → 聴く still kept producing 聞く). Concentrating the bonus on the learned surface lets the user's habitual conversion win even mid-sentence. **Score merged readings at one-word cost and learn consecutively-confirmed runs as merged blocks** (0.8.85): (1) the learned-block synthesized node cost now uses a **one-word `UNK_COST` baseline** (`(UNK_COST - bonus)`) instead of length-scaled unknown kana (`UNK_COST * char count`). Previously an out-of-dictionary loanword (`びるど` → `ビルド`, with `びるど` absent from the lex) cost `UNK_COST×3`, so even with the frequency bonus it could never beat the dictionary split (`びる`+`ど`) and never stuck together no matter how often it was used. With a one-word baseline the "drops as you use it (count↑)" bonus exceeds the split cost and a frequent chunk auto-merges into a single block (`BlockLearningTest` verifies `びるど` converges to one bunsetsu `ビルド` at high frequency, and stays split when unlearned). (2) An auto-split word was only ever learned as individual blocks, so the "whole reading" never entered history; `ComposingState` now accumulates consecutive confirmations within one split run into `committedRun` and records the **merged reading → merged surface** into `ImeHistoryStore` when the run ends (`learnMergedRun`) and on batch confirm (bounded to reading length 2…`MERGE_MAX_READING_LEN`=6 so long sentences don't collapse into one block; the run is cleared when `text` is edited). So a chunk confirmed as split "びる"+"ど" becomes eligible for the (1) bonus next time and sticks together as one block.
- **Whole-sentence batch prediction** (`fullPrediction`): when a tail remains during split, a single "whole-sentence" candidate is shown as a light-green pill, concatenating the **leading block's top candidate (= head of `candidates`) + the Viterbi 1-best of the remaining kana**. Tapping it (`commitFull`) confirms the whole sentence at once. **When `splitHeadLen` moves with ◀▶, it is rebuilt via `refreshPredict`, re-flowing to follow the boundary change** (0.8.16). The remaining-kana Viterbi uses the leading surface form as context for bigram re-ranking. **Batch confirmation learns per block** (0.8.74): when `fullPrediction` is built, the bunsetsu-block `(reading, surface)` breakdown is kept (`fullPredictionBlocks` = leading block + `bunsetsu(tail)`), and `commitFull` learns each block's `(reading → surface)` plus the bigram between adjacent blocks. Previously the whole sentence was learned under one key (`record(text, full)`), which only helped when that exact reading recurred verbatim, so frequent blocks (今日の / 天気は …) were never reused in other sentences (a cause of long-text predictions keeping "combinations you'd never use" on top). If the breakdown is inconsistent with `full`, it falls back to the old single whole-sentence entry. Note: the old "Viterbi 1-best over the whole reading (boundary-independent)" didn't move the light-green text with ◀▶, so it was replaced in 0.8.16. The old "bunsetsu rearrangement variations (`multiSegmentVariants`)" produced mostly unused candidates and were removed in 0.8.4.
