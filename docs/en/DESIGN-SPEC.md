...
Last updated: 2026-09-05 / Target version: 0.8.512-alpha (versionCode 520)
...
- **GUI stack install (`ensure_pkgs`)**: Xvnc, openbox and D-Bus form the GUI base; if Xvnc, openbox and `dbus-daemon` are present it **starts immediately with no network**. **No GUI terminal is installed or launched automatically (0.8.505).** The desktop is an application surface: installed GUI applications launch directly from ☰ / `z2menu`. `install_pkgs` (apk add / apt install / pacman -S) fetches only a missing base after the app-side download-confirm gate takes consent. Only `clean` wipes caches and reinstalls the base.
