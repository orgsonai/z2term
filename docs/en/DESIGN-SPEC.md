...
**Per-tracee rootfs (0.8.416)**: if `Z2ROOT_ROOTFS=<host path>` is present in an `execve`'s envp, that process and its descendants **see that rootfs as "/"**. An empty value restores the default (whatever `-r` set). This is how "enter another environment" (the container equivalent) is done, since nesting is impossible (see 10.1).
