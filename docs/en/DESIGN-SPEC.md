...
Last updated: 2026-09-03 / Target version: 0.8.493-alpha (versionCode 501)
...
- **Audio (rdpsnd, 0.8.481–0.8.492)**: the `rdpsnd` static virtual channel (`RdpSound`) speaks [MS-RDPEA] and plays the peer's audio through the device speaker (`RdpAudioSink`).
  ...
  - ⛔⛔⛔ **Audio requires the `rdpdr` channel to be open (0.8.491, found by diffing against FreeRDP).** The peer **only redirects audio to a client that can redirect devices**; FreeRDP likewise loads `rdpdr` whenever `/sound` is given. ⇒ `gui/rdp/RdpDeviceRedirection.kt` implements just the [MS-RDPEFS] handshake. ⛔⛔ **Zero devices are ever shared** (`ioCode1` = 0, `SpecialTypeDeviceCap` = 0, Device List Announce with `DeviceCount` = 0): no drives, no printers, no smart cards — **this channel exists solely for sound**. ⚠ **Opening the channel is not enough**: the peer waits for the full exchange (Server Announce → Client Announce Reply → Client Name → Server Capability → Client Capability → Device List) before moving on. ⚠ Client ID Confirm arrives either before or after the capability exchange, so **never hard-code the order** (measured).
