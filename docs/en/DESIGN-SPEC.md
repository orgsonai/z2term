...
Last updated: 2026-09-02 / Target version: 0.8.491-alpha (versionCode 499)
...
- **Audio (rdpsnd, 0.8.481–0.8.491)**: the `rdpsnd` static virtual channel (`RdpSound`) speaks [MS-RDPEA] and plays the peer's audio through the device speaker (`RdpAudioSink`).
  ...
  - ⚠ **When there is no sound, make it possible to tell whose side it is on (0.8.487).** Log the first chunk to arrive on the channel and the type of every non-Wave PDU. **A channel that opens but never delivers a single message** means the peer is not redirecting audio at all — nothing on this side can fix that.
