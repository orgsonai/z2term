...
Last updated: 2026-09-15 / Target version: 0.8.602-alpha (versionCode 610)
...
**Initial panel (0.8.601)**: `EdgeStore.ensureInitialPanel` creates `main` only when no panels exist and never replaces definitions. Its fields come from `EdgeDefaultPanel.fields` (`handle=bar side=right offset=31.5 size=8 length=8 alpha=0.2 width=14% height=47% flow=vertical labels=off add=on settings=on`, `scroll-variable` for `actions-up`/`actions-down`, `gesture-speed=40000 gesture-range=400`). Items are resolved only at creation, in order: this app, `ACTION_VIEW https` (BROWSABLE), `STILL_IMAGE_CAMERA`, `ACTION_DIAL`, `SENDTO smsto:` and `ACTION_SETTINGS`. The chosen default is used; without one, a single system candidate is used. Only launchable packages are kept, deduplicated by package. `z2-intent -p PKG` gains `--window freeform` only on freeform-capable devices (`FEATURE_FREEFORM_WINDOW_MANAGEMENT` or `enable_freeform_support=1`). A failed item write removes the new panel. The panel opens immediately after enabling. The `edge-panel` guide runs `z2-edge permission`, `z2-key permission` and `z2-edge on`, then shows a read-only card about long-pressing outside the menu to open settings.
