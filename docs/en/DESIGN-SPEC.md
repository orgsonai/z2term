...
Last updated: 2026-09-04 / Target version: 0.8.499-alpha (versionCode 507)
...
- ⚠ **Qt6 applications cannot save configuration files in this environment (diagnosed in 0.8.498, not yet fixed)**. On Linux, Qt's `QSaveFile` / `QTemporaryFile` create an unnamed file with `O_TMPFILE` and give it a name at the end via `linkat(AT_FDCWD, "/proc/self/fd/<fd>", …, AT_SYMLINK_FOLLOW)`. ⛔ **An Android app process holds no capabilities at all** (`CapEff = 0` in `/proc/self/status`), so that `linkat` always fails with `ENOENT` (the `AT_EMPTY_PATH` form gives `EACCES`). Configuration, caches and the application database are all unwritable, and **a file manager that resolves "open with" through the KDE database therefore does nothing on a double-tap**.
  - Measured on the device (Qt 6.11.2): `kwriteconfig6` **returns exit code 0 while creating no file at all**; `kbuildsycoca6` likewise finishes silently and leaves no cache. ⚠ **Opening with `O_TMPFILE` succeeds**, so this does not look like a permission problem (`touch` and `rename` both work in the same directory).
  - ⚠ **GLib-based tools are unaffected**: `update-desktop-database` writes `mimeinfo.cache` normally. When this symptom appears, first work out **which of the two stacks** the application belongs to.
  - The next step is an `LD_PRELOAD` shim (built like `z2accept`: raw syscalls, no libc dependency) that strips `O_TMPFILE` from `openat` so Qt falls back to named temporary files. ⚠ **Whether that fallback actually engages is unverified** — confirm it with a small reproducer before writing the shim.
