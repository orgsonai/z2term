...
Last updated: 2026-07-28 / Target version: 0.8.263-alpha (versionCode 271)
...
**Rules are plain text**: `~/.z2term/when/<id>.rule` (`filesDir/shared_home/.z2term/when/`), three lines `trigger=` / `run=` / `enabled=` (`settings/WhenRule.kt`; optionally `order=` and the 0.8.263 filters `if=` / `cooldown=` / `between=` / `days=`, see below). Plain files (not DataStore) so **git sync and backups work** (same idea as the resident-server job files). The CLI (`z2-when`) reads/writes them directly and calls `z2api when-reload` to re-arm time triggers.
...
- **`z2-tile` (0.8.258 — quick-settings tiles)**: put a macro or command on up to four **quick-settings** tiles. `z2-tile set <1-4> <macro.sh|command...> [-l label]` / `list` / `clear <1-4|all>`.
  ...
  - **Unassigned slots stay out of the quick-settings list (0.8.260).** Four permanent entries fill up the edit screen of someone who never uses tiles (reported on-device). Slots cannot be **added** at runtime, but `PackageManager.setComponentEnabledSetting` can **disable them individually**, so the shrinking direction is available at runtime ([`TileStore.syncEnabledTiles`], kept in step by `z2-tile set` / `clear` and at startup). ⚠ **Slot 1 is shown even when unassigned** — hiding all four removes every place outside the app where the feature can be discovered. ⚠ If a disabled slot was already on the panel, **the OS drops it from the panel too**, and reassigning does not bring it back, so `z2-tile clear` also means "put one tile away".
  ...
  - **Four slots, fixed.** Each tile needs its own `TileService` in the manifest and **cannot be added at runtime** (Android's rule). Matching the D1 widget's button count keeps it one thing to remember. The implementation is four classes that differ only by slot number (`Z2Tile1`–`Z2Tile4`).
  ...
  - **You do the placing.** An app is not allowed to drop its own tiles into the panel. On Android 13+, `StatusBarManager.requestAddTileService` can ask on your behalf, so Settings › **Quick-settings tiles** offers that button — only where the API exists (no buttons that cannot do anything).
  - **Assignment is not editable from the settings screen.** `z2-tile` in the terminal is the source of truth, the same way `~/.z2term/macros/` is for macros. Two places to change it means never knowing which one is in effect; the settings screen only **shows** the current assignment and **asks** for placement.
...
**Guarding against stale build artifacts (0.8.48)**: the z2root/z2accept `.so` files are build artifacts (not in git) and are regenerated by neither `git pull` nor CMake, so fixing `z2root.c` can still ship an **old `.so` inside the APK**. The Gradle task `buildZ2rootNative` runs `scripts/build-z2root.sh` before the jniLibs merge of the `full` flavor, so `./gradlew assembleFull*` alone always regenerates from current sources (zero manual steps). `build-z2root.sh` resolves the NDK path by itself (env vars / `sdk.dir`+`ndk.version` in `local.properties` / `$ANDROID_HOME`). `foss` downloads at runtime and is out of scope.
