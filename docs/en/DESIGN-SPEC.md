...
Last updated: 2026-09-18 / Target version: 0.8.622-alpha (versionCode 630)

**0.8.622-alpha (versionCode 630) — build unverified**: New `z2-shot` captures only the part you draw around. Called from an edge panel button, it hides the panel and its handle, takes one screen image, and lets you draw on that still picture. Free, rectangle and oval shapes are available, and lifting your finger confirms the outline (Redo draws it again). You then choose whether the outside is transparent or keeps the original background, and Save (to Pictures/z2term) or Share. The panel, its handle and the selection UI never appear in the saved image. Needs Android 11 or later and the z2term Android actions permission.
...
`QrHistory` (pure functions) and `QrHistoryStore` (one JSON-array key in the `z2term_qr_history` DataStore) hold the history. Only `read` in `QrToolsActivity` records: camera, image, choosing among several results and content shared from other apps. Showing an entry again (`show`) and Show QR do not record. Rereading moves an entry to the top and keeps its pin; up to 50 unpinned entries are kept, newest first. Pinned entries are listed first, then the rest, newest first. Long-press opens a `DropdownMenu` to pin or delete; Clear all confirms through `ConfirmDialog`, then removes unpinned entries. Nothing is composed while the app is locked. See 4.14 for the sample edge panel, its guide and `ActionUi` for action automation.
...
`QrAction` classifies scanned content with pure parsing. http/https without user info, `//` schemes and selected opaque schemes such as `geo:` use `ACTION_VIEW` with BROWSABLE, so App Links reach their app. `tel:` uses `ACTION_DIAL`; `mailto:` and MATMSG compose mail; `sms:`/`smsto:` and SMSTO compose SMS. WIFI uses `ACTION_WIFI_ADD_NETWORKS` on Android 11+ for WPA/WPA2, WPA3 and open networks; otherwise the Wi-Fi panel opens and the password is copied as sensitive. vCard and MECARD add contacts; VEVENT adds calendar events (UTC, TZID, floating and all-day times). `javascript:`, `file:`, `content:`, `data:`, `intent:`, `z2term:`, `ssh:` and unknown `word:value` text without `//` are excluded. `QrActionLauncher` starts an activity only when the button is pressed and reports when no app can open it. `z2-qr` sends `z2api 0 qr` to `QrToolsActivity.scan`, which requests camera permission when needed and does not repeat after rotation. The 0.8.601 `QrEntryButton` is removed; Settings uses `SettingsGroup.QR_TOOLS`.
...
`QrEntryButton` sits in the Snippets and Connections header rows, never in the drag-handle row. QR tools and relay sharing remain separate activities built from `PillButton` (now with a disabled state), `Field` (now with a keyboard type), `HintBox` and the `ToolScreenParts` header, note and progress components. See 4.14 for the initial panel and guide.
...
See [QR tools and self-hosted relay setup](QR-TOOLS.md). Separate relay preferences store only the SSH profile ID, HTTPS origin and remote port, with no default destination. SshSessionFactory reuses saved credentials and jump hosts without applying init commands, ordinary port forwards or resident tunnels. Every hop strictly checks existing known_hosts; unknown and changed keys fail without background approval. Verify the profile from the SSH tab first.

DirectShareService snapshots the selection and binds HTTP to an ephemeral port on 127.0.0.1. ShareRelay requests SSH remote forwarding from the configured server-side 127.0.0.1 port to that listener. The user supplies the public HTTPS reverse proxy. A matching token on its health route is required before showing the QR and starting the 5/15/60-minute lifetime. Connection setup is bounded to about two minutes, without automatic reconnection or republication. Checks run every 15 seconds; a failure hides the URL, and four consecutive failures or SSH disconnection stop sharing. Stop and cancellation close all SSH hops, forwarding and listeners belonging to that share. Other servers remain unaffected.

DirectFileServer uses a 256-bit share token, per-file consent POST and HttpOnly/Secure cookies, byte ranges, and bounded connections/headers. Referrer-Policy: same-origin preserves valid browser form submissions while null and invalid origins remain rejected. Folder selection passes only its OpenDocumentTree read grant, without persisting it. Limits are 1 GiB total, 10,000 entries including the root and 64 levels below it. Snapshot failures, cycles and exceeded limits abort before publication. Routes cannot escape the selection, and one file’s consent cookie cannot authorize another file. Stop, expiry or a default-network change closes connections and deletes the snapshot. QR intake never executes commands; imports create new profiles, snippets or hidden edge panels.

External QR input enters through a separate exported QrReceiveActivity. QrIncoming validates it before the QrToolsActivity review screen handles it. Limits are 8 images and 32 results/32,768 total characters; images accept content URIs only. Existing SharedIntake and share automation are not invoked. Processing waits for app unlock; rotation restores handled state/results, while onNewIntent replaces the prior input. Six receiver tests compile; their device execution remains unverified.
...
**Verification (0.8.595)**: All 77 edge-related unit tests passed with no skips. Debug, signed release and instrumentation APK builds passed; lint reported zero errors. Release and debug were updated on the device. One device test passed through the Android shell and real z2-edge API, covering quoted fields/items, parent and child recreation, and preservation of existing parent settings, other tabs and enabled state. Visual layout, scrolling and copying in Manage have not been manually verified on the device.

**0.8.594-alpha (versionCode 602)**: The Android system shell now starts without a Linux installation. Android `z2-*` helpers, automation, edge panels, tiles and widget commands can run using the shared home. `z2help` lists the supported scope. Existing Linux execution paths are preserved; Linux packages and the audio/image helpers still require Linux.

The selected OS can also be deleted after confirmation in Delete OS data. Its terminal, GUI and background processes stop before removing its rootfs, OS-specific home directories and downloaded images. Shared home and other OS data are kept. Affected tabs move to a remaining OS, or to the Android shell after the last OS is removed. Deletion is refused while startup or installation is in progress; active root chroot processes or mounts must be stopped and unmounted first. Failures appear in Settings and the diagnostic log.

**Verification (0.8.594)**: 1,130 of 1,132 unit tests passed; two existing RDP tests were skipped (external server/fixture required). This includes nine deletion-specific unit tests. Debug and signed release APK builds, lint (zero errors) and instrumentation APK build passed. Release and debug 0.8.594 were installed on a device and both versions were confirmed; the release app data directories were retained. All three additional device tests passed: Android-shell command invocation, deleting a selected synthetic OS while preserving shared home, and concurrent exit checking during shutdown while keeping another terminal alive. Automatic UI startup with no OS, panel display, permission interactions, event firing and deletion with an active Linux GUI or root chroot remain unverified.
...
**Edge note improvements (0.8.582)**: Blank labels omit the heading. Notes add optional ruled lines, bottom icons for undo/redo and text size, and bounded history retained across saves and restarts. Notification logging uses the public SDK user-ID accessor to fix an existing compilation error. Release build, lint and 48 targeted tests pass.
...
The enum becomes PERMISSIONS while retaining the automation accordion storage ID. Server, rule, detection and log storage formats and CLI behavior remain unchanged. The common permission UI distinguishes Android 10 runtime storage access from Android 11+ all-files access and requests notification posting on Android 13+. USB requests reuse UsbHostAccess. XML, string references, diffs, release build and lint checked. Full management and permission flows on a device remain unverified.
...
**Low-power setting entry (0.8.581)**: Consolidated in Command list → Servers. The description covers both resident servers and terminal keep-alive. The servers_low_power DataStore key and TerminalService.start re-evaluation stay unchanged. Servers apply it on their next start; with neither service resident, changing it changes no held lock. Battery exemption and phantom-process guidance are in Settings → Permissions and notifications → Permissions.
...
With notification access, capture and logging enabled, Android notification posts and updates are saved as JSONL (ts / time / pkg / app / title / text / category / key). Sound or popup visibility is not a filter. New messages delivered in one batch are joined with newlines. Notification triggers remain available with logging disabled.
...
**Receipt diagnostics (0.8.588)**: With capture enabled and the listener connected, `z2-noti trace start [package]` records every delivered update before deduplication: extras/ticker types and UTF-16 lengths, array counts and selected-body lengths. Notification/group keys are hashed. Text, title and sender-name values are not retained; no file is written automatically. Collection stops after five minutes; the latest 128 events remain in memory with an overflow count. `trace dump` returns JSON while collection continues; `trace stop` stops and returns it. Restarting clears results; process exit loses them. Each extras traversal is limited to depth 4 and 256 nodes, with 48 Bundle fields and 32 array entries. Compare original and emitted counts before interpreting omitted fields. Extraction failures also retain field metadata. Body selection and deduplication are unchanged. This does not recover text shortened before delivery or notifications never posted. Build unverified; added device tests not run.
...
- ⛔⛔ **Never let this shim reference a symbol the target libc lacks (fixed in 0.8.508).** `LD_PRELOAD` applies to **every process started in that rootfs**, from the terminal's `sh` to `apk`, so a single unresolvable symbol kills **every command before it execs**. From 0.8.425 through 0.8.507 `libz2usb.so` used bionic's `CMSG_NXTHDR()`, which is **not a macro but an expansion into the libc function `__cmsg_nxthdr()`**. musl has no such function, so on **Alpine neither a terminal nor the GUI would start at all** (`Error relocating /usr/local/lib/libz2usb.so: __cmsg_nxthdr: symbol not found`). glibc distros (Ubuntu / Arch / Kali) resolve it fine, so **testing only on those cannot reveal it**. The next cmsg is now walked by hand in `z2_cmsg_next()`. ⭐ After touching a shim, read `nm -D --undefined-only` and keep the list to `weak` entries and standard symbols every libc exports (`memcpy` and friends).
...
- `share:any` / `share:text` / `share:file` / `share:contains=<part>` / `share:ext=<ext>` (0.8.266; extended in 0.8.608) — receiving content from another app, independent of detection. The share activity comes to the foreground. Rules are evaluated once after import, independently of manual action selection or terminal insertion. Mixed receipts match text and file; contains checks the body, ext checks filenames. Variables: `Z2_WHEN_SHARE` (original insertion), `Z2_WHEN_SHARE_KIND` (text/file/mixed), `Z2_WHEN_SHARE_TEXT` (body), and `Z2_WHEN_SHARE_MANIFEST` (relative to HOME). See §5.1.2 and the [workflow guide](SHARE-WORKFLOW.md).
...
- ⛔ **A dropdown is not just text (0.8.522)**: while the key and operator anchors were drawn as "text + ▾", the user reported that **it reads as plain white text with no sign it opens anything, and the triangle is too small to notice**. ⭐ **One look for everything tappable** — the anchor now carries the same 1dp border, background and 6dp corners as `PillButton`, and the only thing separating it from a button is **the ▾ at its right edge** (`DropdownAnchor`). ⚠ **The ▾ has to be larger than the text** (a 12sp ▾ inside 12sp text reads as part of the sentence, not as a marker). The touch target grew from the text's own height to 8dp vertical / 10dp horizontal padding.
...
**Automation tab (0.8.581)**: Command list → Automation switches between Action automation and Automation rules. From 0.8.586, ActionMacrosBody embeds the manager inside the tab for creation, editing, running, stopping and history. It shares ActionMacrosEditor with the standalone activity, retaining the same storage, conflict checks and CLI. A rememberSaveable Bundle retains the draft, step being edited and editor scroll position across recreation and app lock. Coordinate and UI-element requests retain the originating activity as their return destination. Swipe dismissal is blocked while editing; Close, Back and tab changes check for unsaved changes. Automation rules uses WhenRulesBody for listing, editing, toggling, logs, trying, deleting and pausing rules. Duplicate settings entries are removed. Top tabs stay fixed; from 0.8.583 they use equal-width rounded buttons, with long labels ellipsized. Tool visibility and tab selection use rememberSaveable to survive returning.
...
- **One drag event swaps as many items as it crossed (0.8.510)**. The check was a single `if`, so moving with any speed left **the finger ahead of the order** — "it will not move in one go, it only steps one at a time" (obvious in the settings toolbar with its 10 chips). Drag events arrive coarser than the finger moves, so one event often carries more than one item of travel. `swapWhilePossible` keeps swapping while the offset is still past a neighbour. ⚠ Items measured at size 0 are skipped (`pitch <= spacingPx`), or the loop never ends.
- **Auto-scroll while held against an edge (0.8.510, `rememberReorderState(scrollState = …)`)**. A finger cannot leave the screen, so without it **nothing can be carried past what is visible** (the settings toolbar shows about 6 of its 10 chips). When the held item's apparent centre comes within one item's width of an edge, the container scrolls that way by 1/5 of an item per frame. ⚠ **Add whatever actually scrolled to the drag offset.** The finger is still while the content moves, so without it the held item is left behind; with it the item stays under the finger while the row slides beneath — which is exactly "the order keeps advancing". Positions come from `onPlaced`'s `positionInParent()` (the **layout** position, without the `graphicsLayer` translation, so the apparent position is that plus the offset). Pass no `scrollState` and nothing auto-scrolls, as before.
...
- **A tap runs it (0.8.314, changed at the user's call.)** Through 0.8.313 a tap only put the command on the input line and the user pressed return. But a card can be tapped **mid-typing**, so pressing one after typing `ls -l` produced a line like `ls -lz2-macro install remind`, and return then ran something nobody meant. Now **`Ctrl-C` (0x03) throws the line away first**, then the command is pasted with a final Enter (`runGuideCommand`). Bracketed paste is honored; Enter follows the closing marker in the same write to preserve ordering. ⚠ Do not write the command in the same burst as `Ctrl-C`: the tty **flushes the input queue** the moment it sees INTR (the default with `ISIG` and without `NOFLSH`), so the command would be flushed along with it. Leave 150ms.
...
- ⚠ **The card list scrolls, and a swipe is not a tap** (0.8.620, user report: the cards could not be scrolled and got sent instead). The cards take at most 40% of the screen height; beyond that only the list scrolls, while the heading ✕ and footnote stay put. Cards showing long commands used to push the terminal off screen with no way to see the rest, and `clickable` only cancels a tap when the finger leaves its bounds, so a vertical swipe on a card sent its command on release. `GuideCardRow` no longer counts a gesture that moved past touch slop as a tap (it only observes on the Initial pass and consumes nothing, so the outer scroll still works).
...
**Start the Android system shell without Linux (0.8.594)**:

- `TerminalSession.startupPlan()` returns `NeedOsInstall` for an empty installation; the UI calls `startTerminal()` to start Android sh without downloading Linux. Restart and `z2-session new` follow the same rule. Explicit selection in Settings still installs the chosen OS.
- `AndroidShellEnvironment` stores readable CLI scripts and PATH symlinks under `filesDir/android-shell/`. The links target the APK-bundled `libz2android.so`, which invokes `/system/bin/sh` to read each script under Android's executable-file policy. See [Android 10 execution policy](https://developer.android.com/about/versions/10/behavior-changes-10#execute-permission). Helpers reuse `z2ApiScripts`, changing only bridge, helper and temporary paths.
- `HOME` is `filesDir/shared_home`, seen as `/root` in Linux. Rules, macros, panels, action definitions and tile assignments keep their formats and storage. Use `$HOME` or `~/` in portable scripts and run shell macros with `sh "$HOME/.z2term/macros/name.sh"`. Android sh does not source Linux profiles.
- `HeadlessRun` selects Android sh **only when no distro is installed**. Existing installations keep the selected Linux execution path; a missing selected distro or failed Linux launch is not silently replaced. `EdgeRunner` writes output, errors and exit status relative to the launched shell's `$HOME`.
- `z2help` lists available helpers: notifications, clipboard, battery/state, torch, volume, Android actions, `z2-when`, `z2-edge`, `z2-action` and others. `z2-audio` and `z2-img` fail with an explicit Linux requirement. Linux packages, GUI, Linux servers and `z2-macro` sample installation require Linux. Android permissions, Accessibility, event capture and background limits are unchanged.
- The no-OS notice explains Android sh and `z2help` and retains the Linux installation entry. Starting Android sh does not dismiss it; installing an OS does. The pinned Settings notice opens and scrolls to the Linux section. Existing introductory cards include Linux samples and remain gated on OS installation.
- Generated helpers are syntax-checked in all six languages. This also fixes missing Spanish margin trimming and Korean help comment continuation found by those checks.
...
- **The paste field stays.** Create or paste, two choices, no mode switch (anyone who can produce a PEM keeps their path). The private-key body is masked by default and has an explicit Show/Hide control (0.8.457); opening the editor must not put a full PEM on screen.
...
- **Format 2 encrypts everything except the manifest (0.8.449).** Through 0.8.448 only `ssh.enc` was wrapped; settings, snippets, macros, rules and IME learning history remained plaintext inside the ZIP. Tokens, passphrases and typed text can live in any of them, so format 2 removes the need to maintain a list of which files are “secret”. Its outer ZIP has exactly two entries: `manifest.json`, left readable so item counts can be previewed without a passphrase, and `payload.enc`, an AES-GCM-wrapped inner ZIP containing everything else. Format 1 import compatibility, including its old `ssh.enc`, is retained.
- **Encryption is decided by the passphrase alone (0.8.452).** Through 0.8.451 `includeSecrets` doubled as the encryption switch, so a backup that excluded secrets — which is every scheduled one — could not carry a passphrase and was always plaintext format 1. **Carrying no secrets and not being readable are different things**: snippets, macros, automation rules and what the keyboard learned go in exactly as the user wrote them. A non-empty `Options.passphrase` now selects format 2; `includeSecrets` only means "also include the SSH secrets" and still requires a passphrase. The manifest reports `hasSecrets` and `encrypted` separately.
...
- ⚠ **Nothing written automatically carries secrets.** Automating that means **keeping the passphrase on the device**, which would leave the promise above — no path that writes secrets without a passphrase — meaningless once the device itself is taken. Create one by hand to take secrets along.
- **The contents can still be encrypted (0.8.452).** Settings holds a passphrase for the schedule; when set, the file is written as format 2 while still carrying no secrets. **"No SSH secret inside, so plaintext is fine" does not hold** — the snippets, macros, automation rules and learned text that do go in are the user's own writing. Left empty it stays plaintext format 1, so **doing nothing changes nothing**. The stored passphrase is masked by default and can be revealed only with Show (0.8.457); the manual export/import passphrase fields follow the same rule.
- ⚠ **The passphrase is wrapped by `KeystoreCrypt` in DataStore and never exported** (`AppSettings.EXPORT_EXCLUDE`). Exporting it would put **the passphrase that opens the file inside the file** (readable outright in a plaintext format 1 backup). Import leaves it alone too: overwriting it with another device's passphrase would start piling up files this device cannot open. ⚠ The trade-off is that restoring settings does not carry it over, so it has to be entered again on the new device.
...
**What it does**: a destination can sit behind **any number of jump hosts** (the same shape as `ssh -J a,b,c`). Each hop is dialled in turn and the next connection leaves from inside the last one. **The shell, SFTP, every FTP / SMB / WebDAV / VNC / RDP service hanging off that destination, and resident tunnels all travel the same route** — there is only one entry point (`SshSessionFactory`), so support only had to be added once.

**⭐ Do not relay through `-L`**: the obvious approach — install `-L` on the previous session and point the next SSH at `127.0.0.1:<free port>` — walks into two problems at once.

1. ⛔ **known_hosts gets recorded under `127.0.0.1`.** This UI always asks the user to confirm a host key, so **every jump host's far side ends up under the same name** and a swapped key can no longer be told apart.
2. Any other app on the phone can reach that listening port too.

⇒ Implement JSch's `Proxy` instead (`JumpProxy`): open one `direct-tcpip` channel on the previous session and speak the next SSH inside it. That is OpenSSH's `ProxyJump` shape, and it **opens no listening port on the phone**. The `Session` keeps the real hostname, so known_hosts and the key-confirmation dialog both name the right peer at every hop.

**⚠ Register keys on a separate `JSch` per hop**: JSch identities are **shared across the whole instance and tried on every session**. Put them in one and the jump host's key gets offered to the real destination, which answers `Too many authentication failures`. Only the `hostKeyRepository` is shared (there is one known_hosts).

**⚠ `getSocket()` is null**: JSch copes without a real socket (`connect`'s `setSoTimeout` and `setTimeout` both null-check — verified in the 0.2.26 bytecode). But **a hop without a socket has no read timeout**, so its keepalive never fires. ⇒ `SshLink.enableKeepAlive` installs the interval on **every** hop, and the first one — the only one holding a real socket — is what notices the route dying (once it drops, everything behind it follows, so reconnection still works).

**⭐ Hops hold values, not references**: an `SshHop` carries its own destination and credentials. "Point at a saved connection" was rejected because **deleting the target breaks the jump** and because A→B→A **cycles become expressible**. Instead the editor's "Import" **copies** a saved connection's fields in (`SshProfilesSheet.toJumpHost`). ⚠ Exporting without secrets **also strips the hops' secrets** (`SshProfileStore.exportRaw`) — forget that and the jump host's password is the one secret left in the file.

**⚠ The data cap is judged on the first hop**: the machine the phone actually dials over the air is hop 1; the real destination is resolved inside the tunnel, on the other side's link. `NetGuard.ensureAllowed` therefore gets `jumpHosts.first()` (or the destination itself when there are no hops).

**⚠ Resident tunnels check known_hosts for every hop**: one unapproved hop is enough to block on a key dialog that cannot be shown. `TunnelManager.isKnownHost` only starts a tunnel when **all** hops and the destination are already known.

**Forward direction was being dropped (0.8.494)**: `SshChannel` installed `-L` **unconditionally**, ignoring `PortForward.reverse` — opening a destination with a `-R` forward from the SSH tab silently reversed it. Only `TunnelManager` branched correctly, and having two places that install forwards was the root cause. ⇒ Both now go through `channel/PortForwarding`.
...
**What 0.8.509 fixed (the order changing every time you switch tabs, and the one-frame reshuffle)**:
- ⭐ **Stop dropping buttons the current tab does not have.** `toolbarOrder` is **one value shared by terminal and GUI tabs**, yet the write path based itself on `mergeOrder(saved, allIds)` (= only the buttons of the tab you are on), so a single reorder in a terminal tab erased the stored places of ☰ / 🖱 / 📎 (and reordering in a GUI tab erased 🔍 / ⚪). The erased side gets appended in default order the next time it is shown, which is what made **the icons rearrange every time you moved between tabs**. The base is now "the stored value plus ids it does not know yet", and only the slots that belong to `allIds` get refilled (`ToolbarOrderTest.otherTabButtonsSurviveAReorder` / `.reorderingOnOneTabDoesNotShuffleTheOther`).
- ⭐ **The order is decided during composition** (`ReorderableToolbar`). It used to fill an empty `mutableStateListOf` from a `LaunchedEffect`, so the **first frame after a tab switch had zero buttons** and the next frame had them all — the "icons shuffle for a moment" seen on every CLI → GUI switch.
- ⭐ **Grabbing, swapping and holding your own order until the save comes back now lives only in `ReorderList`'s `ReorderState`.** To reuse the vertical-list implementation it gained `vertical = false` (same test, measured on width with `translationX`) and `reorderLongPressHandle` for items that have no room for a handle. ⛔ **Do not write that behaviour a second time in the toolbar** — bugs like "it snaps back and then swaps again" always return if only one copy is fixed.
- ⭐ **Settings › Display › Toolbar can reorder too** (requested). Long-press a chip and drag left/right (`ToolbarVisibilityRow`). Both terminal-only and GUI-only buttons are listed there, so the default order can be set without opening either tab. ⚙ stays pinned at the end, as it is on the toolbar itself.

**What 0.8.510 fixed (the reshuffle 0.8.509 did not remove)**:
- ⭐⭐ **The real cause was the initial value of the GUI tab's settings subscription.** A GUI tab has no `TerminalSession`, so it subscribes to `AppSettings.flow` (the DataStore flow) directly — and it started from `collectAsState(initial = AppSettings.Snapshot())`, i.e. **the defaults**. The first emission from DataStore is several frames away, so **right after switching to a GUI tab the bar was drawn with `toolbarOrder = ""`** and snapped to the stored order the moment the real value arrived. Both 0.8.509 fixes assume a correct stored value has already arrived, so the flicker survived them. The initial value is now `AppSettings.lastKnown` (the last `Snapshot` actually read, kept in the companion), so the first frame already has real values. ⚠ **Never write through it** — DataStore is the source of truth and this is only a copy. It is the defaults only until the first successful read after launch.
- ⚠ **The same shape of flicker can hit any screen that subscribes to DataStore directly.** It shares its root with the `stateIn(Eagerly)` default-`Snapshot` trap (the distro decision in 0.8.314). **Never paint a value you have not read yet with the defaults.**

**What 0.8.510 fixed (reordering moved one item at a time)**: see the `ReorderList` notes.
...
- **Shell configuration follows the OS (0.8.502)**: the app-specific Login shell and Shell prompt settings were removed. Terminal tabs and GUI terminals now use root's shell from the rootfs `/etc/passwd`; the app no longer rewrites `/etc/passwd` or `/etc/shells`. Prompts are managed directly in files such as `~/.ashrc`, `~/.bashrc`, or `~/.zshrc`. Prompt blocks written by older releases are deliberately not removed automatically, so an existing user environment is not changed without consent.
...
- **`z2-noti` command (0.8.236)**: prints the notifications currently on screen as TSV (key / package / app name / title / body). The trace subcommand (0.8.588) temporarily records received field types and lengths. Notification detection already existed, but it could only record; there was no way to ask "what is on screen right now" from the shell.
...
- **`z2-tile` (0.8.258 — quick-settings tiles)**: put a macro or command on up to twelve **quick-settings** tiles. `z2-tile set <1-12> <macro.sh|command...> [-l label]` / `list` / `clear <1-12|all>`.
  ...
  - **When on and off are separate commands, `--off` puts both on one tile (0.8.261).** `z2-tile set 3 z2-torch on --off z2-torch off`. Tapping alternates between them and the tile **looks on while it is on**. A slot holding only `z2-torch on` runs `on` on every tap — **the tile can never turn it off** — so the off side became writable (the user's proposal).
    ...
    - Ordinary command pairs remember toggle state. Since 0.8.591, direct `z2-torch` assignments use Android observations, including external changes. `z2-screen keepon` reads its actual active setting.
...
- **`z2-icon` (0.8.294 — draw the status-bar and tile icons yourself)**: `z2-icon pick <target>` / `sample [name|target name]` / `edit <target>` / `set <target> [file|-]` / `show <target>` / `clear <target|all>` / `grid [24|48|64]` / `scale <target> <24|48|64>` / `list`. The targets are **`notify`** (one drawing, used by every notification this app puts out) and **slots 1-12** (one drawing each).
  ...
  - **Bell and heart contours (0.8.590)**: Redraw the 64-cell originals with continuous curves from the rounded tops to the bell rim or heart tip, preserving left/right symmetry. Assigned icons exactly matching the old stock artwork are read as the new drawings for display and editing, including the 24/48/64-cell resized forms. Their configured grid size is retained. Edited drawings and other samples do not match and are excluded. The stored data is not rewritten; the updated app resolves the corrected appearance when reading it.
...
**Per-tracee rootfs (0.8.416; path handling since 0.8.417)**: if `Z2ROOT_ROOTFS=<path>` is present in an `execve`'s envp, that process and its descendants **see that rootfs as "/"**. An empty value restores the default (whatever `-r` set). This is how "enter another environment" (the container equivalent) is done, since nesting is impossible (see 10.1). ⭐ **Write the value as the path you can see from the environment you are in.** A process inside the guest has no way to know its own host path (HOME is not under the rootfs but bind-mounted from `shared_home`, so no mechanical concatenation with the rootfs yields it), so z2root converts it through `host_path_for` first. ⚠ **Writing a host path directly is not supported**: one that already sits under the current rootfs is passed through (no conversion needed), but anything outside it — a bind target, for instance — is converted as a guest path and will not resolve. ⚠ 0.8.416 accepted host paths only, which is why `z2c run` failed with `No such file or directory`.

- Implementation: `struct pid_state` carries a `rootfs_idx` (an index into the `g_rootfs_tab` table — storing the string inline would add `PATH_MAX_Z` × `MAP_CAP` ≈ 1MB), and `rootfs_select(pid)` picks the effective rootfs at the top of the `waitpid` loop. Path translation is funnelled through just three functions (`translate_abs` / `host_to_guest` / `host_path_for`), so having those consult `EFF_ROOTFS()` covers every syscall while `struct config` stays a single instance (no need to touch its 157 use sites).
- Inheritance: `fork`/`clone` copy it from the parent (a child forked inside a container stays in that container). It only changes where an `execve` envp says so.
- Ordering: applied right after `record_exec_argv`, i.e. **before** `plan_exec`, because the program being exec'd must itself be resolved against the new rootfs.
- Binds (`-b`) stay in effect across a switch, which is what you want: `/dev`, `/proc` and `/sys` are needed in the other environment too.
- Only the **filesystem** is separated. Without namespaces the process table and the network are shared, and being ptrace-based a tracee can work around it. This is **environment separation, not security isolation**.
- The limit is `MAX_ROOTFS` = 16 environments.
- The caller side is `scripts/z2c run <image>` (chapter 9).
...
`libz2accept.so` is generated by `scripts/build-z2root.sh` and gitignored. `ProotLauncher` places it at `/usr/local/lib/libz2accept.so` in the rootfs and injects `LD_PRELOAD` (a load failure is non-fatal — ld.so warns and moves on). `__errno_location` is referenced as `__attribute__((weak))` with a NULL guard, so bionic binaries (aapt2 etc.) still start even if LD_PRELOAD leaks to them (0.8.55). ⚠ **Leaking into a process of another architecture is fatal**: this `.so` matches the rootfs ABI (aarch64), so running an x86_64 binary through an emulator makes ld stop with `Error relocating …: unsupported`. Unlike the bionic leak this cannot be rescued by weakening a symbol (it is a machine-code mismatch, not a symbol one), so any path that launches another architecture must **clear it explicitly with `env -u LD_PRELOAD`** (as `scripts/z2c` does).
...
- Placement advances the cursor by the image's cell width (line breaks are expected from the TUI as `\n`). This is what kitty's `C=1` (cursor movement policy) asks for; `C=` is not read — the behaviour is always this
...
**Chunked transmission was broken (0.8.496)**: `TerminalEmulator` called `KittyGraphicsParser.reset()` at the start of every APC, so each continuation chunk (`m=1`) **wiped the accumulated payload and header**. Chunks arrive as separate APC sequences, so clearing everything on each start leaves only the final fragment. `a=` was lost too, defaulting to `T`, and the fragment cannot be decoded as PNG — with `q=2` it is dropped silently. In other words, **no image larger than a single APC (4096 bytes) ever appeared**. ⚠ A QR from `qr.sh` fits in one chunk and worked, which is why this looked fine. ⇒ Added `beginSequence()`, which clears only the body buffer, and call that at sequence start; `reset()` stays for discarding the accumulation. ⚠ **The parser's own tests passed through the join** (they never call `reset()` in between) — the caller was the broken part, so `KittyGraphicsParserTest.beginSequenceKeepsHeaderAndPayloadAcrossChunks` pins the regression **in the emulator's own order** (`beginSequence()` on every start).
...
- ⭐ **0.8.519 adds "reinstall the GUI with `z2gui clean`".** Since 0.8.507 took the clean-install
  switch out of Settings (no destructive reinstallation lives in the settings page), the recovery
  path is **a terminal command only**. ⚠ **The command appears nowhere on screen**, which is the
  same reason the other tips are here (to anyone not told, it does not exist). The body states that
  it clears the package cache and puts the GUI stack back **without touching the user's files**, and
  mentions `z2gui start clean` for reinstalling and launching in one go.
...
- `PARTIAL_WAKE_LOCK`, notification (`ic_notification` = a transparent-mask **`>_`**, tap to return / stop action). **A small icon cannot use color** (every opaque pixel is tinted the same), so shape is the only thing that distinguishes it. The old two-character "Z2" collapsed into an unreadable blob at the ~24px the status bar gives it; 0.8.196 switched it to the launcher's `>_`, which then shared its silhouette with **other major terminal apps**, so 0.8.200 moved the notification alone to a **single Z**. **0.8.511 put `>_` back (requested)** — being recognisable as *this* app at a glance was judged more useful than being distinct from other terminals, and it now matches the launcher icon. ⚠ **Do not draw the chevron as a filled path**; its tip starves and disappears at this size. Use **strokes** (width 3.2, round caps), keep it to two elements, and leave ~4 of padding so the OS mask does not eat it. The SAF provider root and the quick-settings tile use the same icon.
...
- `terminal/TerminalScreen.kt`: overall layout. TopBar / TabBar / render area / keyboard toggle / keyboard area. `KeyboardMode = CUSTOM | SYSTEM`. In **landscape**, orientation is detected via `LocalView.OnLayoutChangeListener`, switching to a Row layout (`SideKeyboardColumn`) per the `landscapeKeyboardPosition`/`Width`/`Height` settings. `landscapeScaledStyle()` scales keyHeight/font proportionally to landscape height.
  ...
  - **An external keyboard folds the in-app keyboard away (0.8.523)**: while a physical keyboard that can type characters is attached, `keyboardCollapsed` is held down and the screen is handed back to the terminal (or to the GUI on a GUI tab). ⚠ **Typing never stops working** — physical keys are picked up by `TerminalInputView.onKeyDown` / `GuiInputView`, so the input path has nothing to do with whether a keyboard area is drawn. ⭐ **The device list from `InputManager` is what we look at** (`ui/terminal/input/PhysicalKeyboard.kt`). `Configuration.keyboard` only carries the coarse QWERTY / 12-key / none classification and never says **which device claimed it**; device by device, three conditions separate the real ones: (1) **skip virtual devices** (the on-screen keyboard is always present as "Virtual", so counting it means the flag is always on), (2) it advertises `SOURCE_KEYBOARD`, (3) its type is `KEYBOARD_TYPE_ALPHABETIC` (volume keys and gamepads also advertise `SOURCE_KEYBOARD` but cannot type). ⚠ **The collapse happens only on a change** (`LaunchedEffect(physicalKeyboard)`). Forcing it on every composition would fold the keyboard straight back after ⌨ reopened it, so it **could not be opened at all**. ⚠ **`keyboard` was added to `configChanges` in `AndroidManifest`** — without it, plugging or unplugging a keyboard recreates the Activity and the tab state is lost. ⛔ **No new setting** — ⌨ and the toggle bar already bring the keyboard back, which is not enough of a fork to deserve an on/off switch.
  - **With an external keyboard, the OS input method does the conversion (0.8.527, user report: "I cannot type Japanese")**: ⛔ **in the in-app keyboard mode (CUSTOM), `TerminalInputView.onCreateInputConnection` returns null**, so no input method is involved at all and physical keys go **straight to the PTY** from `onKeyDown`. The in-app conversion engine only runs for on-screen key presses, so **attaching a keyboard left no way to type Japanese** (0.8.523, which folds the in-app keyboard away, exposed the hole). ⇒ **While an external keyboard is attached, `imeEnabled` is turned on** (the keyboardMode itself is untouched). ⚠ **`requestKeyboard` is not called** — filling the screen with a soft keyboard right after folding it away would defeat the point. ⭐ Letting the input method convert **without showing itself** is the standard shape for a physical keyboard on Android. ⚠ Control keys still work (anything the input method does not consume flows from `TerminalInputConnection.sendKeyEvent` into `AndroidKeyMapper`). ⚠ Whether plain letters go through conversion depends on the **input method's own on/off** (the half/full-width key, exactly as on a PC).
  - **Three-level ⌨ toolbar action (0.8.428)**: single-tap switches keyboard mode, double-tap shows/hides it, and **triple-tap opens an in-place size bar**. `multiTapClickable` does not commit the one- or two-tap action until the third-tap window closes, so a triple-tap cannot switch mode or collapse the keyboard midway. The bar writes the current orientation's height, or the width for a side-docked landscape keyboard, back to the existing setting. It remains available with `keyboardToggleBar=false`. ⭐ **0.8.430 adds it to the Tips list** — the third level of ⌨ is **not visible anywhere on screen**, so without being told, nobody finds it (the same reason the other tips are there).
  - **The size bar carries both height and width (0.8.431, user report)**: ⛔ **a side-docked landscape keyboard only offered width.** The height setting (`landscapeKeyboardHeightDp`) had always applied to side docking as well — **the bar simply never reached it** — and a bottom-docked keyboard had no width setting at all. ⇒ **the bar always shows two axes** (it now takes a list of `SizeAxis`). Bottom width is the new `portraitKeyboardWidthPercent` / `landscapeBottomKeyboardWidthPercent`. ⭐ **Width alone is a percentage, not dp**: screen width varies by device, so a dp maximum would be device-dependent and "as wide as it goes" unreadable from the value. 100% is the previous full-width behaviour; narrower centres the keyboard. ⚠ **Kept per orientation** for the same reason as height (landscape is nearly twice as wide, so the same percentage is a different reach). ⚠ The bar lives **outside the keyboard** (below).
  - **Never put the size slider inside the frame it resizes (0.8.431, user report)**: on a GUI tab in landscape the keyboard strip is drawn as **an overlay inside the green frame**, and the size bar rode along inside it. ⇒ **dragging the slider changed the frame's dimensions, which changed the width of the slider itself**, forcing a re-grab. ⛔ **A tool that changes a size must not live in a container whose size it changes.** ⇒ the bar moved out of `GuiKeyboardPanel` to **the bottom of the tab screen, full screen width**, outside the frame. Terminal tabs were already outside a frame, so their placement is unchanged. ⚠ Now that a bottom keyboard can be narrowed by percentage, **putting the bar "inside the keyboard" is barred for the same reason**.
  - **GUI keyboard placement (0.8.428)**: the built-in keyboard in **portrait is a sibling outside the green GUI frame**, shrinking the frame to the remaining height. It therefore cannot lose its bottom rows to Compose constraints when its configured height exceeds the old frame height. `onSizeChanged` measures the reduced frame, so local GUI resolution follows through the existing renegotiation path. **Landscape keeps the existing in-frame overlay**, since splitting its already short GUI area vertically would make it unusable. The OS IME also keeps its overlay behaviour.
  ...
  - **Landscape moves the toolbar and tabs into a vertical rail (`TabScaffold`, 0.8.431, §12-7)**: ⛔ **both were stacked on top in either orientation.** A landscape screen is only 350–400dp tall, so **48dp + ~40dp = a quarter of the height** went here (turn the keyboard on and almost nothing is left for the content). ⇒ **in landscape they become a 76dp-wide rail docked to one side** — landscape has width to spare and no height, so the space is taken from the side that has it. ⭐ **Which side is derived from `landscapeKeyboardPosition` rather than a new setting** (keyboard left ⇒ rail right; both on one side would load one edge). ⚠ **Reordering only swaps axes**: `ReorderableToolbar` / `TabBar` / `TabChip` gained `vertical`, measured sizes record **the main axis only** (`it.height` vs `it.width`) and the drag reads `amount.y` vs `amount.x`. The swap test (did it pass the neighbour's centre?) is axis-agnostic and is reused **as is**. ⚠ **Tab names do not fit the rail**, so vertical cuts them at 6 characters; the full name and engine stay readable through the **long-press popup** (always leave a way back from truncation). ⚠ **`ToolbarTooltip` now positions itself clamped inside the window** — centred on the chip, a description from a rail at the screen edge fell outside it and could not be read. ⚠ ⚙ stays pinned to **the end of the toolbar** in the rail too (the same promise as the right edge when horizontal: out of both reordering and hiding).
  - **The rail is two columns, not one (0.8.433, user report)**: ⛔ **0.8.432 stacked the toolbar and the tabs in a single column.** "Portrait has two rows but landscape has one" — the boundary was invisible and the column was cramped. ⇒ **turn the portrait two-row layout 90°: two columns** (toolbar 48dp + tabs 44dp), each framed so the boundary shows. ⭐ **The order keeps the portrait relation**: the outer column is the toolbar, the one touching the content is the tabs (portrait's toolbar-on-top, tabs-below). ⭐ **Tab names are written vertically**, one character per line. ⛔ **Not rotated** — rotated text needs a tilted head and lays Japanese names on their side; a monospace face makes per-character wrapping line up. Past 8 characters a "…" closes it off, and the full name stays in the long-press popup. ⚠ **Vertical text spends height**, so that character limit is also the limit on how many tabs fit at once. ⭐ **The side is now a setting (`landscapeRailPosition`, left by default)** — 0.8.432 derived it from `landscapeKeyboardPosition`, which left **anyone docking the keyboard at the bottom with no way to choose**. ⚠ It belongs under Display › Toolbar (it is not a keyboard setting, so it does not go in that group). ⚠ **The `+` / `🖥` buttons at the foot of the tab column stack vertically** (0.8.434: two side by side do not fit 44dp, and the second one — the GUI button — hung outside the column and was invisible).
...
- `ssh/SshProfilesSheet.kt` + `HostKeyVerificationDialog.kt`: SSH destinations plus their FTP / SMB / WebDAV / VNC / RDP services, with SSH host-key verification. Every service uses SSH local port forwarding by default and automatically reserves a free local port when none is specified. Disabling forwarding warns that the route is unencrypted and connects directly to the parent SSH destination host rather than a service-specific host. **Jump hosts (`-J`) are edited here too** (add hops, import a saved connection's fields, one-line route preview; 0.8.494). RDP services gain a **folder sharing** toggle plus shared-folder and share-name fields (off by default, explicit opt-in).
- `sftp/SftpSheet.kt`: shared SFTP / FTP / SMB / WebDAV file browser (**full-screen page**) backed by `RemoteFs`. WebDAV uses OkHttp with normal TLS certificate validation; SMB uses SMBJ, which does not implement SMB1. Android Back and the top-left arrow move to the parent folder and ask before disconnecting only at the root. Scrolling the listing downwards collided with the ModalBottomSheet close drag and dismissed the sheet, so it moved to the same separate-page style as the settings page.
  - **Device-side files in the same screen (0.8.474)**: the tabs at the top switch between "Server" and "This device". The device side is listed through an Android **SAF tree permission** (`sftp/SafFileTree.kt`), and the chosen folder is kept as a persisted permission, so **uploads no longer bounce out to the system file picker every time**. ⚠ Uris are not converted into external-storage paths; they are walked as the `documentId` the provider returned (so providers without real paths work through the same code). Files and folders transfer recursively in both directions.
  - **Preview (0.8.474; full-screen images in 0.8.479)**: text and images open in place, on both the remote and the device side. **Images use a full-screen viewer instead of being squeezed into the file screen’s small dialog**, with pinch zoom and drag pan across the whole display. Text keeps its selectable, scrollable dialog. Reads are **always bounded** (2MB for text, 24MB for images, and images above 16M pixels are downsampled with `inSampleSize`). ⚠ Without a limit and a type check, one tap on a huge remote file drags the screen into a transfer and a decode it cannot come back from. Unsupported types are not opened and say "Only text and image files can be previewed.".
...
- `components/ReorderList.kt`: **drag-to-reorder for vertical lists** (0.8.249). Brings the Snippets-tab feel (grab ≡, move up/down) to lists whose **rows vary in height**; used by the Servers and Automation tabs. Snippet rows are a fixed height, so a constant pitch was enough; server / rule rows grow with status lines and expanded logs, so every row reports its height via `onSizeChanged` and the swap test uses **the measured height of the neighbouring row**. While a row is held, the outer list never overwrites the order (otherwise rows jump out from under the finger). ⚠ Wrap rows in `key(id)` — without a stable node identity the pointer detaches from the row being dragged. ⛔⛔ **Put `key(id)` directly inside the loop** (0.8.511). With a conditional wrapped around it (`if (…) { key(id) { … } }`), **key can only move items within that branch group**, so every swap destroys and rebuilds the item; the in-flight `pointerInput` dies with it and **the drag ends after a single swap** — exactly "it will not move in one go, it only steps one at a time" (hit in the settings toolbar). ⚠ The multi-swap and auto-scroll added in 0.8.510 both assume the gesture is still alive, so with this bug present **neither has any effect and the symptom does not change at all**. When a reorder stops after one item, suspect the **placement of `key`** before speed or thresholds. ⚠ Attach the drag to **the handle only**; on the whole row it fights the on/off and log-toggle taps. Persistence is the caller's job (Servers: store the reordered `ServerEntry` list; Automation: `order=` in each rule file).

### 4.12 GUI desktop (`gui/`)

- **Loader reservations and address-space limits (0.8.516, build unverified)**: on-device diagnosis shows the image-loader child aborting before the 0.8.515 compatibility entry point runs. The custom loader retains Android Scudo primary mappings (8.25GiB on the observed device), counted against `RLIMIT_AS`; a limit of a few GiB makes bionic initialization fail on the next exec. A GUI-independent fork/setrlimit/exec reproducer fails at 5GiB and succeeds when the measured reservation is added. z2root now translates getrlimit/setrlimit/prlimit64 to account separately for that reservation while preserving guest soft/hard limits, returned values and fork/thread inheritance. Limits are not disabled and there is no application-name matching. Regression coverage is in `scripts/z2root-rlimit-test.py`. The engine rebuild and on-device GUI retest remain required before claiming resolution.
...
- **GUI stack install (`ensure_pkgs`)**: Xvnc, openbox and D-Bus form the GUI base; when they are present it **starts immediately with no network** (Alpine also checks the bash and desktop-schema requirements below). **No GUI terminal is installed or launched automatically (0.8.505).** The desktop is an application surface: installed GUI applications launch directly from ☰ / `z2menu`. `install_pkgs` (apk add / apt install / pacman -S) fetches only a missing base after the app-side download-confirm gate takes consent. Only `clean` wipes caches and reinstalls the base.
  - ⭐⭐ **Root cause of GUI applications appearing in the list but exiting immediately on Alpine (0.8.513)**: the minimal rootfs has no bash, while some `.desktop` launch targets installed by GUI packages are wrappers with `#!/usr/bin/env bash`. `z2menu` can confirm that the wrapper itself exists with `command -v`, but not that its interpreter exists; tapping it therefore ended before any window was created with `env: can't execute 'bash': No such file or directory` (127). ⇒ bash is part of Alpine's GUI base, and `gui_stack_ready` requires `has bash`, so an existing GUI rootfs is repaired on its next start too. There is no application-name substitution or OS-by-application exception table.
  - ⭐⭐⭐ **Shared causes behind GUI failures varying by OS/application (0.8.514)**: (1) `GuiSession` re-read the global distro setting independently for start, listing, and application launch, so after an OS switch it could send an application from a new rootfs to the old OS's Xvnc on the same display. ⇒ inject `Z2_DISTRO_ID` into every proot, carry it in `z2run`'s `OPEN display distro` event, bind the GUI session to that source OS once, and close the old GUI/Xvnc when switching a terminal's OS. (2) z2root fakeroot presented uid 0 through `getuid()` and `SCM_CREDENTIALS`, but left DBus authentication's `getsockopt(SO_PEERCRED)` at the real Android app uid. The client's claimed uid 0 therefore never matched the socket peer and session-bus authentication failed for every application. ⇒ after successful syscall 209, rewrite `struct ucred.uid/gid` to 0 as well. (3) make XDG Desktop Portal plus its GTK backend part of the GUI base for every package manager, providing settings, file-selection and URI services over DBus. The Alpine Dolphin SIGSEGV was mapped on-device to KDE Frameworks 6.8.0 `libKF6GuiAddons`, not Dolphin: its known old Settings-portal path dereferences an empty `arguments().first()` on an error reply. No Dolphin-specific wrapper is added. (4) future unknown fatal signals write only PC/LR and matching maps through `Z2ROOT_CRASHLOG`; no full-syscall tracing cost exists during normal use.
  - ⭐⭐⭐ **GUI-wide GTK/glycin launch compatibility (0.8.515)**: modern GTK/gdk-pixbuf starts a glycin loader through `bwrap --unshare-all ...` for SVG and other image reads, but a Linux user/mount namespace cannot be nested inside Android's application sandbox, so bubblewrap exits before the loader returns even one image. Any GUI application using that image stack can therefore abort at startup; this is not Remmina-specific. ⇒ prepend a managed `bwrap` entry point only to GUI-child PATHs. It parses the request and, only when the **actual executable** is a `glycin-*` loader under z2root, omits the namespace/bind/seccomp layer and executes the loader directly. There is no application/distro exception table, while every non-glycin bubblewrap call is delegated unchanged to `/usr/bin/bwrap`. This is an explicit compatibility decision to omit glycin's inner sandbox; Android's outer application UID/SELinux sandbox remains in place. Normal terminal PATHs are unaffected.
- **Xvnc rendering compatibility (0.8.505)**: besides disabling MIT-SHM in Xvnc, Qt receives `QT_XCB_NO_MITSHM=1`, GTK receives `GDK_RENDERING=image`, and both are pinned to X11 backends.
- **gThumb / SMPlayer compatibility (0.8.506)**: gThumb 3.12.10 aborts while loading its first SVG icon when glycin tries to nest bubblewrap's Linux-namespace sandbox inside Android's app sandbox. z2term loads `libz2glycin.so` only from a gThumb-specific wrapper, selecting gdk-pixbuf's upstream “already externally sandboxed” path. The shim is limited to the gThumb process and does not alter other applications, root chroot, or the real `bwrap`. SMPlayer starts mpv with `--no-config`, so `/etc/mpv/mpv.conf` alone is insufficient. Its real configuration is in the distro-specific `.config` overlay, and Qt serializes its `General` group as `[%General]`; z2term therefore preserves every other setting while correcting `[%General] driver\vo=x11` and `[performance] hwdec=no` in that overlay.
- **GUI clean-install setting removed (0.8.507)**: the GUI is an application display surface, so Settings no longer exposes a destructive stack-reinstallation switch. Its stored state, next-launch arm, and dedicated confirmation dialog are removed; GUI tabs always take the normal startup path. The low-level `z2gui clean` recovery command remains terminal-only. ⭐ **0.8.519 lists that command under Settings → How to use (Tips)** — with the switch gone, it has no on-screen entry point at all.
- `GuiSession`/`GuiActivity`/`GuiScreen`/`GuiViewport`/`GuiInputView`/`GuiKeyMapper`/`GuiEventWatcher` + `gui/RemoteDesktopClient.kt` (the shared drawing/input boundary) + `gui/rfb/RfbClient.kt` (the built-in RFB implementation). Pairs a terminal tab with a GUI tab with IME linkage. `GuiSession`, Compose drawing, the input View and the GUI keyboard no longer require the concrete `RfbClient` type, so an RDP implementation can enter through the same boundary (0.8.450). **The destination itself is `gui/RemoteTarget.kt`** (implemented by `VncTarget` and `RdpTarget`); `GuiSession` only calls `remote.createClient()` and never learns which protocol it got (0.8.459).
- **RDP (0.8.450–0.8.492)**: `gui/rdp/` covers X.224/TLS, CredSSP/NTLMv2, T.124 GCC and T.125 MCS, Client Info, licensing, Demand/Confirm Active, connection finalization, slow-path classic Bitmap Update reception, and the **Graphics Pipeline (RDPGFX)** — **with no external library** (MD4, RC4 and NTLM are written here too). `RdpClient` expands clipped multi-rectangle 15/16/24bpp updates (uncompressed or Interleaved RLE) into an ARGB framebuffer and publishes dirty redraw notifications. **0.8.459 added the connection UI: RDP is now a service on an SSH destination, opened with `[RDP]`** (see §6.3.1). **0.8.476 added slow-path mouse/keyboard input**, **0.8.480 dynamic resize** (see "Display Control" below); CLIPRDR text sharing is also supported. Fast-Path, Surface Commands, Bitmap Codecs, individual drawing orders and 32bpp RDP 6.0 compression are **not advertised in the legacy capability set at all** (`orderSupport[32]` all zero, General cap `extraFlags` = 0). **Declaring what is not implemented** is the point here — this is not about discarding what arrives.
  - ⚠ **Receive-side diagnostics log each kind once** (0.8.480). They were what pinned down the connection sequence, but once the screen is up they **scroll past everything else on every update**. ⇒ Rather than deleting them, `ActiveSession` remembers the kinds it has already seen **per connection** (the same counting RDPGFX uses for commands and codecs). ⚠ Putting that suppression on the `object` would silence the second tab and every reconnect.
- **RDPGFX (0.8.477)**: ⛔ **Windows 11 does not fall back to classic Bitmap Updates for a peer that cannot use RDPGFX.** This was measured, not guessed: `xfreerdp3` with the graphics pipeline turned off goes just as silent (connection and finalization complete, and not one screen PDU arrives). So the GCC CS_NET request now includes **`drdynvc`** (`RdpDynamicChannel`); after the DVC capability/create exchange the client opens `Microsoft::Windows::RDS::Graphics` and receives the screen over **RDPGFX** (`RdpGfx`). CS_CORE declares 32bpp together with `RNS_UD_CS_SUPPORT_DYNVC_GFX_PROTOCOL` (⚠ **announcing GFX while omitting 32bpp contradicts itself**). Only the CAPVERSION_8 thin-client capability is advertised; surface create/delete/map-to-output, frame acknowledgement, solid fill, surface-to-surface copy, the surface cache, 32bpp uncompressed data and **ClearCodec** (`RdpClearCodec`) are handled. Incoming data is wrapped in RDP 8.0 bulk compression, so `Rdp8Bulk` decodes it here as well (outgoing PDUs stay uncompressed, exactly as FreeRDP sends them).
  - ⛔ **Never throw on an unimplemented command or codec.** RDPGFX PDUs are length-delimited and can be skipped. Throwing kills the whole receive loop and **loses even the parts that could be drawn**. What arrived is logged once, so the next decoder is chosen from measurement rather than guesswork.
  - ⛔ **`CHANNEL_FLAG_SHOW_PROTOCOL` belongs only on a channel declared with `CHANNEL_OPTION_SHOW_PROTOCOL`** ([MS-RDPBCGR] 2.2.6.1). Setting it on `drdynvc`, which does not declare it, makes Windows take the 8-byte Channel PDU Header as payload and **go silent** (found against real Windows in 0.8.477: the Graphics DVC opens, and the capability response never comes). The chunk size likewise follows the **VCChunkSize the server advertises**, not the one the client sends.
- **RemoteFX (0.8.478)**: ⭐ **What this Windows 11 actually chose over RDPGFX was RemoteFX (CAVIDEO, codec 0x3) and uncompressed (0x0)** — measured in 0.8.477 by counting the codecs that arrived, not guessed. `RdpRemoteFx` turns each 64x64 tile back through **RLGR1/RLGR3 decoding → differential restore (lowest band only) → dequantization → three levels of inverse DWT → YCbCr-to-RGB**, then clips it with the `TS_RFX_REGION` rectangles and writes it into the surface. ⭐ **This is what finally put a real Windows screen on the phone** (0.8.478).
  - The unit test drives a **hand-assembled RLGR bit stream** (4032 zeros plus the value 72 encoded as a single symbol) so the run-length build-up, the differential restore, all three inverse DWT levels and the color conversion are pinned **end to end**. An empty component decoding to mid gray is pinned alongside it.
- **Display Control (0.8.480)**: rotating the phone or resizing the split sends a Monitor Layout over the `Microsoft::Windows::RDS::DisplayControl` DVC (`RdpDisplayControl`) and **makes the peer rebuild its session**. A peer that accepts it answers with an RDPGFX Reset Graphics, so `RdpClient.publishGraphicsFrame` reallocates the framebuffer at the new size through the path that already existed.
  - ⭐ **"May we change the resolution?" is a property of the peer, not of the protocol name** (`RemoteDesktopClient.ownsDesktopSize`). RDP builds **a session of our own** on every connection, so changing it is fine; an RFB peer is **a real screen that is already running**, so it is left alone. `GuiSession.requestResize` looks only at that flag — no protocol names at the call site.
  - ⚠ **Do not send a Monitor Layout before the CAPS arrive** ([MS-RDPEDISP] 1.3.1). Until then the monitor count and area limits are unknown, so there is no guarantee the peer would accept it. ⇒ **A request made before CAPS is held — only the most recent one** — and sent when CAPS arrive (repeated rotations request only the size things settled on).
  - The requested size goes through **the same** `RdpTarget.fitDesktopSize` used at connect time (landscape, multiple of 4, 640-4096) and is then filtered by [MS-RDPEDISP] 2.2.2.2.1: **the width must be even**, sides stay within 200-8192, and the area must fit what CAPS advertised. ⚠ **The same size is never re-sent** — every request rebuilds the session, blanking the screen on the way.
  - ⛔ **Never call a DVC handler while holding a lock.** The receive thread enters as "`RdpDynamicChannel` lock → `RdpDisplayControl` lock", so taking them in the opposite order on the sending side deadlocks both when a rotation coincides with a screen update. ⇒ **Build the PDU under the lock, send it outside.**
- **Audio (rdpsnd, 0.8.481–0.8.492)**: the `rdpsnd` static virtual channel (`RdpSound`) speaks [MS-RDPEA] and plays the peer's audio through the device speaker (`RdpAudioSink`).
  - ⚠⚠ **One message can carry two or more PDUs (0.8.487)** — the same behaviour measured on CLIPRDR. Reading only the first and discarding the rest **loses the Training PDU, so the peer never starts sending audio**. ⇒ Walk the buffer. ⛔ **Stop after a WaveInfo**: the raw bytes that follow carry no PDU header, so anything left in that message must not be read as one.
  - ⛔⛔⛔ **We were the ones declaring that no audio was wanted (0.8.488, found in the device log).** The Client Info PDU flags were a bare `or` of numbers, and `INFO_NOAUDIOPLAYBACK` (0x00080000) was among them. The peer obeys it, so **not a single message arrives on `rdpsnd` or `AUDIO_PLAYBACK_DVC`** — the channel simply opens and stays silent, which makes it look like the peer is misconfigured. ⇒ Drop that bit and **always spell these flags by name**. ⭐ Audio redirection is **on by default**, so *not* saying "no audio" is exactly how you ask for sound (`INFO_REMOTECONSOLEAUDIO` stays clear too — that one asks the peer to play it locally). ⚠ **It does not matter that the peer has no audio hardware**: an RDP session gets a virtual "Remote Audio" endpoint. ⚠ Nobody could spot this bit when audio landed in 0.8.481 — **a row of literals gives a later reader no way to check what it means.**
  - ⛔⛔⛔ **Audio requires the `rdpdr` channel to be open (0.8.491, found by diffing against FreeRDP).** The peer **only redirects audio to a client that can redirect devices**; FreeRDP likewise loads `rdpdr` whenever `/sound` is given. ⇒ `gui/rdp/RdpDeviceRedirection.kt` implements the [MS-RDPEFS] handshake. ⚠ **Opening the channel is not enough**: the peer waits for the full exchange (Server Announce → Client Announce Reply → Client Name → Server Capability → Client Capability → Device List) before moving on. ⚠ Client ID Confirm arrives either before or after the capability exchange, so **never hard-code the order** (measured).
  - **With folder sharing off, zero devices are shared** (`ioCode1` = 0, `SpecialTypeDeviceCap` = 0, Device List Announce with `DeviceCount` = 0): no drives, no printers, no smart cards — **then this channel exists solely for sound**.
  - **Folder sharing (0.8.494, `gui/rdp/RdpDrive.kt`)**: turning it on for a destination's RDP service offers one folder on the phone as `\\tsclient\<share name>`, **readable and writable**. ⚠ **Off by default, explicit opt-in** — a folder on the phone must never quietly become writable by the far side. The default location is **the same one clipboard files land in**, `Download/z2term` (`RdpShareDefaults` reads `ClipboardFileTransfer.FOLDER`; splitting the return path across two folders just means hunting for the file). ⭐ **Never ask where to save** — only people who want a different folder type a path.
    - **The peer talks to it as if it were an ordinary file system**: open / read / write / list / query / rename / delete / free space arrive as individual IRPs (`IRP_MJ_CREATE` … `IRP_MJ_DIRECTORY_CONTROL`) and are answered one at a time. ⇒ this is **a small file server**, not a bulk transfer.
    - ⛔⛔ **Nothing outside the shared folder is ever reachable.** Rejecting `..` is not enough: the path is **resolved (`canonicalFile`) and then checked against the share root**, because a symlink pointing outside can be sitting inside the shared folder. Renames go through the same check (`RdpDrive.resolve`; `RdpDriveTest` covers both).
    - ⛔⛔ **File I/O never runs on the receive thread.** IRPs arrive on the RDP receive loop, so waiting on the disk there **freezes the screen, input and sound together** (the same contract as the CLIPRDR fetch). ⇒ hand it to the single `rdp-drive` thread and send once the answer exists. **One thread**, so ordering is preserved.
    - ⚠ **Forget to advertise the drive and no IRP ever arrives.** The capability response must carry `ioCode1` = `0x0000FFFF` (the fixed value [MS-RDPEFS] prescribes) and `CAP_DRIVE_TYPE` before the announced folder is used at all. Without sharing, `ioCode1` stays 0 as before.
    - ⚠⚠ **Packing structures exactly as [MS-FSCC] tabulates them makes the peer misread them.** Omit the trailing `Reserved`(1) of `FILE_BOTH_DIR_INFORMATION` and `FILE_FS_VOLUME_INFORMATION`, and the tails of `FILE_BASIC_INFORMATION` / `FILE_STANDARD_INFORMATION` (93 / 17 / 36 / 22 bytes). FreeRDP made the same call, and interop against real hosts says this is the correct one.
    - ⚠ **Directory change notifications (`IRP_MN_NOTIFY_CHANGE_DIRECTORY`) get no answer.** A real file system holds the request until something changes; answering with an error makes Explorer show one the moment a folder is opened. ⇒ swallow it silently (FreeRDP does the same).
    - ⚠ **Report free space.** Explorer refuses a copy based on it, so `FILE_FS_SIZE_INFORMATION` carries the device's actual free space (returning 0 forever fails every copy with "not enough space").
    - ⚠ **Share names are folded to ASCII** (`RdpDeviceRedirection.asciiShareName`): the device list carries the name as ASCII, so a Japanese folder name would arrive mangled. `PreferredDosName` is truncated to 7 characters + NUL. **File names inside stay UTF-16, so Japanese names travel fine.**
    - ⚠ **Closing the tab closes the files.** A peer can disconnect without closing them, so `RdpClient.close` / `closeTransport` always route through `RdpDeviceRedirection.close()` → `RdpDrive.close()`.
  - ⛔⛔⛔ **Fail to declare the link speed and audio is the first thing dropped (0.8.490, found by diffing against FreeRDP).** `connectionType` in the Client Core Data was left at 0 and `RNS_UD_CS_VALID_CONNECTION_TYPE` (0x0020) was never set. **Without that bit the field is not read at all** ([MS-RDPBCGR] 2.2.1.3.2), so the peer sees "unknown, therefore slow" and **disables audio redirection entirely** — the same state as picking Modem in mstsc's Experience tab. ⇒ Set the bit and declare `CONNECTION_TYPE_LAN`. ⭐ **This is not a bandwidth claim; it is the input the peer uses to decide what to cut.** ⚠ The symptom is "the `rdpsnd` channel opens but never delivers anything", which points suspicion at the peer's configuration. It took **connecting to the same host with FreeRDP and diffing the two client announcements** to see it (FreeRDP `earlyCapabilityFlags=0x0fb2` vs z2term `0x0900`). ⚠ **The peer will also use `AUDIO_PLAYBACK_DVC`** (a dynamic channel) — it opens that one for FreeRDP. ⭐ But **it falls back to the static `rdpsnd` when the client offers no dynamic listener**, so the static implementation alone is enough (measured in 0.8.491: one 16-bit PCM format among the 30 offered, playing at 44.1kHz stereo).
  - ⚠ **Declare the static channel's priority (0.8.489).** `CHANNEL_OPTION_PRI_MED` was added to the `rdpsnd` channel options (both FreeRDP and mstsc set it). The spec should not require it, but while chasing a channel that delivers **nothing at all**, leave no difference from the widely deployed implementations.
  - ⚠ **Log the flags actually sent (0.8.489).** Whether a bit we removed is really gone cannot be settled by reading the source — what runs is an APK that went through R8.
  - ⚠ **When there is no sound, make it possible to tell whose side it is on (0.8.487, 0.8.492).** Log the first chunk to arrive on the channel, then **only the first PDU of each type**. ⚠ While nothing is playing the peer keeps sending Training (`0x06`, 4-byte body) every few seconds as a liveness check; logging each one **floods logcat and buries every other investigation**, so a type is never repeated. **A channel that opens but never delivers a single message** means the peer is not redirecting audio at all — nothing on this side can fix that.
  - ⭐ **What is advertised decides what the peer sends** (same lesson as RemoteFX). Only **16bit PCM** is accepted, and only **one format is returned**. With a single entry the `wFormatNo` is always 0 and the format cannot change mid-session ⇒ **no decoder is needed**. ⚠ If the peer offers no PCM at all there is no sound (the connection is unaffected); whether ADPCM/AAC decoding is worth adding is decided **from what actually shows up in the log**.
  - ⚠⚠ **The continuation of a WaveInfo has no PDU header.** In the RDP 5-era shape the samples continue in the next message, whose first 4 bytes are discarded; without tracking that state the audio data is parsed as a PDU and everything breaks. RDP 8's Wave2 fits in one message, so `wVersion` advertises 8.0. ⚠ Either shape **must be answered with a Wave Confirm** — without it the peer stops sending and the audio dies within a few hundred ms.
  - ⛔⛔ **Never call `AudioTrack.write` on the receive thread.** It blocks until the buffer drains, so **the moment audio backs up, the screen and input stop too**. ⇒ A dedicated thread and a bounded queue sit in between, and **the oldest audio is dropped on overflow** (dropped sound is acceptable; a frozen screen is not).
  - Static virtual channel reassembly is identical for cliprdr, so it moved into `RdpChannelReassembler` (⚠ DVC fragmentation is **a separate layer** whose payload `RdpDynamicChannel` reassembles again).
...
**Terminal audio (0.8.588)**: `z2-audio install` installs audio packages in the current OS. `z2-audio run COMMAND [ARG...]` allocates a dedicated output to the local terminal session, waits for PulseAudio and the Android connection, then runs the command in the foreground with its original arguments, stdin and exit status. Child-only PULSE_SERVER and temporary configuration isolate it from GUI runtime directories and existing settings. Exit/interruption releases the server and output; tab close, restart and SSH switching also stop Android output. Each invocation uses a separate localhost port. Audio is s16le/48kHz/stereo; samples split across TCP reads are retained. Small buffers reduce added delay, although downstream Android latency is not part of the player audio clock. No recording permission is needed. SSH sessions are unsupported.The root chroot path also binds the API shared directory and exports the source OS identifier.
- ⭐ **The openbox config is now an overlay, not a replacement (0.8.498)**. Up to 0.8.497 `z2gui` wrote an 8-line `/tmp/z2-openbox-rc-<N>.xml` holding **nothing but the window position override** and passed it to `openbox --config-file`. ⛔ **openbox has no built-in key bindings, mouse bindings or menus — every one of them is data in rc.xml.** Replacing the distro default (743 lines on the test device) with 8 lines silently removed `<keyboard>` (Alt+Tab and friends), `<mouse>` (titlebar buttons, resizing) and `<menu>` (desktop right-click → `root-menu`) **all at once**. The right-click menu is the **only way to launch an application from inside the GUI**, so losing it leaves a desktop that can show nothing but a terminal — this was the real cause of the "all I get is a console" report. ⇒ The default (`~/.config/openbox/rc.xml`, then `/etc/xdg/openbox/rc.xml`) is now copied through awk, which replaces **only two things: where the menu comes from, and where windows are placed**.
  - ⚠ **Drop every `<file>` and put exactly one back.** openbox accepts several menu files, so replacing only the first one leaves the distro's fixed list in place.
  - ⚠ **The position override goes at the end of `<applications>`.** openbox applies every matching rule in order and the later one wins, so placing it before the default per-app rules would have no effect.
  - The old minimal config remains only as a fallback for distros with no default, or if the rewrite fails (on that path windows can neither be moved nor resized — it is a last resort).
- **`z2menu` — a menu of what is actually installed (0.8.498)**: the default `menu.xml` is a **fixed list written by the distro** and is mostly made of **applications that are not installed here** (a menu whose entries do nothing when pressed is harder to use than no menu at all). `Z2MenuScript.kt` installs `/usr/local/bin/z2menu` and openbox calls it as a **pipe menu** (run each time the menu opens; whatever XML it prints becomes the menu). It reads `.desktop` files from `~/.local/share/applications`, then `/usr/local/share/applications`, then `/usr/share/applications`, and emits only entries that are `Type=Application`, not `NoDisplay`/`Hidden`, and whose `TryExec` and first `Exec` word **actually exist on PATH**. `z2menu list` returns the same list as TSV (name, command, comment, terminal flag, category).
  - ⚠ **Field codes in `Exec` (`%f %F %u %U %d %D %n %N %i %c %k %v %m`) are stripped.** Left in place, an argument-less launch makes the application try to open a file literally named `%U`. `%%` is a real `%`, so it is parked under a marker first and restored last.
  - `Terminal=true` entries are excluded (0.8.505), keeping the GUI base independent of any particular terminal.
  - Categories become submenus **only past 20 entries**. Below that, a hierarchy just adds taps.
  - When the same file name (desktop id) exists in several directories, **the first one read wins**, so what the user dropped in `~/.local/share` beats the distro default.
  - ⛔ **Keep the awk POSIX-only.** busybox awk has neither `ENDFILE` nor whole-array deletion. File boundaries are detected with `FNR==1` and the last record is flushed in `END`; values live in **scalar variables**, not associative arrays. ⚠ `sh -n` does not look inside the awk program (`GuiScriptSyntaxTest`), so run `z2menu list` on a device after touching it.
  - The window list is openbox's own `client-list-menu`. ⚠ `wmctrl` and `xdotool` are **not part of the install set on any distro**, and counting windows is not worth a new dependency.
- **The D-Bus session bus and `XDG_RUNTIME_DIR` are now set up regardless of terminal choice (0.8.498)**: up to 0.8.497 they were only created **when the selected in-GUI terminal was konsole**. Anything started from the right-click menu or from `z2run` in another tab therefore ran without them, and applications that **start their helpers as separate processes** (KIO and friends) failed across the board — a file manager would come up with no thumbnails, no trash and no device list. ⇒ `start_session_bus` runs after `start_audio` and **before openbox**, because anything launched from the right-click menu inherits openbox's environment verbatim.
  - ⭐ **The address is a fixed path under `XDG_RUNTIME_DIR`.** `dbus-launch` picks a new address on every start, so another tab's `z2run` could never join it. `dbus-daemon` is tried first and `dbus-launch` is the fallback; either way the address is recorded in `$XDG_RUNTIME_DIR/dbus-address` for `z2run` to read (and removed by `z2gui stop`).
  - ⚠ **`dbus-daemon --print-pid` takes a file *descriptor*, not a path.** Up to 0.8.497 a path was passed there, so it **always failed** with `Invalid file descriptor` — hidden only because `dbus-launch` was tried first. The pid is now taken from `--print-pid` with no argument, with **stdout redirected to the file**.
  - `dbus-daemon` was added to `is_gui_proc`. Its pid had always been recorded in PIDFILE, but the process-kind table did not list it, so `z2gui stop` left it running.
- ⭐⭐ **Why the GUI would never open again, and why applications started from ☰ never appeared (fixed in 0.8.504)**: **the liveness test for X was always wrong under the z2root engine.**
  - ⛔ **Under z2root every guest's `/proc/<pid>/comm` reads `libz2root.so`** (never the real name). `x_alive` / `is_gui_proc` compared that against `Xvnc` / `openbox` / `xterm`, so they were **always false**: `z2gui stop` killed nothing, **Xvnc survived**, and because it still held the display the next start died with `Cannot establish any listening sockets`. ⭐ **Identify the process by `DISPLAY=:N` in its `environ` instead** (measured: an Xvnc under z2root has `comm=libz2root.so` and `DISPLAY=:12` in `environ`). The name match stays for the chroot engine, where comm is the real name.
  - ⚠ **A socket file is not proof that X is running.** The GUI dies by `--kill-on-exit` (SIGKILL), so `/tmp/.X11-unix/X<N>` and `/tmp/.X<N>-lock` are **left behind**. `z2run` reads that leftover as "the GUI is up", skips z2gui and `exec`s the application, which then dies with `Cannot open display` — that is what "picking an entry in ☰ does nothing" really was (the user's device still had a socket from a week earlier).
  - ⭐ **Clean it from the app side (`ProotLauncher.cleanStaleXSockets`).** ⛔ **Do not judge liveness from `/proc`** — comm is useless there and **pids from another instance are not visible at all**. **Actually connecting to the socket** (`LocalSocket`) is the only reliable test, and only the app can do it. If the connection fails, the socket, its lock and the pids file are removed together.
- **The application database is built only when missing (0.8.498)**: `ensure_desktop_db` runs `update-desktop-database` only for directories with no `mimeinfo.cache`. ⛔ **Never on every start** — it re-reads every `.desktop` and makes startup visibly slower.
- ⭐ **☰ application sheet (0.8.499) — a permanent way into the applications**: getting the right-click menu back only restores an entrance **you have to know about** (long-press an empty part of the desktop). What was actually asked for is the equivalent of a Windows start button, so the GUI toolbar gained `ToolbarButtons.APPS` (☰, `guiOnly = true`), which opens the sheet in `ui/gui/GuiAppsSheet.kt`. Tapping a row hands it to `GuiSession.launchApp`, which starts it through `z2run`.
  - ⭐ **The list is fetched once per app launch** (0.8.509, requested). `GuiAppCatalog` keeps the result per distro and later opens return it immediately (`GuiSession.refreshApps(force = false)`). Starting `z2menu list` on every open cost roughly a second each time — proot start plus reading the `.desktop` files — and the sheet sat empty for it. The only way to fetch again is the **Refresh** button at the top of the sheet (`refreshApps(force = true)`). ⚠ **An empty result is never cached** — empty means the GUI is not installed / the rootfs is not unpacked / the read failed, and returning empty forever after would trap you in "I installed it and it is still not listed".
  - ⭐ **The list is whatever TSV `z2menu list` returns** (`gui/GuiAppCatalog.kt`). ⛔ **Do not re-implement `.desktop` parsing on the Kotlin side.** What gets listed (`Type` / `NoDisplay` / `Hidden` / `TryExec` / whether `Exec` resolves on PATH / field-code stripping) is decided by z2menu, and **two copies of that rule will always drift** — showing up as the right-click menu and ☰ listing different things.
  - ⚠ **Reading happens over a PTY, so newlines are CRLF** (`ONLCR`). Strip `\r` before splitting on TAB, and drop lines with too few columns so a stray line from proot cannot break the list.
  - ⚠ **Never wait on `z2menu list` forever** (20 s cap). If it does not return, one tap on ☰ leaves the sheet loading for good. Reading a few dozen `.desktop` files measures under a second in practice. ⛔ **Do not write that cap with `withTimeoutOrNull` (fixed in 0.8.502).** The body is a **blocking read** on a PTY and never observes cancellation, so nothing happens when the deadline passes until the read returns on its own — the 20 s cap did nothing. **Closing the PTY makes the read fail and return**, so the cap is a separate watchdog coroutine calling `close()`.
  - ⭐⭐ **`readBytes()` was why ☰ listed nothing at all (0.8.499–0.8.501, fixed in 0.8.502).** When the guest side exits, a PTY master's read fails with `EIO` — EOF arrives as an **exception**, not as a return value. `InputStream.readBytes()` propagates it, **throwing away the TSV it had already read, so the list was always empty**. It showed up as `z2menu list` returning 20 rows from a terminal while the app alone saw nothing. ⛔ **Never read a PTY with `readBytes()`.** Accumulate what each read returns and treat the exception as a normal end (the same shape as `GuiSession.drainPty` and `HeadlessRun`).
  ...
  - ⚠ **Refetch only when Refresh is tapped.** If a freshly installed package does not appear, that entry point refreshes the list.
  - ⭐⭐ **Root cause of applications opening or not depending on the OS / application (0.8.512)**: every application picked from ☰ starts under an independent z2root, but the launcher did not pass `--wait-tracees`. An application that stays in the foreground worked; one that forked at startup and let its parent exit (daemonisation or single-instance hand-off) vanished when z2root left on the main tracee's exit and `PTRACE_O_EXITKILL` **killed the process that owned the window**. Package builds use different process models across distributions, making this look like an OS-by-application compatibility matrix. ⇒ `GuiSession.launchApp` passes `waitTracees = true`, keeping the tracer alive until every descendant exits. No per-application or per-distribution exceptions are needed.
  - ⛔ **Do not close a launched application's PTY either.** z2root uses `--kill-on-exit`, so closing the root PTY **kills the GUI application with it** (`setsid` does not escape z2root's supervision). `waitTracees = true` handles a parent that exits naturally; retaining the PTY in `GuiSession.appPtys` prevents Android from closing that parent prematurely. Both are required. They are closed together only when the GUI tab closes (`stop`).
  ...
  - As above, `Terminal=true` entries are excluded. `Terminal=false` GUI applications launch directly regardless of which terminal packages happen to be installed.
  ...
  - ⚠ **The window list is not in the sheet.** Reading X's window list (`_NET_CLIENT_LIST`) needs `wmctrl` or `xdotool`, and **neither is part of the install set on any distro**. Rather than add a dependency, openbox's own `client-list-menu` (long-press the desktop → "Windows") handles it, and the sheet points there in its footer.
- ⭐⭐ **Qt6 applications could not save a single byte of configuration (diagnosed in 0.8.498, fixed in 0.8.500)**. On Linux, Qt's `QSaveFile` / `QTemporaryFile` create an unnamed file with `O_TMPFILE` and name it at the end via `linkat(AT_FDCWD, "/proc/self/fd/<fd>", …, AT_SYMLINK_FOLLOW)`. ⛔ **An Android app process holds no capabilities at all** (`CapEff = 0` in `/proc/self/status`), so that `linkat` always fails with `ENOENT` (the `AT_EMPTY_PATH` form gives `EACCES`). Configuration, caches and the application database were all unwritable, and **a file manager resolving "open with" through the KDE database did nothing on a double-tap**.
  - ⚠ **Opening with `O_TMPFILE` succeeds**, so the symptom does not look like a permission problem (`touch` and `rename` work in the same directory, and there are 41 GB free). Measured on Qt 6.11.2: `kwriteconfig6` **returned exit code 0 while creating no file**, and `kbuildsycoca6` finished silently leaving no cache. What Qt reports — "Disk full?", "No such file or directory", `QLocalServer::listen: Name error` — **points at none of the real cause**.
  - ⭐ **The fix (0.8.500): an `LD_PRELOAD` shim refuses `open` with `O_TMPFILE`.** When `O_TMPFILE` is unavailable Qt **falls back to named temporary files**, and everything writes normally again. ⚠ **That premise was verified first**: with `HOME` on a filesystem that rejects `O_TMPFILE` (`/sdcard` returns `EOPNOTSUPP`), `kwriteconfig6` wrote its settings and `kbuildsycoca6` built a 300 KB database — with no shim at all. **Confirm this before writing a shim**; without the fallback, a shim fixes nothing.
  - ⛔ **No new `.so` — it went into `z2usb.c`.** `LD_PRELOAD` symbol resolution takes **the first match only**, so stacking two `.so` files that intercept `open`/`openat` leaves **the second one entirely dead** (`libz2usb.so` already hooks the whole `open` family). ⭐ **Anything that needs the `open` family goes here, never into another shim.** The file name comes from USB, but its role has widened to "the shim that owns open/openat" (stated at the top of the file).
  - ⚠ **Return `EOPNOTSUPP` (95) and nothing else.** It means "this filesystem has no `O_TMPFILE`", which is what makes Qt and glibc fall back. `EPERM` or `EACCES` reads as "you may not write here", and some implementations then fail outright instead of falling back.
  - ⚠ **Do not strip `O_TMPFILE` from the flags and pass the call through.** The path is a directory, so it fails with `EISDIR`, and the caller sees a puzzling error rather than "no temporary file".
  - **Measured on the device with the shim active**: `dolphinrc` (113 B), `user-places.xbel` (2467 B) and `dolphinstaterc` (544 B) — none of which had ever been written — were all created, and the application database was built at 305 KB. ⭐ **Startup errors went from 40+ lines to zero.**
  - ⚠ **Only the z2root engine gets this** (`LD_PRELOAD` is part of `ProotLauncher`'s z2root-only env). PRoot intercepts syscalls itself, so its behaviour differs and is untested.
  - ⚠ **GLib-based tools were never affected** (`update-desktop-database` wrote `mimeinfo.cache` normally). When this symptom appears, first work out **which of the two stacks** the application belongs to.
...
- ⚠ **Historical GUI-terminal record (0.8.343; automatic terminal installation/launch was removed in 0.8.505).** `ensure_pkgs` used to decide "installed" from **`has $GUI_TERM_BIN` (does the binary exist)**, so anything that was present but *failed to start* slipped straight through and `z2gui` ran to completion with only the desktop up. All the user saw was "the desktop appears but there is no terminal".
...
- **Remote VNC (A1, 0.8.418)**: `GuiSession(remote = VncTarget(host, port, password, name))` turns the tab into **nothing but an RFB connection to an outside server** — no z2gui, no proot. Drawing, input, the keyboard, the clipboard and zoom/pan all sit on the same `RfbClient` / `GuiScreen` / `GuiInputView` as the local GUI, so **what is new is only "a way to point somewhere else" and "VNC authentication"**.
  ...
  - **Cursor and pointer controls (0.8.427)**: `GuiSession` owns one `GuiCursor`, shared by `GuiInputView` (input) and `GuiScreen` (drawing). Its arrow is always drawn locally even when the server does not include a cursor in the framebuffer, and its position survives tab changes, View recreation and reconnects (centred only once and clamped when the framebuffer shrinks). The default remains **relative mode**, where finger motion moves the cursor by the same delta. Double-tap 📜 in the GUI toolbar to switch to **absolute mode**, where touching a framebuffer position moves the cursor there directly; a green ring at the arrow root marks absolute mode. A single tap on 📜 still opens snippets.
  - **Double-tap gesture (0.8.427)**: hold the second tap still for at least 350 ms and **release** for a right click; release sooner for the second click of a double-click; move beyond touch slop for a left drag. The 350 ms timer now only gives haptic feedback and never commits the right click while the finger is still down, so pausing to aim before moving can still become a drag.
  - **The right click no longer waits (0.8.429, user report)**: ⛔ **"hold for 350 ms, then release" is gone.** The report was "I cannot tell how long to wait": even with haptic feedback, **the decision of when to lift stays**, and that was the difficulty. ⇒ **after 150 ms the timer sends the right click on the spot**, with no lift required. ⚠ **A left drag must therefore start moving within those 150 ms** — shortening the delay makes the right click easier and the drag harder, so the value lives in one place (`RIGHT_HOLD_MS`). ⚠ **`ACTION_UP` can arrive before the timer**, so the UP path also decides on elapsed time and drops nothing. ⚠ The haptic sits **next to the send**, not before it (fired early, it says nothing about whether a menu actually appeared).
  - **Press and hold for a right click (0.8.431, user request)**: the only route to a right click was "double-tap and hold the second tap", which put **the most-used action — opening a menu — the furthest away**. ⇒ **hold one finger still for [`GuiCursor.HOLD_MS`] = 150 ms and the right click is sent on the spot**, with no lift required (the same "the timer sends it" shape as the double-tap route). A single tap stays a left click and a double tap stays a double click. ⭐ **The wait is visible while you hold**: `GuiCursor.holdStart` records `uptimeMillis` and `GuiScreen` sweeps **a green arc around the arrow tip over those 150 ms** (the `withFrameMillis` loop only runs while a finger is down). ⚠ **The duration lives in one place, `GuiCursor.HOLD_MS`** — split between input and drawing, "the ring closed but nothing was pressed" shows up immediately; the double-tap path's `RIGHT_HOLD_MS` points at it too, because **two different waits for the same "hold for a right click" can never be learned by hand**. ⚠ **The ring is sized from screen density, never multiplied by the display scale** (zoomed out it would hide under the finger). ⚠ **This route was removed once in M8-6 T3/T4/T5, so interference is cut off at the entrance**: (1) the second tap of a double tap has `pendingRightClick` set, so no hold starts (`gesture.onTouchEvent` runs first, which is what makes that test work), (2) a second finger discards it, (3) crossing touch slop discards it and leaves plain cursor movement, (4) **once it fires, the same touch's `onSingleTapUp` is swallowed** — with `setIsLongpressEnabled(false)` the GestureDetector reports an UP as a tap **however long the finger stayed down**, and without this a left click would follow every right click.
  - **🖱 switches the pointer mode (0.8.431, user report)**: relative/absolute used to hide behind **a double tap on 📜**. ⛔ It is **visible nowhere on screen, and "command list" has nothing to do with a cursor** ⇒ nobody ever found it. ⇒ **a new `ToolbarButtons.POINTER_MODE` (🖱)** appears as its own button in the GUI toolbar, lit while absolute mode is on. ⭐ **`ToolbarButtonSpec` gained `guiOnly = true`** (the mirror of the existing `terminalOnly`); terminal tabs never show it. ⚠ **`canHide = true`** so Settings › Display › Toolbar can remove it — the §0-B promise. ⚠ The room for this existed because a GUI tab carries only five buttons and **the bar is half empty**.
  - **The ring waits out the tap window; the hold becomes 300ms (0.8.433, user report)**: ⛔ **the ring started the instant a finger landed.** Every tap and every drag flashed it — "annoying". ⭐ **A tap and a hold are different actions, so they must look different too**: starting the hold's visuals on touch-down shows **the face of a long press to a finger that may still be a tap**. ⇒ wait `GuiCursor.HOLD_RING_DELAY_MS` = 150ms (comfortably past Android's `ViewConfiguration.getTapTimeout()` of 100ms) before drawing, then sweep the remaining 150ms. ⚠ **There are now two timers** (show the ring / send the click), so cancelling must remove **both**. ⚠ The drawing side receives the moment the ring started, so progress divides by `HOLD_RING_MS`, not `HOLD_MS`. ⭐ **The hold went from 150ms to 300ms** (the user's number). The double-tap path's `RIGHT_HOLD_MS` points at the same constant and grows with it, which **doubles the time available to start a drag** — the trade-off written down in 0.8.429 ("shorter makes the right click easier and the drag harder"), swinging back the other way.
  - **Unicode has no arrow-cursor glyph (0.8.433, reverted to 🖱 in 0.8.434)**: the report was right — 🖱 draws **the device**, not the thing being switched (the cursor on screen). `↖` (U+2196 + VS16) was tried and rejected on the device: **it is a direction arrow, not a pointer**. ⛔ **No emoji for the arrow cursor exists** (🖱 U+1F5B1 is the device; 🖰/🖯 are absent from the device fonts). ⇒ **back to 🖱.** ⭐ **The toolbar draws icons as text**, so the choice is limited to glyphs the font has; changing that needs icons drawn as vectors instead of characters (in both `ToolbarChip` and the settings `CATALOG` preview). ⚠ When a glyph does change, change **the `CATALOG` icon and the one the toolbar draws together** — one alone and the settings screen and the toolbar disagree.
  - **The double-tap right click is gone (0.8.435, user report: "the double tap decides and I cannot drag")**: ⛔ **a decision made on a timer was eating the drag that came after it.** While the second tap was held, `RIGHT_HOLD_MS` sent a right click and cleared `pendingRightClick`, so **moving afterwards could no longer become a left drag** (MOVE fell through to plain cursor movement). ⇒ **the second tap now only distinguishes "move = drag" from "release = click"; no timer is armed.** ⭐ **With the deadline gone you can hold still, take aim, and then move** — the behaviour 0.8.427 was after, reached the long way round. ⭐ **Right-clicking is now solely the one-finger press and hold (0.8.431)**: when one action has two routes and one of them breaks a different action (dragging), **the breaking route is the one to delete**. ⚠ This retires `RIGHT_HOLD_MS`, `rightClickRunnable` and `rightClickFired` (the last of which was **dead state nothing ever read**). ⭐ **Lesson: "fire it automatically when the time is up" is only safe when the gesture ends there.** Put it at the entrance to something longer (a drag) and waiting becomes a dead end.
  - **The first tap's click is held back (0.8.436, user report: "unless I pause after the double tap, the drag misbehaves")**: removing the timer in 0.8.435 was not enough. ⛔ **The cause was not our decision logic but the order of what reached the server.** The first tap **actually sends a left click**, so a drag that follows looks like **a double click that was then dragged** (click, then a press a few pixels away well inside the double-click time). Pausing pushes the press outside that time, which is exactly why "pausing works". ⇒ **hold the tap's left click for `doubleTapMs` (`ViewConfiguration.getDoubleTapTimeout()`)**: (1) a second finger **discards** it (the gesture becomes a drag or a double click), (2) an unrelated tap **flushes it immediately** (never lose a click), (3) otherwise it fires on timeout. ⭐ **A double click is built by sending two left clicks when the second tap is released** (the first one having been discarded). ⚠ **The cost is that a click lands `doubleTapMs` later.** Nothing else keeps "click then press" from falling inside the server's double-click window. ⛔ **Do not fake it with distance** — that depends on the server's threshold (≈5px in GTK/Qt) and the zoom factor, so it comes back whenever those line up. ⭐ **Cursor tracking is not delayed**; only the button press waits. ⚠ **Tap detection moved off `GestureDetector` into this View**: "how long the click is held" and "how long a double tap is accepted" must be **one single duration**, and two mechanisms holding it separately drift apart. It also escapes AOSP's quirk of handing `onDoubleTap` **the first tap's down coordinates**.
  - **Two cursors on screen (0.8.431, user report)**: the cause was that **`SetEncodings` never advertised a cursor pseudo-encoding**. In RFB, **unless the client asks for Cursor (-239) / XCursor (-240) the server composites the pointer into the framebuffer**; our own arrow is always drawn on top, so the two sat side by side. ⇒ **add them to the request** (a server that honours it stops compositing and sends the shape as a pseudo-rect instead). ⚠ **Asking brings the duty to skip the bytes exactly**: `Cursor` is `w*h*4 + ((w+7)/8)*h` bytes, `XCursor` is `6 + ((w+7)/8)*h*2`. One byte off and the stream is corrupt and the screen freezes. `w*h == 0` means "no cursor" and carries no body. ⚠ **These are not drawing rects**, so they stay out of the bounding-box calculation too. ⭐ **The shape is discarded and our own arrow keeps being drawn** — a remote 24px cursor is too small to aim at with a finger once the picture is scaled down, while an arrow sized from screen density stays usable on a phone. A server that ignores the request still composites its pointer next to our arrow, which is exactly the state before the request was added — not a regression.
...
- **Sharing the clipboard with the far side (0.8.475)**: while the GUI tab is in front, text copied on Android is sent to the far side and text copied there is pulled into Android. The outbound door is `RemoteDesktopClient.sendClipboardText`, which defaults to doing nothing so implementations without clipboard support need no changes.
  ...
  - **RDP (CLIPRDR)**: request the `cliprdr` static virtual channel through GCC `CS_NET`, lay down MCS channel routing (`RdpActivation.channelData` / `RdpMcs.sendVirtualChannel`), and speak **the text and file** parts of [MS-RDPECLIP] (`gui/rdp/RdpCliprdr.kt`). Only `CF_UNICODETEXT` and `FileGroupDescriptorW` are advertised — no HTML, no image formats (advertising what you cannot accept makes the server send it). Bodies over 16KB are chunked with `CHANNEL_FLAG_FIRST` / `CHANNEL_FLAG_LAST` (`RdpChannelReassembler`).
  - **Clipboard files (0.8.482)**: `CB_STREAM_FILECLIP_ENABLED` + `CB_FILECLIP_NO_FILE_PATHS` transfer **contents rather than paths**, so our filesystem is never exposed to the peer. ⛔ **Never wait mid-transfer** — this runs on the receive loop, so "request and wait for the response" stops all of RDP. ⇒ It is a state machine that **requests the next chunk when a response arrives**.
    - ⛔⛔ **A copy on the peer must not pull the contents (0.8.483).** Copying there often stays there (moving a file between folders, say); fetching every time wastes both bandwidth and storage. ⇒ Take **only the list** (`FILEDESCRIPTORW`, 592 bytes each) to show what is on offer, and fetch the contents **only once the user decides to accept** (`RdpCliprdr.receiveOfferedFiles`). ⭐ **This is RDP's own convention** — when we hand files the other way, the peer asks for the bytes only at the moment of pasting. The accept affordance is a thin strip along the bottom of the GUI screen (`GuiScreen.ClipboardFileBar`) that simply disappears when ignored, so no extra step is imposed.
    - **📎 picks files on the phone and hands them over (0.8.484).** ⛔ **Do not wait for the system clipboard** — on Android a "copy" in a file manager often **stays inside that app and never puts a `content://` on the system clipboard**, so watching `ClipData` leaves some phones with no way to send anything. ⇒ Put 📎 in the GUI toolbar (`ToolbarButtons.CLIPBOARD_FILE`, **GUI tabs only**), open `OpenMultipleDocuments`, and turn the chosen URIs straight into a `ClipboardFiles.Source` (`ClipboardFileTransfer.fromUris`). ⭐ The button only appears for clients whose `RemoteDesktopClient.supportsClipboardFiles` is true (never on VNC). ⚠ **Some providers do not report a size** — `FILEDESCRIPTORW` needs one, so fall back from `OpenableColumns.SIZE` to the length of `openAssetFileDescriptor`; if neither answers, drop that one file rather than claiming zero.
    - ⛔⛔ **Never round down the number of bytes the peer asked for (0.8.484).** A `FILECONTENTS_RANGE` reply was being `coerceIn`-ed to **the receiving side's own step, `CHUNK_BYTES` (64 KiB)**. Windows asks for more than 64 KiB at a time and **reads a short successful reply as EOF**, so large files finished pasting truncated. ⇒ **Return exactly the requested length.** ⭐ The step size is ours to choose only **when we are the one fetching**; it is not ours to impose on the peer's request. ⚠ To avoid unbounded reads, cap at `MAX_OUTGOING_CHUNK_BYTES` (4 MiB) and answer `CB_RESPONSE_FAIL` for anything past it, for a **position outside the file**, or for invalid flags (e.g. `SIZE` and `RANGE` together). ⛔ **"Less is left than was asked for" is not a reason to refuse** — refusing there dropped the final partial chunk (see 0.8.486).
    - ⛔ **One clipboard message must not drop the RDP session (0.8.484).** CLIPRDR is **an optional channel, independent of screen, input and audio**, yet an exception out of `acceptChannelChunk` took down the receive loop and **the desktop with it**. ⇒ Catch it in `acceptChannelChunkSafely`, reset `RdpChannelReassembler` to **throw away the partial reassembly, and fail only the transfer in flight** (`abortIncoming`). ⭐ The next well-formed PDU resumes as if nothing happened.
    - ⛔⛔ **Offering the list alone leaves the peer unable to paste (0.8.485, reported as "the transfer failed with an error").** The Format List carried only `FileGroupDescriptorW`. Windows treats the clipboard as files only when **both "names and sizes" (`FileGroupDescriptorW`) and "contents" (`FileContents`) are announced together**; with one of them missing there is nowhere for the bytes to come from at paste time, and the peer reports an error. ⇒ **Announce both whenever we offer files.** ⚠ The bytes themselves never travel in a Format Data Response (they arrive as `FILECONTENTS_REQUEST`); listing the name is how we claim we can serve that format.
    - ⛔⛔ **Fetching later means locking first (0.8.485, reported as "receiving to the phone fails too").** 0.8.483 stopped requesting contents until [Receive] is pressed, but never told the peer. **A peer that has sent a Format List is free to drop that data unless asked to keep it** ([MS-RDPECLIP] Lock Clipboard Data). ⇒ **While the list is on screen, ask with `CB_LOCK_CLIPDATA` and carry the `clipDataId` in every `FILECONTENTS_REQUEST`** (24 → 28 bytes). Always release with `CB_UNLOCK_CLIPDATA` — after the last file, when the list is replaced, when ✕ dismisses it, and on close — or the peer holds the data forever. ⚠ **Never send a lock to a peer that did not advertise `CB_CAN_LOCK_CLIPDATA`**: read its General Capability Set to decide (until now `CB_CLIP_CAPS` was discarded).
    - **The receive strip can be closed with ✕ (0.8.485).** It disappears on its own, but it occupies the bottom edge until then. ⭐ **Closing it also releases the lock** — nothing we have declined should stay pinned on the peer.
    - ⛔⛔ **Never refuse the final partial chunk (0.8.486, from the device log).** The range check added in 0.8.484 treated "less is left than was asked for" as out of range and answered `CB_RESPONSE_FAIL`. The peer keeps asking in the **same size to the very end (256 KiB in practice)**, so a 25 MB file succeeded 96 times and then failed on the last one, taking the whole transfer with it. ⇒ **Serve whatever is left.** ⭐ A successful reply shorter than the request **is** how EOF is expressed; what must never be shortened is a chunk **in the middle** of the file (do not conflate this with 0.8.484). Refuse only when the position itself lies outside the file.
    - ⛔⛔ **One message can carry two or more PDUs (0.8.486, logged as "invalid CLIPRDR message length").** A peer may batch several PDUs into a single channel write. Looking only at the first one and declaring the length inconsistent **discarded a valid response and dropped the whole message with it** — which is why receiving always failed. ⇒ **Walk the buffer and handle each PDU in turn.** ⚠ When a PDU cannot be cut out, log its first 32 bytes in hex; without them there is no way to tell where the framing broke.
    - ⭐ **Files win over text.** Copying a file on Windows also announces **a string pointing at where it lives**, so taking the text first delivers a path instead of the contents. ⚠ `FileGroupDescriptorW` is a registered format, so **its id differs per peer** — look it up by name.
    - ⚠ Directories (`FILE_ATTRIBUTE_DIRECTORY`) are skipped and names are reduced to the leaf (copying a folder delivers `sub\name`). ⚠ **An empty response means "no more"**, so anything short of the declared size is finished as incomplete (never leave a half-written file behind).
    - **They land in "Download / z2term"** (`clipboard/ClipboardFileTransfer.kt`, `MediaStore`). ⭐ **Never ask where to save**: choosing every time stalls each transfer and loses track of where things went. The Downloads folder is visible from any other app's file picker and needs no permission. Whatever is saved also goes onto the Android clipboard as a `content://` URI, so apps that accept one can paste it directly. ⚠ Files stay `IS_PENDING` until fully written, and a transfer that breaks is deleted.
    - ⛔ **Never do the saving I/O on the receive thread** (same reason as audio); the `ClipboardFiles.Sink` implementation hands it to one worker thread.
...
**Sample panel (0.8.601; a guide command from 0.8.603)**: Turning panels on never creates a panel (the user's decision). 0.8.601–0.8.602 created `main` and opened it when no panel existed; `EdgeStore.ensureInitialPanel` is removed. The `edge-panel` guide creates the sample. When the guide is shown, `EdgeDefaultPanel.command` (through `GuideStep.commandOf`) builds one line joining the existing `z2-edge panel main …` and one `z2-edge set main:app_N …` per item with `&&`. No subcommand is added. Words containing anything other than ASCII letters, digits and `_@%+=:,./-` are single-quoted. A step whose line cannot be built is left out of the guide. Fields come from `EdgeDefaultPanel.fields` (`handle=bar side=right offset=31.5 size=8 length=8 alpha=0.3 width=14% height=47% flow=vertical labels=off add=on settings=on`, `scroll-variable` for `actions-up`/`actions-down`, `gesture-speed=40000 gesture-range=400`), with `label` from `edge_default_panel`. Items are resolved when the guide is shown, in order: this app, `ACTION_VIEW https` (BROWSABLE), `STILL_IMAGE_CAMERA`, `ACTION_DIAL`, `SENDTO smsto:` and `ACTION_SETTINGS`. The chosen default is used; without one, a single system candidate is used. Only launchable packages are kept, deduplicated by package. `z2-intent -p PKG` gains `--window freeform` only on freeform-capable devices (`FEATURE_FREEFORM_WINDOW_MANAGEMENT` or `enable_freeform_support=1`). If `main` exists, the given keys and `app_N` items are overwritten and everything else is kept. The `edge-panel` guide runs `z2-edge permission`, `z2-key permission`, the sample panel line and `z2-edge on`, then shows a read-only card about long-pressing outside the menu to open settings. Without `open`, the bar opens by swiping inward.

**ON/OFF buttons (0.8.589)**: `type=run button-state=on` adds state display to application and macro run items. ON uses a thicker accent border and coloured background; OFF uses a subtle border and transparent background. State adds no text or check marks to the menu and preserves the icon/label layout, including icon-only layouts. Accessibility still receives a state description. `run` is the ON action; `off` is the OFF action and defaults to the same command. Optional `state` queries run on opening, after successful actions and at the `every` interval, accepting only `on/off`, `true/false` or `1/0`. Without a query, only exit-code-zero actions flip the remembered state; failures and timeouts retain it. This memory does not represent external changes or application liveness. Macros may report observations with `z2-edge state PANEL:ITEM on|off`; older queries/actions cannot overwrite newer reports. Per-item `.button-state` files survive restart and are invalidated by changes to run/OFF/query commands. Results from changed or deleted definitions are rejected. Ordinary run items remain stateless by default.

**State synchronization and automatic OFF (0.8.591)**: Choose **State source** below Show ON/OFF. Automatic follows the actual state of directly assigned `z2-torch` and `z2-screen keepon DURATION` commands. Torch observations include changes made through Quick Settings and other applications. Screen keep-on follows expiration, cancellation and deadline changes, including when the panel was closed. API operations inside macros also record the last supported feature as their source. Select Flashlight or Prevent screen sleep explicitly to synchronize before the first run or when a macro controls several features. ON while running; OFF on exit clears the state on completion, failure, timeout or stop; tapping a running button stops it. For detached background work or custom work started elsewhere, configure a state query and refresh interval. An explicit state query takes priority. No state text or check marks are added to menus. CLI: `button-source=auto|torch|screen|process|remember`. Learned sources are stored with the item definition signature. API metadata carries an action token; completed/superseded tokens and changed/deleted definitions are rejected. Process state comes from active executions and is never restored as ON after app restart. Torch selects a rear flash when available and remains unknown until the initial Android observation. `z2-torch status` reads actual state. Direct torch Quick Settings assignments use the same observations. If the OFF command is omitted, tapping an active direct torch assignment turns the flashlight off.
...
**Action automation aligned with the command sheet (`ActionUi`, 0.8.603)**: At the user's request it leaves `EdgeSettingsUi` for `ActionUi`, which draws the sizes and colours of `WhenRulesBody` while staying on Views; saving, drafts, app lock and returning from coordinate picking are unchanged. The body uses 16dp sides, 10dp above, 24dp below and 10dp between items. The heading is 18sp green monospace (16sp while editing), with `PillButton`-style New and Close on the right. Each macro is one bordered row (bgCard, 1dp border, 8dp corners) with its name in 13sp monospace and `IconCell`-style ▶, Duplicate, ✎ and ✕ at the right edge; the row itself no longer opens the editor. Duplicate keeps its word because no glyph for it reads reliably in every font. Steps are bordered rows too, with the number column, the monospace line and ⋮; nesting stays a left rule and indent. Hints use the `HintBox` look, inputs the `Field` look, and pickers sit in the same frame with ▾. Confirmation, step editing and pickers replace platform `AlertDialog` with `ActionUi.modal` and `ActionUi.choices`, drawn like `ConfirmDialog` (bgCard ground, monospace, text buttons at the lower right). Cancel, Back and an outside tap go through `cancel`, which also drops the step draft. ⚠ `EdgeSettingsUi` stays as it is, because the edge panel editor uses it. The floating coordinate-picking bar (`ActionCoordinatePicker`) is out of scope. The Action automation / Automation rules subtab labels are monospace too.
...
**Grouped item editor and bulk panel removal (0.8.620)**: what is saved, how it maps to the CLI and how unsaved drafts are handled are unchanged; only the arrangement and opening/closing changed (user feedback: the open editor was hard to read, could not be closed, and panels could not be deleted as a whole).
- **The item editor is split into named blocks** (`EdgeSettingsUi.group`): Component and behavior (App launch for app items) → Display (label, icon, rows, note colours, lines) → Command (run, off, state, state source, on select, QR) → Value (default, choices, required) → Connections (values to pass, standard input, result display). A block name sits under a rule in bold so it is not mistaken for a field name (small, quiet type). **A block with no field for the chosen component and behaviour is hidden, name and all.** Value sources such as inputs and result boxes show a bordered note that connections are made on the button.
- **Rarely used settings fold at the foot** (`EdgeSettingsUi.fold`): Advanced settings and Item size and position. Their headers share the block edge and type instead of adding an indented second level.
- **An open editor shows which item it belongs to**: an accent rule down its start edge (`EdgeSettingsUi.attached`) and a tinted row (`opened`). Cancel and Save sit together under a rule.
- **Edit opens the editor and the same button closes it**: while open it reads Close, sized to the longer of the two labels so the row does not shift. Tapping the row does the same. Closing is cancelling and asks before discarding unsaved changes (`EdgeEditorSession.discard`); a closed draft is removed from tracking with `untrack`. Add custom slot toggles the same way.
- **Manage › Remove panel or tab uses checkboxes**: every panel is listed (the one being edited first, marked Current) with its tabs indented below. Ticking a panel ticks and locks its tabs and deletes the whole panel. Ticking only a tab deletes that tab and keeps its parent and siblings. **The first tab is the panel itself, so it cannot be ticked on its own.** The delete button shows the count and is disabled at zero; pressing it lists what will be deleted in a red frame, and confirming calls `EdgeStore.removePanels`. Changing the selection closes the confirmation.
  - ⚠ `removePanels` **deletes tabs before parents**, so a failure part-way leaves the parent with a consistent tab list (deleting the parent first would leave the remaining tabs behind as separate panels). An unknown ID deletes nothing. Up to 0.8.619, Delete current panel or tab removed one entry with `removePanel`, which keeps a parent's tabs, so a panel could never be deleted as a whole.
...
**Capturing only the part you draw around (`z2-shot`, 0.8.622)**: handled by `RegionShot`. The order is **hide the panel and handle → wait for that to reach the screen → take one screen image → draw on that still picture → lift to confirm → preview → save or share**. Drawing always happens on the captured still image, never on the live screen: a live screen shifts under the selection as it scrolls or redraws, and the selection overlay itself would be captured. `EdgeRuntime.suspendForActions(true)` hides both the panel and its handle, and `AccessibilityService.takeScreenshot` runs three `Choreographer` frames later — right after `GONE` the composition has not caught up and the panel is still in the image, so the code counts drawing boundaries rather than guessing a delay. Every path (done, cancelled, failed, thrown) restores the views. Free, rectangle and oval selections all become one `Path` cropped to its bounding box, so a single output path serves all three. Transparency composites an `ALPHA_8` mask with `DST_IN` instead of `clipPath`, which would leave stepped edges on ovals and diagonals. Coordinates are kept in image pixels and only stretched for display: overlay and screen dimensions can differ with cutouts and rotation, and sharing one transform is what keeps the saved region where it appeared. Saving writes a PNG to `Pictures/z2term` through `MediaStore` with `IS_PENDING` held until the bytes are complete; sharing reuses the 0.8.608 `shared-files` cache and `FileProvider`. The checkerboard is preview-only and never baked into the file. Requires Android 11 (`takeScreenshot`) and an enabled accessibility service; requests are arbitrated against automatic bar-colour sampling to avoid the capture-interval error.
...
`MainActivity.handleShareIntent` accepts ACTION_SEND and ACTION_SEND_MULTIPLE. `SharedIntakeModel` calls `SharedIntake.intakeFrom` on IO, saving body, subject and attachments as one receipt under `shared_home/z2term-inbox/UUID/`, with a `manifest.json`. Identical EXTRA_STREAM/ClipData URIs are imported once. Attachments must be content URIs, up to 32 files and 512 MiB each. Any failure rolls back the entire receipt. Filenames lose separators/control characters, fit within 220 UTF-8 bytes, and receive numeric suffixes on collision.

If snippets with `shareAction=true` exist, an unlocked dialog offers the body, filenames and registered actions. A selected snippet can open its argument form, then receives the original body and absolute manifest path through environment variables. Without registered actions, the original insertion continues: body when present, otherwise space-separated `"$HOME"/'z2term-inbox/UUID/name'` arguments.

`SessionManager.insertText` is the shared insertion endpoint. It leaves the clipboard alone and appends no Enter. Existing share automation rules run once after import, independently of manual processing. Combined `mixed` receipts match text and file conditions; contains checks body text, while ext checks attachment names.

The activity is singleTask. The intent marker and ViewModel prevent duplicate import on rotation and retain pending choices across rotation. Pending UI choices are not restored after process death; saved receipts remain. See [Sharing and command input](SHARE-WORKFLOW.md) for outgoing snapshots, FileProvider read grants, receipt format and snippet placeholders. Sending allows 32 files and 512 MiB combined, with copies older than 24 hours cleaned on the next send.
...
- **Flicking looks the same as on the kana face** (`FlickCommitPopup`, 0.8.405): while a key is held,
  the character that would be committed on release is shown large, black-on-green, just above the key.
  ⚠ **Until then the alphabet face instead enlarged its own on-key hint by 1.6×**, so the two faces
...
**0.8.618 correction**: Letter presets still carried `repeatable=false` after the widget merge. Letters, their uppercase layers and symbols now enable repeat. Flicking before repeat starts still cancels it. Saved custom layouts are unchanged.
...
- **Dynamic block segmentation (learned)** (0.8.71): block boundaries are not fixed by dictionary cost alone — they are learned from how often the user confirms a given reading-block. `ImeHistoryStore.learnedBlock(reading)` returns `(top surface, cost reduction)` for confirmed `(reading → surface)` pairs (wired into `KkcConverter.learnedBlock`); during `nbest` lattice construction, any reading ≥2 chars matching a learned block has its node cost reduced (`BLOCK_BASE_BONUS=3000` + `count`-scaled `BLOCK_COUNT_STEP=1500` (capped at count 4) + recent `BLOCK_RECENT_BONUS=1000`). This overcomes the katakana penalty + connection costs after 1–2 confirmations, so a mis-split frequent reading auto-merges into a single block from then on. Even readings absent from the dictionary get a synthesized node (`lc=rc=0`) from the learned surface (unlearned readings behave unchanged). **The cost reduction applies only to the surface the user actually confirmed** (0.8.74), because applying it uniformly to every surface of the reading let the dictionary-cheapest surface win and the user's chosen kanji was not reflected. **Score merged readings at one-word cost and learn consecutively-confirmed runs as merged blocks** (0.8.85): the learned-block synthesized node cost uses a one-word `UNK_COST` baseline instead of length-scaled unknown kana, and `ComposingState` accumulates consecutive confirmations within one split run into `committedRun`, recording the merged reading → merged surface when the run ends (`learnMergedRun`) and on batch confirm (bounded to reading length 2…`MERGE_MAX_READING_LEN`=6).
  - ⚠ **The synthesized node's cost baseline was decoupled from `UNK_COST` (0.8.493).** It used to be `UNK_COST(17000) − bonus`, so even at the maximum bonus (8500) the node still cost 8500 and **could never win against readings the dictionary happens to segment cheaply (measured: total path cost in the 6000s), no matter how many times they were confirmed**. Learning was recorded but the segmentation never changed, forcing the user to re-split the same phrasing by hand every time (user report). A learned block *is* the evidence that the user chose that surface at that boundary, so the baseline is now `LEARNED_BLOCK_COST=9000` — on par with an ordinary dictionary word rather than an unknown-word estimate — minus the bonus (6000–5000 at count=1, 4500 at count=2).
  - ⚠ **A floor of `LEARNED_BLOCK_MIN_COST=4000` is mandatory; never go below it.** The synthesized node approximates context with `lc=rc=0` (BOS/EOS), so it **cannot judge whether a connection makes sense and attaches cheaply anywhere**. Too cheap and a learned chunk cuts **into the middle of other words** that contain its reading as a prefix, wrecking readings the dictionary converted correctly as a single word (measured: learning one 4-character chunk broke every other word starting with it). The measured safe band is an **effective cost of 4000–6000** — below 4000 it intrudes, at 7000 and above learning has no effect. Heavy use saturates at 4000, i.e. **as strong as it can be without adding misconversions**. `BlockLearningTest` pins both halves: a chunk merges after a single confirmation, and it never cuts into other words.
  - ⚠ **Building a whole-sentence candidate from the learned boundaries was tried and removed (0.8.493).** Greedy longest-match over learned chunks ignores context, so whatever region is left over gets force-converted even when it is meaningless, **pushing an unusable candidate to the top**. Boundary learning stays entirely on the lattice (cost) side, where connection costs always get a say.
...
- `SshProfile` remains the persisted record name for migration compatibility, but each record has `protocol = SSH | WEBDAV | SMB`. Missing/unknown protocol values fall back to SSH, so existing DataStore and backup JSON stay readable. `remotePath` is the SMB share and `domain` is its optional auth domain; all three protocols reuse the encrypted `password` field.
...
- **Bare hosts accept DNS, IPv4 and IPv6 (0.8.457).** `HostAddress` strips optional surrounding brackets before connection and adds them back only when forming an authority (`[2001:db8::1]:22`). The same rule is used by SSH/JSch, port forwarding, SMB, WebDAV Host headers, VNC labels and known_hosts keys; otherwise an IPv6 colon is indistinguishable from the port separator. Public IPv6 must not match NetGuard's “single-label LAN name” exemption merely because it has no dot.
...
### 6.3.1 A remote screen (VNC / RDP, A1, 0.8.418 / 0.8.459)

- **One host, one entry.** A single `SshProfile` row offers shell (Connect), files (SFTP) and **the screen (VNC, RDP)**. **No new tab and no new button** — the row's actions were split over two lines. Screen services ride on the same `RemoteService` as FTP / SMB / WebDAV, and the single `RemoteServiceProtocol.opensDesktopTab` flag decides "GUI tab or file browser" so callers never enumerate protocol names.
- **[VNC] and [RDP] call `RemoteServiceConnector.desktopTarget()` → `SessionManager.openRemoteDesktop()`** → a `GuiSession` in remote mode (see 4.12). By default the connection rides a **temporary local forward over the same SSH host** (`ServiceRoute`), and closing the tab removes the forward. Turning off "SSH port forwarding" connects straight to the SSH destination host instead, after a warning that encryption is lost.
...
**What is specific to RDP (0.8.459-0.8.472)**

- **Only Network Level Authentication is spoken.** CredSSP + NTLMv2, so a user name and password are required (domain optional, stored in the same `RemoteService.domain` as SMB). If the server does not select NLA, `RdpNlaUnsupportedException` produces **advice that names the cause** — a typo and "the host is configured differently" are fixed in different ways, so they must not collapse into one "connection failed".
- ⭐ **This side decides the desktop size.** RFB **looks at a screen that already exists**, so the server's resolution is taken as given; RDP **creates a new session on every connection**, so without a request you get the host's default (1024x768 or similar). `RdpTarget.fitDesktopSize` derives **landscape (long edge as width), multiples of 4, clamped to 640-4096** from the device's pixel size: GUI tabs are used in landscape, a leftover 1-3 px edge never repaints, and a size the server would reject is never asked for. ⚠ This does not contradict never sending VNC's `requestResize` (which would change someone's real screen) — the two situations are different. **Since 0.8.480 it also follows the frame after connecting**: rotating the device requests the same rounded size over Display Control and the peer rebuilds its session (see "Display Control" in §4). ⚠ The screen blanks while it is rebuilt, so **the same size is never re-sent**.
- **The certificate is confirmed once per host and remembered** (`RdpCertificateTrust`, the same idea as SSH's known_hosts). RDP certificates are usually self-signed, so system CAs cannot verify them, yet accepting everything would hide a man in the middle. ⇒ **show the SHA-256 fingerprint on first use, then require a match** (ask again if it ever changes). ⚠⚠ **The name it is remembered under is the destination before SSH forwarding** (`RdpTarget.trustKey`). Remember it as the forwarded `127.0.0.1:<port that changes every time>` and **every connection is a first connection**, which makes the confirmation meaningless. The dialog reuses SSH's `HostKeyVerifier` with only the heading swapped, so there is no second screen to learn.
- ⚠⚠ **A TLS 1.3 / ECDHE handshake cannot complete against the certificate Windows generates for RDP (0.8.460, found on-device).** Its `keyUsage` is **`keyEncipherment, dataEncipherment` only — no `digitalSignature`** — yet TLS 1.3 and ECDHE have **the server sign with that key**. Android's TLS stack rejects the combination as the spec requires: `KEY_USAGE_BIT_INCORRECT`. ⚠ **OpenSSL on a PC overlooks the same contradiction**, so it shows up in the least suspicious way possible: **the very same host connects from the PC but not from the device** (diagnose it by reading the certificate's `keyUsage`, not by "does it work from the PC"). ⇒ **Only when the certificate is known to forbid signing** does the handshake get retried once, restricted to **RSA key exchange over TLS 1.2**, which asks for no signature (`RdpTlsTransport.signingIsForbidden` → `restrictToRsaKeyExchange`). ⛔ **Never restrict unconditionally** — that costs forward secrecy against hosts with a proper certificate. ⛔ **Never decide from the exception's message text**; the evidence is the certificate the server presented. ⭐ Credentials stay protected even with RSA key exchange: CredSSP binds the TLS public key with the NTLM session key (`CredSspBinding`), so a man in the middle is detected by the mismatch.
- ⚠⚠⚠ **Passing against a permissive server proves nothing (0.8.461-0.8.472, each found against real Windows).** The in-device test bed (`scripts/rdp-testbed.sh`) accepts everything, so all of it "worked" there while real Windows surfaced seven defects one after another. ⭐ **Never treat "one peer accepted it" as evidence for a protocol implementation.**
  - **Licensing may never arrive** — a host that needs no license skips the License Error PDU and goes straight to Demand Active. Requiring it means never connecting to such a host.
  - **Detect the security header by length, not by flag bits** — over TLS an ordinary PDU carries no security header. ⛔ Testing flag bits misreads a screen update as licensing whenever a Share Control PDU's totalLength happens to contain `SEC_LICENSE_PKT` (0x80) (`isShareControlPdu`).
  - **Write Client Info to the end** — stopping `TS_EXTENDED_INFO_PACKET` after clientAddress makes Windows refuse with `errorInfo 0x1118` (SECURITYDATATOOSHORT9). The 172-byte time zone block is required.
  - **Never validate against `uncompressedLength`** — some implementations put the whole PDU length there, others only the body; matching one guarantees rejecting the other.
  - **`errorInfo = 0` means no error** — Windows sends exactly one right after the connection settles. Throwing without reading the value **disconnects the moment it works**.
  - **Never discard the server's complaint** — handle Set Error Info in all **three** places (connection sequence, finalization, receive loop). Skipping it anywhere turns a stated reason into an unexplained EOF.
  - **Do not omit mandatory capabilities** — `ZEROBOUNDSDELTASSUPPORT` in `orderFlags` is required even when no drawing order is used, and Offscreen Bitmap Cache must be sent (declaring no support) rather than left out.
  - **Do not contradict yourself** — advertising `RNS_UD_CS_WANT_32BPP_SESSION` while claiming only 24/16/15bpp leaves the server unable to choose a format.
- ⛔⛔ **Resolved in 0.8.477-0.8.478: the connection completed but not one byte of screen data arrived.** The symptom was "NLA → MCS → activate → finalization all complete, the tab reaches `CONNECTED` at `2400x1080`, Save Session Info (`pduType2=0x26`) arrives, and then `readTpkt` blocks without reading a single byte" (not fast-path either — that would fail the TPKT header check, and requesting a full-screen Refresh Rect with `refreshRectSupport=1` changed nothing). ⇒ The cause was **not a defect on our receiving side: Windows 11 does not send legacy Bitmap Updates to a peer that cannot do RDPGFX.** Implementing `drdynvc` + RDPGFX + RemoteFX brought the screen up (see "RDPGFX" and "RemoteFX" above). ⭐ **That conclusion came from diffing against an `xfreerdp3` TRACE, not from guessing** (point `scripts/rdp-testbed.sh trace` at the host; needs its credentials). Measuring that **`xfreerdp3` with GFX disabled goes equally silent** settled that it is a property of the sender. See `99_private/HANDOFF/z2term/RDP-HANDOFF.md`.
- **Slow-path input**: `RdpInput` converts the shared GUI input into TS_INPUT_PDU_DATA. It turns the pointer's current button mask into Move / Press / Release differences and sends wheel motion as a signed 9-bit rotation. Function and modifier keys, plus Ctrl/Alt/Meta shortcuts, use scancodes; ordinary text uses Unicode Keyboard Events so it does not depend on the peer's keyboard layout. `RdpClient` moves writes to one sender thread and serializes them with CLIPRDR through `RdpTlsTransport.writeLock`.
...
- **The settings live in the "Keyboard and input" group** (0.8.277). In 0.8.276 they sat under
  "Permissions and notifications", where nobody looking for keyboard settings would find them.
...
`noInstallTimeout` (disable install timeout), etc. are also kept in DataStore (`z2term_settings`). SSH profiles are saved as JSON in a separate DataStore (`z2term_ssh`).
...
- **The F-Droid build strips the signing config mechanically (0.8.414).** The build server deletes the `signingConfigs { ... }` block and any `signingConfig = <expression without spaces>` line before running `assembleRelease` (F-Droid signs the APK itself). So **never spread that assignment over two lines with `?:` inside `release { }`** — only the first line is removed, the `?:` line is left orphaned, and the build dies with a Kotlin syntax error. Resolve it outside `buildTypes` in `val releaseSigningConfig` and keep a single-line assignment inside `release`. The submission steps and the other compliance points (scanignore, how the NDK is passed) are in `docs/FDROID.md`.
- **`useLegacyPackaging=true` is required** (so the `.so` files that get execve'd are placed as real files in nativeLibraryDir).
- **`scripts/z2c` (OCI image fetch)**: fetches an image from an OCI/Docker registry (Docker Hub, ghcr.io, …) and merges its layers following the whiteout (`.wh.*`) and opaque (`.wh..wh..opq`) conventions into a rootfs. Token auth and manifest retrieval are plain HTTPS GETs and need no privilege; layers are cached per digest under `~/.z2c/cache` and shared between images. Subcommands: `pull` / `ls` / `verify` / `run` / `path` / `sh` / `rm`. `run` execs with `Z2ROOT_ROOTFS` set, switching the rootfs (= entering the container; see "Per-tracee rootfs" in chapter 4 — **it only actually works on 0.8.417 and later**: the 0.8.416 z2root accepted host paths only and did not match the guest-visible path z2c passes). It honours the image's `entrypoint`/`cmd`/`env` and resolves the command against the PATH *inside* the rootfs (resolving it outside would launch the host's copy). Images of another architecture cannot be `run` (z2root does not insert an emulator); use `sh` for those. **Fetch and extract only — isolated launch is not supported** (see the nesting constraint in 10.1). `verify` calls the rootfs's own loader (`ld-musl-*` / `ld-linux-*`) to run a binary from inside it and confirm the merge is usable. `sh` takes the same path, so **the rootfs is not switched** (absolute paths still resolve outside) — treat it as an inspection aid. **Other architectures are supported too** (`z2c pull --arch amd64 …`, stored with an `_amd64` suffix); there `verify`/`sh` invoke an emulator (`qemu-x86_64` etc., from the `qemu-user` package) with `-L <rootfs>` instead of the loader. Measured at roughly 11x slower than native (20k-iteration busybox arithmetic loop: 0.10s → 1.09s). `LD_PRELOAD` **must be cleared** when running — see the z2accept item in chapter 4.
