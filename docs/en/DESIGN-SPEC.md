...
Last updated: 2026-08-30 / Target version: 0.8.450-alpha (versionCode 458)
...
- **RDP is not yet a user-facing feature (the 0.8.450 NLA spike)**: `gui/rdp/` now implements X.224 Enhanced Security negotiation (requesting TLS + CredSSP), TLS upgrade on the same TCP connection, an explicit certificate-fingerprint approval boundary, CredSSP `TSRequest` ASN.1 DER, and the NTLMv2 MD4/HMAC-MD5 response and session-base-key primitives without adding a dependency. ⚠ The connection UI, the three NTLM messages/SPNEGO and sealing, the public-key-binding exchange, MCS, display updates and input are not wired yet, so no `[RDP]` action is shown at this point.
