...
Last updated: 2026-08-29 / Target version: 0.8.433-alpha (versionCode 441)
...
- **Remote VNC (A1, 0.8.418)**: `GuiSession(remote = VncTarget(host, port, password, name))` turns the tab into **nothing but an RFB connection to an outside server** — no z2gui, no proot. Drawing, input, the keyboard, the clipboard and zoom/pan all sit on the same `RfbClient` / `GuiScreen` / `GuiInputView` as the local GUI, so **what is new is only "a way to point somewhere else" and "VNC authentication"**.
  ...
  - **The icon is a pointer, not a mouse (0.8.433, user report)**: 🖱 draws **the device**, not the thing being switched (the cursor on screen). ⇒ `↖` (U+2196 + VS16). ⚠ **Change the `ToolbarButtons.CATALOG` icon and the one the toolbar actually draws together** — change one alone and the settings screen and the toolbar disagree on the glyph.
