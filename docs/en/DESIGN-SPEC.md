...
Last updated: 2026-06-22 / Target version: 0.8.121-alpha (versionCode 129)
...
- `TerminalEmulator`: processes byte streams with a state machine (Ground/Escape/CSI/OSC…).
  ...
  - DEC modes: alternate screen, cursor keys (DECCKM), **mouse reporting** (X10/Normal/Button/Any × Legacy/SGR/urxvt). While mouse reporting is ON (the TUI enabled `?1000` / `?1006` etc.), swipes inside `TerminalInputView` are **routed by screen type, direction and scrollback position**: on the **alternate screen** (lazygit/vim/htop/btop/nano etc., `buffer.primaryActive == false`) there is no scrollback, so **both directions** are forwarded to the PTY as wheel events (0.8.119; finger up = wheel-down/button 65, finger down = wheel-up/button 64). Without this, downward swipes fall through to the scrollback fallback, which is a no-op when `scrollbackSize == 0`, and the user can only scroll one way ("touch slide only scrolls down, not up" report). On the **primary screen** the previous behaviour is preserved: **finger going up (= "advance")** is sent as wheel-down only when **scrollback is at the bottom (scrollOffset == 0)**. While scrollback is non-zero (the user is looking at past lines), upward swipes are absorbed by scrollback and pull the view back toward the latest line instead of sending a wheel notch (0.8.116; otherwise `TerminalSession.writeBytes` would reset scrollback to 0 on the very first wheel byte and snap the view to the bottom in one go — the "jumps to bottom" complaint). On the primary screen **finger going down (= "look back")** still falls back to scrollback control (many reader TUIs deliberately ignore wheel-up and "leave it to the terminal scrollback"). One notch is sent every time the cumulative finger dy crosses `MOUSE_WHEEL_STEP_PX (=40px)`, so a long swipe produces a multi-line scroll (on the alt screen the accumulator is signed so direction reversals cancel naturally). Flings follow the same gate: on the primary screen no-op only when `mouseEnabled && velocityY < 0 && scrollOffset == 0`, otherwise drive scrollback inertial scroll; on the alt screen the gate is skipped and `sendMouseWheelRows` converts each inertial frame into a wheel notch on the PTY (so lazygit etc. keep an inertial feel).
