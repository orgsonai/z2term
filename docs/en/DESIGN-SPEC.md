...
- ⚠ **Input is not implemented** (`sendPointerEvent` / `sendKeyEvent` are empty). The UI says so — **staying quiet about what does not work reads as "broken"**.
