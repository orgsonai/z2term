...
Last updated: 2026-08-31 / Target version: 0.8.452-alpha (versionCode 460)
...
- **RDP is not yet a user-facing feature (the 0.8.450–0.8.451 NLA spike)**: `gui/rdp/` implements X.224 Enhanced Security negotiation, TLS upgrade with an explicit certificate-approval boundary, CredSSP `TSRequest` DER framing, all three NTLMv2 messages with SPNEGO and MIC, connection-oriented SIGN/SEAL, the v5/v6 public-key binding exchange, and encrypted `TSPasswordCreds`, without adding a dependency. Official crypto vectors and a scripted end-to-end CredSSP peer cover the wire path; a live Windows NLA probe also reached its NTLM challenge through this implementation. ⚠ Completing a real login still needs credentials entered locally; MCS, display updates, input and connection UI are not wired, so no `[RDP]` action is shown yet.
