...
Last updated: 2026-06-13 / Target version: 0.8.85-alpha (versionCode 93)
...
- Idempotently injected on every launch: `ensureShellHistoryConfig` (history rc), `ensureSshdWrapper` (`/usr/local/sbin/sshd` = dropbear wrapper), `ensureOsc7CwdConfig` (OSC7 hook for cwd restore), `ensureZ2ApiScripts` (`z2-*` bridge), GUI/z2run scripts, `ensureVersionScript` (`/usr/local/bin/z2version`).
