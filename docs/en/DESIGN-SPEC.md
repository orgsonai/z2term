...
Last updated: 2026-07-29 / Target version: 0.8.287-alpha (versionCode 295)
...
- **`z2-tile` (0.8.258 — quick-settings tiles)**: put a macro or command on up to four **quick-settings** tiles. `z2-tile set <1-4> <macro.sh|command...> [-l label]` / `list` / `clear <1-4|all>`.
  ...
  - **The label comes from the first word**, so the same macro on two slots gets **the same name** (`remind.sh ask` and `remind.sh peek` are both `remind`). Use `-l` to tell them apart. ⚠ Arguments are not folded into the name: there are only four slots and long names are silently clipped on some devices.
