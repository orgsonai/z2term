...
Last updated: 2026-08-31 / Target version: 0.8.457-alpha (versionCode 465)
...
- **RDP is not yet a user-facing feature (0.8.450–0.8.455)**: `gui/rdp/` covers X.224/TLS, CredSSP/NTLMv2, T.124 GCC and T.125 MCS, Client Info, licensing, Demand/Confirm Active, and connection finalization. The opt-in live test completes Client Synchronize, Control Cooperate/Request/Granted, and Font List/Map against the on-device NLA-enforcing FreeRDP server. Surface Commands and Bitmap Codecs remain intentionally omitted so the next stage can target classic bitmap updates. ⚠ Bitmap decoding, input and connection UI are not wired, so no `[RDP]` action is shown yet.
