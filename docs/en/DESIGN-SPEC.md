...
Last updated: 2026-09-04 / Target version: 0.8.500-alpha (versionCode 508)
...
- ⭐ **☰ application sheet (0.8.499) — a permanent way into the applications**: getting the right-click menu back only restores an entrance **you have to know about** (long-press an empty part of the desktop). What was actually asked for is the equivalent of a Windows start button, so the GUI toolbar gained `ToolbarButtons.APPS` (☰, `guiOnly = true`), which opens the sheet in `ui/gui/GuiAppsSheet.kt`. Tapping a row hands it to `GuiSession.launchApp`, which starts it through `z2run`.
  ...
  - ⚠ **Never wait on `z2menu list` forever** (20 s cap). If it does not return, one tap on ☰ leaves the sheet loading for good. Reading a few dozen `.desktop` files measures under a second in practice.
