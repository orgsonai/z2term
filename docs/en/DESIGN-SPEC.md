...
Last updated: 2026-08-31 / Target version: 0.8.458-alpha (versionCode 466)
...
- `GuiSession`/`GuiActivity`/`GuiScreen`/`GuiViewport`/`GuiInputView`/`GuiKeyMapper`/`GuiEventWatcher` + `gui/RemoteDesktopClient.kt` (the shared drawing/input boundary) + `gui/rfb/RfbClient.kt` (the built-in RFB implementation). Pairs a terminal tab with a GUI tab with IME linkage. `GuiSession`, Compose drawing, the input View and the GUI keyboard no longer require the concrete `RfbClient` type, so an RDP implementation can enter through the same boundary (0.8.450).
- **RDP is not yet a user-facing feature (0.8.450–0.8.458)**: `gui/rdp/` covers X.224/TLS, CredSSP/NTLMv2, T.124 GCC and T.125 MCS, Client Info, licensing, Demand/Confirm Active, connection finalization, and slow-path classic Bitmap Update reception. The internal `RdpClient` expands clipped multi-rectangle 15/16/24bpp updates (uncompressed or Interleaved RLE) into an ARGB framebuffer and publishes dirty redraw notifications. Fast-Path, Surface Commands, Bitmap Codecs, individual drawing orders, 32bpp RDP 6.0 compression, input, clipboard, resize, and connection UI are intentionally not advertised or wired, so no `[RDP]` action is shown yet.
