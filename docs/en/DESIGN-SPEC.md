...
Last updated: 2026-09-02 / Target version: 0.8.479-alpha (versionCode 487)
...
- ⛔⛔ **Unresolved as of 0.8.472: against real Windows the connection completes but not one byte of screen data arrives.** NLA → MCS → activate → finalization all complete, the tab reaches `CONNECTED` at `2400x1080`, Save Session Info (`pduType2=0x26`) arrives, and then `readTpkt` blocks without reading a single byte (it is not fast-path either — that would fail the TPKT header check). Setting `refreshRectSupport=1` and requesting a full-screen Refresh Rect on connect changes nothing. ⇒ **Stop guessing and diff against an `xfreerdp3` TRACE** (point `scripts/rdp-testbed.sh trace` at the real host; needs its credentials). See `99_private/HANDOFF/z2term/RDP-HANDOFF.md`.
