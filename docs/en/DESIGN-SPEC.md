...
- ⚠ **The GUI terminal breaks in the shape of "installed, but no window"** (0.8.343, reported on-device: picking rxvt-unicode / Konsole on Alpine leaves no terminal). `ensure_pkgs` decides "installed" from **`has $GUI_TERM_BIN` (does the binary exist)**, so anything that is present but *fails to start* slips straight through and `z2gui` runs to completion with only the desktop up. All the user sees is "the desktop appears but there is no terminal".
  - **Root cause (urxvt on Alpine)**: the `apk` server set had **no core fonts**. `apt` installed `xfonts-base` and `pacman` `xorg-fonts-misc`, but `apk` only had `font-noto ttf-dejavu` (TrueType), so **a terminal that defaults to the core font `fixed` cannot start**. → `font-misc-misc font-alias` added.
