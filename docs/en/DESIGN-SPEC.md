...
Last updated: 2026-08-29 / Target version: 0.8.438-alpha (versionCode 446)
...
- `ssh/SshProfilesSheet.kt` + `HostKeyVerificationDialog.kt`: shared SSH / WebDAV / SMB destination UI + SSH key verification. Protocol is stored per entry; legacy JSON defaults to SSH. Non-SSH entries never expose shell, VNC or resident-tunnel actions.
- `sftp/SftpSheet.kt`: shared SFTP / WebDAV / SMB file browser (**full-screen page**) backed by `RemoteFs`. WebDAV uses OkHttp with normal TLS certificate validation; SMB uses jcifs-ng with SMB1 disabled. Scrolling the listing downwards collided with the ModalBottomSheet close drag and dismissed the sheet, so it moved to the same separate-page style as the settings page. The back arrow / system back returns to the previous screen.
