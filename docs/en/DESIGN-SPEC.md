...
Last updated: 2026-09-01 / Target version: 0.8.477-alpha (versionCode 485)
...
- **RDPGFX (0.8.477)**: ⛔ **Windows 11 does not fall back to classic Bitmap Updates for a peer that cannot use RDPGFX.** This was measured, not guessed: `xfreerdp3` with the graphics pipeline turned off goes just as silent (connection and finalization complete, and not one screen PDU arrives). So the GCC CS_NET request now includes **`drdynvc`** (`RdpDynamicChannel`); after the DVC capability/create exchange the client opens `Microsoft::Windows::RDS::Graphics` and receives the screen over **RDPGFX** (`RdpGfx`). CS_CORE declares 32bpp together with `RNS_UD_CS_SUPPORT_DYNVC_GFX_PROTOCOL` (⚠ **announcing GFX while omitting 32bpp contradicts itself**). Only the CAPVERSION_8 thin-client capability is advertised; surface create/delete/map-to-output, frame acknowledgement, solid fill, surface-to-surface copy, the surface cache, 32bpp uncompressed data and **ClearCodec** (`RdpClearCodec`) are handled. Incoming data is wrapped in RDP 8.0 bulk compression, so `Rdp8Bulk` decodes it here as well (outgoing PDUs stay uncompressed, exactly as FreeRDP sends them).
  ...
  - ⚠ **This Windows 11 chose RemoteFX (CAVIDEO, codec 0x3) and uncompressed (0x0)**, and there is no RemoteFX decoder yet, so the screen is still black as of 0.8.477. That is the next piece.
