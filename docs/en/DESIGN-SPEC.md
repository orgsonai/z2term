...
Last updated: 2026-08-14 / Target version: 0.8.348-alpha (versionCode 356)
...
- **`z2-icon` (0.8.294 — draw the status-bar and tile icons yourself)**: `z2-icon pick <target>` / `sample [name|target name]` / `edit <target>` / `set <target> [file|-]` / `show <target>` / `clear <target|all>` / `list`. The targets are **`notify`** (one drawing, used by every notification this app puts out) and **slots 1-12** (one drawing each).
  - **Why a dot drawing**: Android **repaints both the status-bar icon and the tile icon in a single colour of its own** (tiles change colour between on and off), and they end up about 24px across. ⚠ **There is no colour to choose — only the shape gets through.** A 24x24 black-and-white grid is therefore the whole of the available expressiveness, and taking a drawing instead of an image means **what you write is what appears** (what `show` prints is what you get), rather than downscaling and thresholding something behind your back.
  ...
  - **Blank space is ignored and the drawing is re-centred.** Lines and columns need not add up to 24, and `$(cat)` dropping trailing blank lines stops mattering. ⚠ Conversely **a drawing that is too big is refused** (ink outside 24x24 is an error): trimming it silently delivers **an icon with its edges missing** to the one person who cannot tell why.
...
- ⚠ **When "nothing is painted until you tap", the X server is not asleep — it is spinning** (0.8.346, confirmed on-device). The earlier reading was "an edge-triggered epoll missed a notification and it is waiting"; the measurement says the opposite:
